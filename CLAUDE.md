...
| `GET /api/bot/order-packages?limit=N&since=ISO_TS&strategy=X` | `{rows:[{orderPackageId, createdAt, updatedAt, strategy, symbol, direction, entry, sl, tp, confidence, status, closeReason, linkedTradeId, pnl, tradeStatus, signalLogic, meta, claudeScore}], count, claude_log_present}` — **decision-level** view (one row per order package = the bot's actual decision). `signalLogic` / `meta` are the decision reasoning the bot recorded at signal time (`order_packages.signal_logic` / `meta` TEXT columns, JSON-decoded to whatever shape the writer used; `meta` typically carries setup_type / killzone / bias) — `null` when unset; they power the dashboard's open-trade detail card. `claudeScore` = the Claude strategy-decision grade `{grade, score, entryQuality, exitQuality, riskManagement, executed, rationale, reviewedAt}` or `null` until a `/health-review` scores it. Per-model **shadow scores are NOT joined here** — the dashboard composes those from `/api/bot/trades/scores` by `linkedTradeId`. | `trade_journal.db::order_packages` LEFT JOIN `trades` (PnL + backtest/demo filter) + `comms/claude_strategy_scores.jsonl` (by `order_package_id`); Tier 1 |
