...
| `LOCAL_PNL_COMPUTE_DISABLED` | Kill-switch for the **local-PnL fallback** — the universal half of the bot's **PnL-resolution contract** (`src/runtime/local_pnl.py` + `src/runtime/order_monitor.py::_sweep_local_pnl_for_unpriced`, BL-20260616-IBKRPNL). **Default OFF → fallback ON (baseline correctness, no default-off gate per the Prime Directive); set truthy to roll back without a redeploy.** Every account resolves realised PnL the same way: **prefer broker truth, else local compute.** Whether an integration *can* provide broker truth is a **declared integration capability** — `src.units.accounts.clients.BROKER_PNL_READER_EXCHANGES` (today `{bybit}`), checked via `account_has_broker_pnl_reader()` — **not** a hardcoded special-case. Integrations with a reader (Bybit) get fee-accurate `closedPnl` from `_sweep_pending_pnl_from_bybit`; integrations without one (**IBKR `ib_paper` MES/MGC/MHG**, Alpaca/OANDA paper) get local compute — `(exit−entry) × qty × direction × contract_value_usd` (per-contract multiplier from `config/instruments.yaml`; MGC=10, MHG=2500, MES=5, crypto=1) using the recorded `exit_price` else a **mark-to-market** last close. Broker-reader rows are deferred to the Bybit sweep within the broker retention window (7d) and only local-filled once older (so fee-accurate truth is never pre-empted, but an abandoned row is rescued from a permanent `$0.00`). Stamps `notes.pnl_source="local_compute"` + `notes.exit_price_source`; re-links a NULL `order_package_id`. Declaring a new integration's PnL source is a `new-broker` skill step (2b). Sibling: the dashboard `/api/bot/positions` server-side mark-to-market **unrealised** fallback (`dashboard.py::_local_unrealised_for_trade`, `unrealizedPnlSource="markprice_local"`) — same broker-first→local contract for open positions. |
