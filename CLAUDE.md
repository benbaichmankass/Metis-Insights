...
**THE TWO FRONTENDS REACH THE BOT OVER DIFFERENT TRANSPORTS — do not
generalise from one to the other** (documented 2026-08-13,
...
- **Streamlit** — the Python server makes the upstream **plain-HTTP** call to
  `http://141.145.193.91:8001` **directly, server-side**. No Vercel rewrite, no
  Cloudflare tunnel, no `cf-worker`. CORS is not load-bearing here.
...
So the SPA's failure set is **not** the Streamlit one — it adds three modes
Streamlit does not have: the **Caddy process**, the **DuckDNS record**, and
**LE cert renewal**. Any claim that the failure set "collapses to Streamlit
Cloud, the VM's FastAPI, and this script's code" describes Streamlit only.
...
Caddy outage — which takes the SPA down while Streamlit stays green — is at
least visible on `/api/diag/services`. Nothing yet watches the cert expiry or
...
VPS (systemd)
  ...
  └── ict-web-api.service     ─── FastAPI :8001
                                   ...
                                   ├── /api/bot/backtests← Streamlit dashboard (historical rows; writer retired 2026-08-20)
                                   ...
                                   ├── /ws/market        ← Android app (WebSocket: live candles + positions, P2b)
                                   └── /api/health
```
...
| `GET /api/bot/learning/courses` | `{present, count, courses:[{course_id, title, subtitle, episode_count, quiz_count, has_audio}]}` — index of the **interactive courses** (audio + quiz modules) the dashboard + Android course players render. Best-effort: `present:false` when the dir is missing. File-backed, Tier 1, read-only. | `comms/learning/courses/*.json` via `src/web/api/routers/learning.py` |
...
| `POST /api/bot/learning/progress` | upsert one resource's progress — body `{resource_id, status∈{not_started,in_progress,done}, note?}` → the stored row. **Unauthenticated** client self-service write (same shape as `POST /devices/register`), so BOTH the dashboard and the Android app can record progress without holding `DASHBOARD_API_TOKEN`; 400 on a bad status / missing id. **Tier 1** (operator observability — no trading impact, no order path, no notification; the store holds no secret). | `trade_journal.db::learning_progress` via `src/web/api/routers/learning.py` |
...
| `BYBIT_HEDGE_MODE_SYMBOLS` | CSV of `<account_id>:<SYMBOL>` pairs that trade in Bybit **HEDGE** position mode (`src/runtime/bybit_position_mode.py`, T.2, 2026-08-22, `BL-20260821-PAIRS-SOL-ETH-STRANDS-ON-EVERY-OPEN`). **EMPTY BY DEFAULT, AND THAT IS THE SHIPPED STATE** — no call site adds a `positionIdx` kwarg, and the wire payload is **byte-for-byte** what it was before, asserted by a test rather than by inspection. ⚠️ **THE PLUMBING IS NOT THE SWITCH.** This resolver makes the code *expressible* in hedge mode; it deliberately does **NOT** call Bybit's `switch_position_mode`. Mutating a venue's position mode is Tier-3 and operator-gated; being ready for it is Tier-1, and conflating the two is how a "no behaviour change" PR changes behaviour. **Why it exists:** `bybit_1` is one-way netting, so a market-neutral pairs leg opposite a concurrent directional position does not open a book — it REDUCES the standing one. Measured 2026-08-21 against exchange truth, every SOLUSDT/ETHUSDT pair since 08-18 stranded, **8 of 8** (`/api/diag/exchange_positions` SOLUSDT Buy 373.0 = `trend_donchian_sol_4h` 367.8 + `pairs_sol_eth_a` 5.2, exactly). A long spread kills the short leg and a short spread kills the long leg, hence 8/8 rather than intermittent. ⚠️ **`positionIdx` NAMES THE BOOK, NEVER THE ORDER SIDE.** Closing a LONG sends `side="Sell"` and belongs to `positionIdx=1`; a resolver keyed on order side passes every naive test and silently acts on the opposite book. Callers pass the POSITION's direction; the reduce-only path inverts once, at the boundary. **Four states, never collapsed:** `one_way` (no hedge declared — an absent kwarg is CORRECT) · `hedge_long` (1) · `hedge_short` (2) · `unresolved` (hedge IS declared but the side could not be read — sends NO `positionIdx` so Bybit **refuses**, logged WARNING; guessing a book would be a live order against the wrong position). Scoped per SYMBOL because Bybit's position mode is itself per-symbol. Read at call time (an env flip needs no redeploy); a malformed entry is DROPPED, never widened. Not yet registered with `collapsed-state-guard` — while the allowlist is empty the hedge states have no production consumer by construction, so registering it today would either fail the guard or invite a decorative branch; it becomes registrable in the change that first arms the allowlist. |
