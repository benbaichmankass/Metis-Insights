...
| `FLIP_CONFIDENCE_THRESHOLD` | **Confidence-gap override for the hold policy** (Tier-3; `src/runtime/intents.py::resolve_flip_confidence_threshold`). Default `0.0` — **disabled, hold-policy behaviour is unchanged**. When set to a positive float (e.g. `0.15`), the `hold` policy may be overridden for a signal whose confidence exceeds the existing position's entry confidence by at least this gap (`new_conf − existing_conf ≥ threshold`). The flip is allowed only when BOTH this gap AND the age gate (`FLIP_MIN_POSITION_AGE_HOURS`) are satisfied; either alone is insufficient. Existing confidence is read from `trade_journal.db` via `get_existing_position_info` (fail-permissive: a read failure keeps the hold, never flips). Logged at INFO with `hold_confidence_override` reason prefix in the delta. **Requires operator approval (Tier-3 order-routing change) before deploying.** ⚠️ **LIVE VALUE (measured 2026-08-10 via `get-env`, issue #8755): `0.15` — i.e. NOT the documented default, and the override IS ACTIVE** (with `FLIP_MIN_POSITION_AGE_HOURS=4.0`; `FLIP_POLICY` unset → `hold`). Do NOT reason about live routing from the "default 0.0 = disabled" sentence above — the field is the truth and it disagrees. No authorization record was found in `ROADMAP.md`, `docs/sprint-logs/`, or `docs/claude/`, and `docs/research/M26-regime-transition-conflict-DESIGN.md` still calls this knob dormant and **never validated**; the `.env` edit history and operator chat are outside what a session can check, so this is "no record in the repo", NOT "unauthorized". Under operator review: `BL-20260810-FLIP-CONFIDENCE-OVERRIDE-LIVE-WITHOUT-RECORD`. This row is deliberately NOT rewritten to present 0.15 as the norm — that would launder an unreviewed Tier-3 state into documentation. |
| `FLIP_MIN_POSITION_AGE_HOURS` | **Minimum position age for the confidence-gap flip override** (Tier-3; `src/runtime/intents.py::resolve_flip_min_position_age_hours`). Default `0.0` — no age requirement (the gap alone is sufficient when `FLIP_CONFIDENCE_THRESHOLD > 0`). When set (e.g. `4.0`), the confidence override is suppressed for positions younger than this many hours, protecting fresh trades from being reversed by the next signal. Works in tandem with `FLIP_CONFIDENCE_THRESHOLD`: both must pass. Age is derived from `trades.created_at` (epoch-ms or ISO string); if unparseable, treated as 0h (fail-permissive — never strands a genuine old position). ⚠️ **LIVE VALUE (measured 2026-08-10, issue #8755): `4.0`, not the documented `0.0`** — see the `FLIP_CONFIDENCE_THRESHOLD` row above and `BL-20260810-FLIP-CONFIDENCE-OVERRIDE-LIVE-WITHOUT-RECORD`. |
