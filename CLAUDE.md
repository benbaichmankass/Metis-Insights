...
| `PROP_STATUS_REQUEST_MAX_AGE_HOURS` / `PROP_STATUS_REQUEST_COOLDOWN_HOURS` | Freshness threshold (default `24`h; `<= 0` pauses) + re-ask cooldown (default `12`h) for the prop **account-status request** — the manual bridge has no broker balance feed, so while a prop position is open and the latest `prop_account_status` snapshot is missing or older than the threshold, the bot asks the operator for a fresh `bal <balance> <equity>` (`src/prop/prop_status_request.py`, once per trader tick). The SAME threshold now also gates the **folded balance nudge** appended to every fill ack (`telegram_report_handler.account_status_nudge`), so logging a trade prompts for the balance inline instead of relying only on the periodic ping — the two share the threshold so they never double-nag (a fresh snapshot suppresses both). Baseline, no enable gate. |
