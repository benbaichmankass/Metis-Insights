...
| `GET /api/diag/tick_cost` | `{present, path, generated_at, age_seconds, ticks_measured, last_ms, max_ms, max_at_utc, mean_ms, process_started_utc, hooks{<name>:{n,mean_ms,max_ms,max_at_utc,pct_of_total}}, hooks_attributed_mean_ms, attributed_pct, nested_hooks, hook_names_refused}` — **per-tick wall-clock cost of the trader's hook chain** (2026-08-09). `src/main.py`'s tick runs a dozen best-effort hooks (order monitor · pairs executor · macro thesis · five prop prompts · two reachability alerts · IB-state dump · exposure soak). **Each is individually bounded and documented as cheap; nothing measured the SUM** — which is the shape of BOTH June 2026 wedges (`MB-20260609-001` steady-state, `BL-20260609-001` cold-start): each new component was cheap in isolation and the total was never watched, so it grew un-observed and the next hook added will be individually cheap too. Read **`max_ms` beside `ticks_measured`** — the max is the statistic that matters (a mean that looks fine while the peak freezes the heartbeat IS the 2026-06-09 incident), and a max over 3 ticks is not the claim a max over 3000 is; `max_at_utc` dates the peak. Counters are **per-PROCESS** (a restart resets them; `process_started_utc` says from when). The in-memory max updates EVERY tick while the file persists on the `TICK_COST_WRITE_SECONDS` cadence, so a quiet write cadence never loses a spike. **MEASUREMENT ONLY — no budget is enforced**: a cap with no distribution behind it is the exposure-ceiling mistake (`gross-exposure-governance-DESIGN.md` § 6 — a ceiling below normal operation silently throttles correct work). **PER-HOOK SPLIT (2026-08-10):** the sum alone could not say WHERE the time goes, which is what the operator's "no live trade goes >60s without evaluation" ask needs. `hooks` carries a bounded (`_MAX_HOOK_NAMES` **64** since 2026-08-12, was 32; overflow COUNTED in `hook_names_refused` so a truncated split cannot read as complete) per-hook block, and **`attributed_pct` is the load-bearing field** — `100 - attributed_pct` is every uninstrumented hook COMBINED. Reporting instrumented hooks without their coverage would invite the reader to conclude those hooks ARE the cost when they are a lower bound on it (the `rCoverage`/`pnlCoverage` discipline). **NESTING (2026-08-11): the block is no longer flat** — **BOTH** top-level hooks are now parents: `order_monitor` of 14 `monitor.*` phase children (the per-strategy loop + 13 reconcilers/sweeps), and `run_one_tick` of 4 `pipeline.*` children (`regime_bar_scoring` · `signal_build` · `news_score` · `dispatch`, added 2026-08-12 — the warm read had localised the tick to two near-equal halves, 56.2s + 49.3s, and only the monitor half was broken out). The `pipeline.*` children aggregate **across symbols** (the tick fans out per symbol), so their `n` is symbols × ticks, not ticks — a per-symbol cut was deliberately NOT taken because it costs one name per instrument against `_MAX_HOOK_NAMES`. Name budget: the live chain MEASURED **27 of 32** after both splits (the earlier "20 of 32" was a projection, not a reading — the live payload of 2026-08-12T21:55Z is the count), which is why the bound was raised to 64 before adding a third cut. `attributed_pct` counts **top-level hooks only** (a dotted name is a child), because a child's time is already inside its parent's and summing both double-counts: the first read after the split deployed returned **`attributed_pct: 136.8`** — a share of a whole exceeding the whole, making `100 - attributed_pct` read as **−36.8%**. `nested_hooks` reports how many children the block holds, so a reader can see why the listed per-hook means do not sum to `attributed_pct` (0 = flat, and then they agree exactly). Per-hook `pct_of_total` is each hook's own share of tick time and stays valid for a child; only SUMMING parents with children was ever invalid. **>100% was the lucky version of that bug** — impossible on its face, so it announced itself; a double-count landing at 95% would have read as excellent coverage. **`offloop_hooks` IS A SEPARATE BLOCK AND ITS WORK IS NOT IN `mean_ms`** (documented 2026-08-12 — it was emitted by the M20 exit-loop decouple and undocumented here until a read tripped over it). `record_hook` routes any hook running on a thread other than the tick's into `offloop_hooks`, so once the decouple moved `monitor.strategy_monitor_loop` off the tick, the tick mean fell 107.9s → 83.9s **without the system doing less work** — it became concurrent. Read the two blocks together or a decouple reads as an optimisation. **`fetchby.*` (added 2026-08-12) is a SECOND CUT of the same fetch seconds, not additive with `fetch.*`** — each fetch records once in each family (timeframe, and the `pipeline.*`/`monitor.*` phase that asked, via a ContextVar `tick_cost.current_phase()` that `hook()` publishes and `fetch.*` names deliberately do not overwrite). Compare within a family; summing across them double-counts every fetch exactly once. A fetch outside every instrumented phase is labelled `fetchby.unattributed` and keeps its own bucket rather than being folded into a neighbour. `_MAX_HOOK_NAMES` was raised **32 → 64** for that cut: the live chain measured **27 of 32** before it, so a second cut would have overflowed into `hook_names_refused` and a truncated split reads exactly like a complete one. `None`, not `0`, before the first tick. A hook that RAISES is still timed (the wrapper records in `finally` and never swallows — each hook keeps its own handler), and `src/main.py::_tick_hook` falls back to `nullcontext()` if the measurement module cannot be imported, because this wraps the live loop. `present:false` until the trader writes it; a large `age_seconds` while the trader ticks means the writer is not running. Pure file read — no socket, no order path. Tier 1 | `runtime_logs/tick_cost.json` (written by `src.runtime.tick_cost` from the `src/main.py` tick) |
