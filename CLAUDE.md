...
| `GET /api/bot/signals` | recent ICT detections — each carries `strategy`, `pattern`, `confidence`, `price`, and `zones[]` (drawable decision geometry the strategy already logged: `{kind:"fvg",low,high}` + `{kind:"sweep",price}` for ict_scalp) | **DB-canonical (WC-5 cutover, 2026-06-16):** `trade_journal.db::signals` (the dual-write target; `meta` JSON expanded back to top-level so the shape matches a JSONL record), filtered to buy/sell. `runtime_logs/signal_audit.jsonl` is the append-only audit **fallback** — served when `SIGNAL_DUAL_WRITE_DISABLED` is set (the coupled rollback) or the table isn't hydrated yet. `zones` are assembled from geometry the signal builder records (e.g. `fvg_low/high`, `sweep_level`) — never a separately-computed indicator. |
...
| `SIGNAL_DUAL_WRITE_DISABLED` | **The single coupled rollback for the WC-5 signals cutover.** When truthy: (1) `signal_audit_logger._dual_write_to_db` skips hydrating `trade_journal.db::signals`, AND (2) the `/api/bot/signals` reader falls back to the `signal_audit.jsonl` audit instead of the (now-frozen) DB. Default off → dual-write on + reader DB-canonical. The dual-write is **fail-loud** (a DB-write failure escalates once per episode to an ERROR outcome, since reads come from the DB) but never raises into the pipeline. Toggle on the live VM via the `enable-signal-dual-write` / `disable-signal-dual-write` operator actions. |
...
| `GET /api/diag/log_file?name={audit\|status\|heartbeat\|bot_log\|advisory_decisions\|shadow_predictions\|shadow_predictions_backfill\|ibkr_mes_pull\|news_decisions\|conviction_sizing\|conviction_arbitration}&lines=N` | log file tail. `conviction_sizing` (P2) / `conviction_arbitration` (P3) are the observe-only unified-confidence soak logs — the would-be conviction size / arbitration vs the actual, never order-influencing; tail them to verify the soak is accruing before P4/P5 graduate it. |
