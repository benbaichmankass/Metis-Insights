...
| `GET /api/bot/notifications` | `{generated_at, count, has_alerts, banners:[{severity∈alert\|warning\|info, kind∈trainer_down\|account_down\|trade_open, message, detail, since}]}` — the **banner feed** the dashboard + Android render at the top of Overview for can't-miss conditions (NOT routine pings): **trainer_down** (the trainer VM's 2-min mirror heartbeat stale ⇒ ML training stalled; from `trainer_reachability_alert.status()`), **account_down** (one per latched-down live broker account, from `account_reachability_alert.down_accounts()`), and **trade_open** (best-effort compact "N trades opened in the last `TRADE_OPEN_BANNER_WINDOW_MIN`m" info line). Banners sorted alert-first. **Connection-free, best-effort** (any source failure omits that kind, never 5xxs). Tier 1 (operator-requested 2026-07-08). | `src/runtime/trainer_reachability_alert.py` + `src/runtime/account_reachability_alert.py` + `trade_journal.db::trades` (`src/web/api/routers/notifications.py`) |
