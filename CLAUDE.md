...
| `LOCAL_PNL_COMPUTE_DISABLED` | Kill-switch for the **local-PnL fallback** (`src/runtime/local_pnl.py` + `src/runtime/order_monitor.py::_sweep_local_pnl_for_unpriced`, BL-20260616-IBKRPNL). **Default OFF → fallback ON (baseline correctness, no default-off gate per the Prime Directive); set truthy to roll back without a redeploy.** The Bybit closed-pnl sweep (`_sweep_pending_pnl_from_bybit`) is **Bybit-only** (`account_closed_pnl_for_trade` returns `None` for IBKR/Alpaca/OANDA), so non-Bybit (incl. **`ib_paper` MES/MGC/MHG**) closed/orphaned trades never got a realised PnL and rendered `$0.00`. This sweep computes it locally — `(exit−entry) × qty × direction × contract_value_usd` (the per-contract multiplier from `config/instruments.yaml`; MGC=10, MHG=2500, MES=5, crypto perps=1) — using the recorded `exit_price` when present, else a **mark-to-market** last close from the bot's own candle feed. **Bybit accounts are skipped** (broker truth owns their PnL). Stamps `notes.pnl_source="local_compute"` + `notes.exit_price_source`. Also opportunistically re-links a row whose `order_package_id` is NULL back to its package. Runs every monitor tick after the Bybit sweep; best-effort. Sibling: the dashboard `/api/bot/positions` server-side mark-to-market **unrealised** fallback (`dashboard.py::_local_unrealised_for_trade`, `unrealizedPnlSource="markprice_local"`) for open IBKR/paper positions when the broker read is unavailable. |
