...
- **`OI-20260901-SCHEDULED-PROBES-AND-DUE-LIST-HAVE-NEVER-FIRED-ON-CRON`** — probes.yml AND due-list.yml have ZERO `schedule` runs since they landed 2026-08-31T14:20Z. Scheduling in this repo WORKS (health-snapshot: 438 schedule runs, latest 2026-09-01T04:24Z), so this is neither a repo-wide outage nor probes-specific — it is specific to the pair added in one commit. Whether it is first-window lag or permanent is UNDECIDED.
  - **Clears when:** A `schedule`-event run of probes.yml appears in the Actions history — i.e. actions_list on probes.yml filtered to event=schedule returns total_count >= 1. ⚠️ A workflow_dispatch run does NOT clear this and must not be read as clearing it: dispatch is already proven to work and is what produced run #32; the open question is entirely whether the CRON fires. ⚠️ Nor does a refreshed PROBES.json — that file is written by whichever run produced it, dispatched or scheduled, so its mtime says nothing about the trigger. If the next window (2026-09-02 05:20Z) also produces nothing, the lag hypothesis is dead and the cause is in the workflow or in GitHub's registration of it — file it then rather than guessing now.
  - Last observed: `2026-09-01`. To clear for another cycle, set `verified_at` to today AND write what you saw into `observation` — a claim of progress is not an observation.
...
- **`OI-20260901-CLAUDE-CHANNEL-SEPARATION-SHIPPED-BUT-UNPROVEN`** — The dedicated Claude bot (@ict_cluade_bot) is CONFIGURED and its drain is FIXED and DEPLOYED, and pings still land in the TRADER chat. #10674 is merged (5c45ca52) and live on the VM (1bae542a); the bridge now resolves via claude_route() and at 21:37:08Z logged isolated=False with its own WARNING naming the missing key. What remains is one Tier-2 set-env putting TELEGRAM_CLAUDE_BOT_SECRET in the VM .env -- which this session could NOT dispatch (issue_write create -> 403) and which needs an operator OK it does not have.
  ...
  - Last observed: `never`. To clear for another cycle, set `verified_at` to today AND write what you saw into `observation` — a claim of progress is not an observation.
- **`OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE`** — The per-endpoint API reference (154,074 B: the /api/bot/* route table, BotStats, Position, CORS, and the /api/diag/* table) was moved VERBATIM out of CLAUDE.md into docs/reference/bot-api-reference.md, taking CLAUDE.md 430,453 -> 280,607 B (-34.9%). CLAUDE.md is the ONLY surface that reaches a session before it acts, so this deliberately removed that content from the pre-act channel. ⚠️ NOTHING HAS BEEN OBSERVED FINDING IT IN ITS NEW HOME. The whole risk of this change is the one the dispatch named: moving reference material into a file nobody opens is not a win if that material was preventing mistakes. Shipped and working are different states and only the first happened. verified_at is deliberately null: no observation has been made, and dating it today would be a claim of one.
