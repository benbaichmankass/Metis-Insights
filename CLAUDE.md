...
| `GET /api/diag/tick_cost` | `{present, path, generated_at, age_seconds, ticks_measured, last_ms, max_ms, max_at_utc, mean_ms, process_started_utc, hooks{<name>:{n,mean_ms,max_ms,max_at_utc,pct_of_total}}, hooks_attributed_mean_ms, attributed_pct, hook_names_refused}` — **per-tick wall-clock cost of the trader's hook chain** (2026-08-09). `src/main.py`'s tick runs a dozen best-effort hooks (order monitor · pairs executor · macro thesis · five prop prompts · two reachability alerts · IB-state dump · exposure soak). **Each is individually bounded and documented as cheap; nothing measured the SUM** — which is the shape of BOTH June 2026 wedges (`MB-20260609-001` steady-state, `BL-20260609-001` cold-start): each new component was cheap in isolation and the total was never watched, so it grew un-observed and the next hook added will be individually cheap too. Read **`max_ms` beside `ticks_measured`** — the max is the statistic that matters (a mean that looks fine while the peak freezes the heartbeat IS the 2026-06-09 incident), and a max over 3 ticks is not the claim a max over 3000 is; `max_at_utc` dates the peak. Counters are **per-PROCESS** (a restart resets them; `process_started_utc` says from when). The in-memory max updates EVERY tick while the file persists on the `TICK_COST_WRITE_SECONDS` cadence, so a quiet write cadence never loses a spike. **MEASUREMENT ONLY — no budget is enforced**: a cap with no distribution behind it is the exposure-ceiling mistake (`gross-exposure-governance-DESIGN.md` § 6 — a ceiling below normal operation silently throttles correct work). **PER-HOOK SPLIT (2026-08-10; attribution corrected 2026-08-11):** the sum alone could not say WHERE the time goes, which is what the operator's "no live trade goes >60s without evaluation" ask needs. `hooks` carries a bounded (`_MAX_HOOK_NAMES` 32, overflow COUNTED in `hook_names_refused` so a truncated split cannot read as complete) per-hook block, and **`attributed_pct` is the load-bearing field** — `100 - attributed_pct` is every hook NOT wrapped, combined.

⚠️ **Two things this sentence used to get wrong, both corrected 2026-08-11** (`BL-20260811-TICKCOST-ATTRIBUTED-PCT-DOUBLE-COUNTS-NESTED-HOOKS`). (a) The wraps are **no longer only** `run_one_tick` + `order_monitor` — a `monitor.*` layer of 14 CHILD hooks was added later, so there are 16, and the parenthetical claiming two was stale. (b) `hooks_attributed_mean_ms` summed **every** slot, counting each parent together with its own children: live that read **`attributed_pct: 136.8`**, making `100 - attributed_pct` **−36.8%**, which is not a quantity. Attribution now sums **top-level hooks only** (`hook()` tracks nesting depth), so the field is a real coverage bounded at 100 and the subtraction above is valid. **`pct_of_total` was never affected** — it is measured against `mean_ms` per row and each row was always individually correct; but the per-hook percentage COLUMN still sums past 100 by design, because a nested hook's time is also inside its parent's row. Each row carries **`nested`** so a reader knows which those are. Reporting instrumented hooks without their coverage would invite the reader to conclude those hooks ARE the cost when they are a lower bound on it (the `rCoverage`/`pnlCoverage` discipline). `None`, not `0`, before the first tick. A hook that RAISES is still timed (the wrapper records in `finally` and never swallows — each hook keeps its own handler), and `src/main.py::_tick_hook` falls back to `nullcontext()` if the measurement module cannot be imported, because this wraps the live loop. `present:false` until the trader writes it; a large `age_seconds` while the trader ticks means the writer is not running. Pure file read — no socket, no order path. Tier 1 | `runtime_logs/tick_cost.json` (written by `src.runtime.tick_cost` from the `src/main.py` tick) |
