...
| `IB_PROBE_TIMEOUT_S` | Hard cap (seconds, default `5.0`) on the post-connect **liveness probe** in `IBClient.connect()` (`src/units/accounts/ib_client.py`, PR #2827). A `reqCurrentTime` round-trip verifies the IB session is actually usable (a socket-accept is not proof); on timeout `connect()` raises `IBConnectionError` and trips the circuit breaker so the dead gateway can't block the trader loop. **Currently `<= 0` on the live VM, skipping the probe entirely** (2026-06-10, gateway-isolation): over the cross-host socat-relayed gateway VM, the FIRST `reqCurrentTime` round-trip sent immediately after `connect()` didn't resolve on a cold TCP flow even though the session was genuinely healthy (logs on, data farms OK, every OTHER call — reqHistoricalData, accountSummary — worked fine once the connection had exchanged a few messages), so the probe false-tripped the breaker and blocked MES. **2026-07-06 (BL-20260610-009):** `_probe_liveness` now retries once after `IB_PROBE_RETRY_GAP_S` (default 1.5s) before condemning the connection — absorbs a one-off cold-start relay miss on a healthy session while still catching a genuine wedge (which never answers either attempt). Re-enabling the probe on the live VM (setting this back to `5`) is a deliberate follow-up step, not yet done — verify against a real cold connect over the relay before flipping it live. With the gateway on its own VM it can no longer starve the trader's CPU, and `IB_FETCH_TIMEOUT_S` still bounds each fetch regardless of probe state. |
