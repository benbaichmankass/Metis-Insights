...
| `NEWS_ENABLED` / `NEWS_API_KEY` | The two gates that activate the **M9 news layer** (`src/news/`). **Both** must be set (enabled truthy + non-blank NewsAPI key) or the pipeline news hook is a silent no-op. Default off. Per-symbol queries/keywords live in `config/news_symbols.yaml`; full reference in `docs/news_layer.md`. |
