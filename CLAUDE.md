...
| `GET /api/bot/config` | effective config view (S-064) — each account carries its public fields incl. `symbols` (added 2026-06-11: the canonical per-account instrument list, so consumers enumerate traded symbols dynamically instead of hardcoding) | `config/accounts.yaml` + `config/strategies.yaml` + `runtime_logs/runtime_status.json`; secrets redacted |
...
| `GET /api/bot/order-packages?limit=N&since=ISO_TS&strategy=X` | `{rows:[{orderPackageId, createdAt, updatedAt, strategy, symbol, direction, entry, sl, tp, confidence, status, closeReason, linkedTradeId, pnl, tradeStatus, signalLogic, meta, modelScores, claudeScore}], count, claude_log_present}` — **decision-level** view (one row per order package = the bot's actual decision). `signalLogic` / `meta` are the decision reasoning the bot recorded at signal time (`order_packages.signal_logic` / `meta` TEXT columns, JSON-decoded; `meta` typically carries setup_type / killzone / bias). **`modelScores`** is the per-model ML decision captured at signal time — `{model_id: {stage, score}}` from the `order_packages.model_scores` column (populated by `strategy_signal_builders._emit_shadow_preds` → `shadow_adapter.capture_shadow_preds` → persisted in `_log_new_order_package`); `null` for pre-column rows. It's the cheap-SELECT replacement for recomputing per-trade aggregates from `shadow_predictions.jsonl`. All three are `null` when unset; they power the dashboard/Android open-trade detail card. `claudeScore` = the Claude strategy-decision grade `{grade, score, entryQuality, exitQuality, riskManagement, executed, rationale, reviewedAt}` or `null` until a `/health-review` scores it. | `trade_journal.db::order_packages` LEFT JOIN `trades` (PnL + backtest/demo filter) + `comms/claude_strategy_scores.jsonl` (by `order_package_id`); Tier 1 |
...
| `NAKED_POSITION_AUTOPROTECT` | Gate (default **off**) for the **broker-side SL/TP re-arm** on naked IB positions (`src/runtime/order_monitor.py::_check_naked_positions` → `_attempt_naked_autoprotect` → `IBClient.place_protective`, a GTC OCA stop+limit). **IB-only** (Bybit/OANDA/Alpaca attach SL/TP atomically at entry). When truthy, each monitor tick scans open live trades whose SL/TP is missing/non-positive, resolves the levels from the most recent matching order package (`_resolve_protective_levels`, direction + symbol-or-base-futures-root), and places a protective bracket — so an IBKR position that lost its bracket (orphan→adopt churn, the async-desync class, BL-20260612-001 / BL-20260615-MGCNAKED) doesn't sit naked. Same flag also gates the reverse-reconciler's **re-arm on adopt/re-attach** (`_rearm_broker_protection_after_recovery`), so recovering a package's SL/TP onto the journal row also re-places the broker stop instead of only updating the dashboard view. A new live-order surface → ships inert, operator-enabled via the `set-env` system-action; reversible by clearing the key + restart. |
