...
| `GET /api/diag/tick_cost` | `{present, path, generated_at, age_seconds, ticks_measured, last_ms, max_ms, max_at_utc, mean_ms, process_started_utc}` — **per-tick wall-clock cost of the trader's hook chain** (2026-08-09). `src/main.py`'s tick runs a dozen best-effort hooks (order monitor · pairs executor · macro thesis · five prop prompts · two reachability alerts · IB-state dump · exposure soak). **Each is individually bounded and documented as cheap; nothing measured the SUM** — which is the shape of BOTH June 2026 wedges (`MB-20260609-001` steady-state, `BL-20260609-001` cold-start): each new component was cheap in isolation and the total was never watched, so it grew un-observed and the next hook added will be individually cheap too. Read **`max_ms` beside `ticks_measured`** — the max is the statistic that matters (a mean that looks fine while the peak freezes the heartbeat IS the 2026-06-09 incident), and a max over 3 ticks is not the claim a max over 3000 is; `max_at_utc` dates the peak. Counters are **per-PROCESS** (a restart resets them; `process_started_utc` says from when). The in-memory max updates EVERY tick while the file persists on the `TICK_COST_WRITE_SECONDS` cadence, so a quiet write cadence never loses a spike. **MEASUREMENT ONLY — no budget is enforced**: a cap with no distribution behind it is the exposure-ceiling mistake (`gross-exposure-governance-DESIGN.md` § 6 — a ceiling below normal operation silently throttles correct work). `present:false` until the trader writes it; a large `age_seconds` while the trader ticks means the writer is not running. Pure file read — no socket, no order path. Tier 1 | `runtime_logs/tick_cost.json` (written by `src.runtime.tick_cost` from the `src/main.py` tick) |
