...
| `POST /api/bot/learning/progress` | upsert one resource's progress — body `{resource_id, status∈{not_started,in_progress,done}, note?}` → the stored row. Token-gated **permissively** via `DASHBOARD_API_TOKEN` (accepts when unset, like `/devices`; 401 on a wrong bearer); 400 on a bad status / missing id. **Tier 1** (operator observability — no trading impact, no order path, no notification). | `trade_journal.db::learning_progress` via `src/web/api/routers/learning.py` |
