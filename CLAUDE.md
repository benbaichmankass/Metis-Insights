...
| `CONVICTION_SIZING_MODE` | Gate for **P2 conviction-driven sizing** (`src/runtime/conviction_sizing.py`, unified-confidence redesign). `off` (default) / `annotate` (log the would-be resize to `runtime_logs/conviction_sizing.jsonl`, no change) / `apply` (resize). Mirrors `NEWS_INFLUENCE_MODE` (a mode, **not** a `*_ENABLED` gate). Applied in `Coordinator.multi_account_execute` right after `RiskManager.position_size`, beside the advisory/news downsizes. **UNLIKE** advisory/news it can **enlarge** the qty (replaces the flat risk pct with `conviction × 2% per-trade budget`, bounded by the available-margin ceiling + a proportional free-margin throttle), so it is demo-scoped + annotate-first. Reads the observe-only `meta.conviction` stamped at signal time; fail-permissive (any error / missing conviction / `sized_qty<=0` → qty unchanged). Rollback = `off` (one env flip, no redeploy). Tier-3. |
| `CONVICTION_SIZING_ACCOUNTS` | Comma-separated account allowlist for `CONVICTION_SIZING_MODE` (set to `bybit_1` for the demo soak). Because conviction sizing can grow an order, P2 starts **strict**: an **empty** allowlist is a no-op (the account must be named) — permissive-when-unset is deferred until after real-money sign-off (unlike `POSITION_NETTING_GUARD_ACCOUNTS`, which defaults permissive). Read at call time (next-tick effect). |
