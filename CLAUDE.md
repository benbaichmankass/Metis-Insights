...
| `IB_ACCOUNT_WARMUP_TIMEOUT_S` | Hard cap (seconds, default `8.0`) on the post-connect **account/portfolio warm-up** in `IBClient.connect()` (`src/units/accounts/ib_client.py::_warm_account_data`, BL-20260706-IBWARMUP). The liveness probe (`IB_PROBE_TIMEOUT_S`) only proves the session answers `reqCurrentTime` — it does not prove `balance()`/`positions()` have real data to read. `ib.accountSummary()` (used by `balance()`) only ever returns data from an explicit `reqAccountSummary()` subscription, which this repo never called anywhere; that lazy first call also has **no timeout of its own** (`ib_insync`'s `RequestTimeout` defaults to `0` = unbounded). `ib.portfolio()` (used by `positions()`) similarly depends on `reqAccountUpdates()`, which `ib.connect()`'s own init requests race against a timeout that is logged, not raised — a slow answer there never fails `connect()`. Net effect (root-caused 2026-07-06 after the symptom recurred twice the same day, most recently in the weekly `/system-review`, following an earlier same-day reclassification as a "diag-tooling limitation" in `docs/claude/health-review-backlog.json::BL-20260705-IBPAPER-POSITIONS-NULL-WEEKEND`): the FIRST `balance()`/`positions()` call after a fresh connect — or a reconnect after a silent idle-timeout drop on the cross-host socat relay — could race an empty/never-populated cache and misreport "gateway not logged in" on a perfectly healthy gateway. `connect()` now explicitly (re-)subscribes to both `reqAccountSummary` and (once an account is known) `reqAccountUpdates` and BLOCKS — bounded, with the same retry-then-condemn shape as `_probe_liveness` (one retry after `IB_PROBE_RETRY_GAP_S`) — until the first real data lands, before declaring success. Gated to run once per underlying connection (`IBClient._account_data_ready`): a still-open cached handle skips it on every subsequent `connect()` call; a fresh connect or a reconnect after an idle drop always re-runs it. `<= 0` opts out, restoring the pre-fix racy behaviour (the same escape-hatch shape as `IB_PROBE_TIMEOUT_S=0`). Landed alongside the fix, not yet deployed to the live VM pending operator ack (Tier-2). |
