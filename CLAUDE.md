...
| `IB_GATEWAY_CPUS` / `IB_GATEWAY_MEMORY` | Hard resource caps the IB-Gateway **Docker container** is created with (`scripts/install_ib_gateway_docker.sh`, defaults `0.75` CPU / `1500m` mem; `--memory-swap` pinned to `--memory` so the container can't swap). The Gateway is a heavy Java GUI app under Xvfb; an unauthenticated re-login loop during IBKR's reset window can spin it hot, and on the shared 2-core live VM that previously starved the trader's single-threaded main loop (loadavg ~10 → heartbeat froze → ~25-min wedge, the 2026-06-10 cascade). The cap means the gateway can NEVER dominate the box — the trader keeps ticking even while the gateway churns. `docker restart` preserves these flags, so the IB-gateway-watchdog's recovery path stays capped too. Paired with high `CPUWeight`/`Nice` on `ict-trader-live.service` + both watchdog units (`deploy/*.service`) so the trader + the dead-man switches always get scheduled under contention. Apply to an already-running container without a restart via `docker update --cpus=<n> --memory=<m> ib-gateway`; new containers get the cap from the script. |
