...
| `GET /api/bot/positions?include_demo=BOOL` | open positions — each carries an `isDemo` flag. **Demo rows excluded by default**; `include_demo=true` includes them. | `trade_journal.db` WHERE status='open' |
...
| `GET /api/bot/trades/closed?limit=N&since=ISO_TS&account_id=X&include_demo=BOOL` | `ClosedTrade[]` (#557) — each row carries `isDemo`. **Demo rows are excluded by default**; `include_demo=true` adds them; `account_id=X` returns only that account (and always wins over `include_demo`). | `trade_journal.db::trades` filtered to closed + non-backtest, joined to `order_packages` for the closed-at proxy |
| `GET /api/bot/performance?window=24h\|7d\|30d\|all` | `{window, since, totalTrades, wins, losses, winRate, totalPnl, expectancy, perStrategy:[{name, trades, wins, winRate, totalPnl, expectancy}], equity:[{t, cum}]}` — windowed aggregate trade analytics computed **in SQL over the full history within the window (uncapped)**. Replaces consumer-side rollups over the 200-row `/trades/closed` cap (which pinned the Android Performance "Trades" count at 200). Excludes backtest + demo rows like `/stats`; close-time basis `COALESCE(op.updated_at, t.timestamp)`. Zeroed envelope (HTTP 200, `error` set) on unknown window / DB error. Tier 1 | `trade_journal.db::trades` LEFT JOIN `order_packages` |
| `GET /api/bot/order-packages?limit=N&since=ISO_TS&strategy=X` | `{rows:[{orderPackageId, createdAt, updatedAt, strategy, symbol, direction, entry, sl, tp, confidence, status, closeReason, linkedTradeId, pnl, tradeStatus, claudeScore}], count, claude_log_present}` — **decision-level** view (one row per order package = the bot's actual decision). `claudeScore` = the Claude strategy-decision grade `{grade, score, entryQuality, exitQuality, riskManagement, executed, rationale, reviewedAt}` or `null` until a `/health-review` scores it. Per-model **shadow scores are NOT joined here** — the dashboard composes those from `/api/bot/trades/scores` by `linkedTradeId`. | `trade_journal.db::order_packages` LEFT JOIN `trades` (PnL + backtest/demo filter) + `comms/claude_strategy_scores.jsonl` (by `order_package_id`); Tier 1 |
...
| `GET /api/pnl/history?days=N&account_id=X` | `PnlHistoryPoint[]` (S-063) — `account_id=X` scopes to one account (default: all real-money accounts, demo excluded). | `trade_journal.db` (closed trades, realised PnL per UTC day) |
| `GET /api/bot/news/recent?limit=N` | `{present, log_path, count, records[]}` — newest-first tail of the M9 news layer's shadow-soak log (per-actionable-signal decision/adjustment/veto/query/symbol + applied influence downsizes). `present:false` until the layer is enabled (`NEWS_ENABLED=true` + `NEWS_API_KEY`). Tier 1 | `runtime_logs/news_decisions.jsonl` (written by `src.news.news_audit` + `src.runtime.news_sizing`) |
...
| `RECONCILER_CLOSE_CONFIRM_SECONDS` | Tuning knob for the netting-guard reconciler half (default `60`, clamped `>= 0`; only consulted when the guard is active for the account). Min seconds a filled trade must read net-flat across ≥2 observations before the reconciler closes it. `0` keeps the extra-grace-tick requirement (a second confirming observation) with no added time wait. Read at call time (next-tick effect). Sibling of `RECONCILER_GRACE_SECONDS`. |
...
| `NEWS_ENABLED` | Master gate for the **M9 news layer** (`src/news/`). **⚠️ Code default is `true` (enabled-when-unset)** — `_is_enabled()` in `news_client.py:39` / `news_client_rss.py:40` treats only `{false,0,no}` as off. The layer is inert **by default only** because the default `NEWS_SOURCE=newsapi` short-circuits without `NEWS_API_KEY`. **Footgun:** setting `NEWS_SOURCE=rss` (keyless) WITHOUT explicitly setting `NEWS_ENABLED=false` activates the layer — and a `news_veto` blocks a live trade (`pipeline.py:477`). To keep it off, set `NEWS_ENABLED=false` explicitly. (A proposal to flip the code default to `false` so this row reads literally is open — Tier-3, changes veto reachability.) Per-symbol queries/keywords live in `config/news_symbols.yaml`; full reference in `docs/news_layer.md`. |
| `NEWS_SOURCE` | Feed backend: `rss` (free, **keyless**, **real-time** — feeds in `config/news_feeds.yaml`) or `newsapi` (default; needs `NEWS_API_KEY`, free tier is ~24h delayed → effectively inert). **`rss` is recommended.** With `rss`, `NEWS_ENABLED=true` alone activates the layer (no key). |
