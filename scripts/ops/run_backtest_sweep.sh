...
# 4. ict_scalp_5m re-validation — fed the qashdev parquet, subsampled
# to the last 90 days. ict_scalp_5m's CLI is a pure-Python loop; round
# 2 (#1370) showed the full 332k-bar input overruns the diag SSH
# timeout, and round 3 (#1371) showed 105k bars (12 months) does too.
# 90 days = 25,920 bars and matches the window PR #1156's original
# pre-live gate ran on, so we get an apples-to-apples re-validation
# without the timeout risk. The full 3-year parquet stays cached for
# the vwap/turtle ablation harness which is fast.
echo
...
# Parquet → CSV (last 90 days) with the columns ict_scalp expects.
echo "  converting parquet -> CSV (last 90 days) for ict_scalp's CSV-only loader..."
"$VENV_DIR/bin/python" - <<PYEOF
import pandas as pd
df = pd.read_parquet("$DATA_ROOT/btc_5m.parquet")
df = df[["timestamp", "open", "high", "low", "close", "volume"]]
# 90 days at 5m = 90 * 24 * 12 = 25,920 bars.
df = df.iloc[-25_920:].copy()
