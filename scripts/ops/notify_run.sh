...
case "${action}" in
    ...
    setup-tailscale-funnel|teardown-tailscale-funnel)
        ...
        ;;
    backfill-pnl-nulls|backfill-orphan-pnl|backfill-monitor-closed-pnl|revert-backfill-monitor-closed-pnl|rebuild-pnl-from-bybit|backfill-shadow-predictions|pull-mes-ibkr-history|pull-mes-ibkr-history-daily|rotate-account-keys|init-diag-token)
        tier=2
        case "${exit_code}" in
            0) result="ok"; priority="normal" ;;
            *) result="FAILED (exit ${exit_code})"; priority="urgent" ;;
