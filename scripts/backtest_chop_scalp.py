...
def _summarize(trades: List[Trade], df: pd.DataFrame, *, timeframe: str,
    ...
    if n == 0:
        base.update({"win_rate_pct": 0.0, "total_r": 0.0, "net_total_r": 0.0,
                     "net_total_r_fee_only": 0.0, "net_expectancy_r": 0.0, "total_fee_r": 0.0,
...
def main(argv: List[str]) -> int:
    ...
    p = argparse.ArgumentParser(
        description="Multi-timeframe chop-scalp backtest (range-boundary bounces, net-of-cost: fee+slippage+funding).")
    ...
    if not _src.ok:
        print(_backtest_data_source.refusal_message(
            _src, harness="backtest_chop_scalp.py",
            legacy_default=os.environ.get("BACKTEST_DATA_PATH", "data/backtest_candles.csv")),
            file=sys.stderr)
        return 2
    args.data = _src.path
