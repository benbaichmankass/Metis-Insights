#!/usr/bin/env python3
# wiring: manual-only — a one-shot adjudication answering "can the e35 dose-response
#   be graded once the venue supplies the mechanism?". It fetches from the diag surface
#   and is run BY A SESSION with the windows that population needs; scheduling it would
#   re-ask a question whose answer is already recorded, against a window nobody chose.
...
def report(treated: dict, verdicts: list[dict], summary: dict,
    ...
    unnam = [v for v in verdicts if v["state"].startswith("unnameable")]
    if unnam:
        print("THE CLOSES THAT STAYED UNNAMEABLE — named, so the next session can check "
              "them rather than take the aggregate on trust")
        print("=" * 78)
        print(f"  {'trade':>6s} {'arm':8s} {'leg':24s} {'account':16s} {'symbol':9s} "
              f"{'journal label':20s} why")
        for v in sorted(unnam, key=lambda x: (x["arm"], x["trade"])):
            print(f"  {v['trade']:6} {v['arm']:8s} {str(v['leg'])[:24]:24s} "
                  f"{str(v['account'])[:16]:16s} {str(v['symbol'])[:9]:9s} "
                  f"{str(v['journal_exit_reason'])[:20]:20s} {v['state']}")
        print()
