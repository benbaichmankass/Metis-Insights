...
⚠️ **THIS GUARD SHIPS BEFORE THE FIX IT ANTICIPATES, DELIBERATELY** (operator,
2026-09-22). The union lives in PR #12736 and is HELD behind a shared-resolver
change that must land first, so **right now ``symbols:`` IS still the gate** and
a rostered-but-undeclared symbol does NOT trade. The operator's standing rule
for that window is exactly what this guard enforces: *any leg promoted while the
union is held MUST have its symbol declared in the same PR.* Landing the guard
with the union would have left that window unguarded, which is the one thing the
ordering was chosen to avoid.

So the finding's CONSEQUENCE moves when the union lands, and the message must
move with it — see :func:`union_state`. Once ``_resolve_tick_symbols`` unions
the roster in, the omission no longer stops a leg trading. It is still a BUG
then, because the declared list independently drives real data pulls that are
NOT the tick: the Bybit per-symbol position cross-check that catches a settleCoin page
...
----------------------
The failure message names the PULL LIST as the thing to fix, never the roster
and never the leg. There is no reading of a finding here under which the remedy
is to stop trading something: the operator was explicit, and a guard whose
message invites the wrong fix is worse than no guard.

⚠️ **WHAT IT MUST NOT SAY WHILE THE UNION IS HELD.** An earlier draft of this
message read *"the leg trades regardless (the tick fetch set unions
roster-implied symbols since 2026-09-22)"* — reassurance that is FALSE until
#12736 merges, on precisely the surface someone reads while promoting a leg.
That is the *field beats comment* failure committed inside the instrument meant
to prevent it. :func:`union_state` reads ``src/main.py`` and the message follows
the CODE, so it corrects itself the moment the union lands and no one has to
remember to edit it.
...
# The three states of the fix this guard anticipates. NEVER COLLAPSED: what a
# missing pull-list entry COSTS is the opposite in the first two, and `unknown`
# is "we could not look", which is not either of them.
#
#   union_absent   `_resolve_tick_symbols` builds the fetch set from the
#                  declared `symbols:` alone. The pull list IS the gate, so a
#                  rostered-but-undeclared leg gets no candles and does NOT
#                  trade. This is the state on `main` until PR #12736 merges.
#   union_landed   it unions `_symbols_for_account(...)` in. The leg trades; the
#                  omission is now "only" a blind spot for the non-tick data
#                  readers (the Bybit position cross-check, the fill sweep).
#   unknown        `src/main.py` could not be read or parsed, or the function is
#                  not where it should be. We say BOTH consequences and say we
#                  could not tell, rather than picking the cheerful one.
UNION_STATES = ("union_absent", "union_landed", "unknown")


def union_state(source: str | None = None) -> str:
    """Has the roster union landed in ``_resolve_tick_symbols``?

    Read off the AST of ``src/main.py``, not off a version string, a date or a
    comment: the whole reason this function exists is that the previous version
    of this guard's failure message asserted the union was live while it sat
    unmerged in a held PR. Field beats comment, including inside a guard.

    Grepping is refused for the same reason `check_guard_selftest_coverage.py`
    refuses it: a call wrapped across two lines, or the name appearing in this
    module's own docstring, would both fool a line-oriented match. The AST walk
    asks the only question that matters — does the BODY of that one function
    contain a call to ``_symbols_for_account``.
    """
    import ast  # noqa: PLC0415

    try:
        text = source if source is not None else MAIN.read_text(encoding="utf-8")
        tree = ast.parse(text)
    except Exception:  # noqa: BLE001
        return "unknown"
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "_resolve_tick_symbols":
            for child in ast.walk(node):
                if (isinstance(child, ast.Call)
                        and isinstance(child.func, ast.Name)
                        and child.func.id == "_symbols_for_account"):
                    return "union_landed"
            return "union_absent"
    return "unknown"


def _consequence(state: str) -> str:
    """What a stale pull list COSTS right now, in the reader's own terms."""
    if state == "union_absent":
        return (
            "⚠️ RIGHT NOW THE LEG DOES NOT TRADE AT ALL. `_resolve_tick_symbols` "
            "still builds the tick's fetch set from the declared `symbols:` "
            "alone (the E42 union is held in PR #12736 behind the shared-resolver "
            "change), so a rostered symbol that is not declared gets NO candles, "
            "NO signal and NO order — while reading as wired everywhere. "
            "DECLARE THE SYMBOL IN THIS PR. That is the operator's standing rule "
            "for exactly this window, and it is not satisfied by a follow-up."
        )
    if state == "union_landed":
        return (
            "The leg TRADES regardless — the tick fetch set unions roster-implied "
            "symbols. What is missing is the DATA: the declared list still drives "
            "the Bybit per-symbol position cross-check and the fill/cost sweep, so "
            "an undeclared symbol can hold a position nothing cross-checks."
        )
    return (
        "⚠️ COULD NOT TELL what this costs: `src/main.py::_resolve_tick_symbols` "
        "was unreadable or unparseable, so whether the fetch set unions the "
        "roster is UNKNOWN. Assume the worse of the two — that the leg does not "
        "trade at all — and declare the symbol in this PR."
    )
...
def _self_test() -> int:
    ...
    # 10-13. THE UNION-STATE AXIS. These are the controls the FIRST version of
    #     this guard did not have and needed most: its message asserted the
    #     union was live while it sat unmerged in a held PR, i.e. the guard
    #     told a promoting session the leg would trade when it would not.
    #     Each state is planted as SOURCE and read back through the AST.
    src_absent = (
        "def _resolve_tick_symbols(settings):\n"
        "    syms = list(acct.symbols or [])\n"
        "    return syms\n"
    )
    src_landed = (
        "def _resolve_tick_symbols(settings):\n"
        "    syms = list(acct.symbols or [])\n"
        "    syms = syms + _symbols_for_account(acct, cfg)\n"
        "    return syms\n"
    )
    check("10 (declared-only source -> union_absent)",
          union_state(src_absent) == "union_absent")
    check("11 (source that unions the roster in -> union_landed)",
          union_state(src_landed) == "union_landed")
    check("12 (unparseable source -> `unknown`, NOT the cheerful state)",
          union_state("def _resolve_tick_symbols(:::") == "unknown")
    check("13 (the function missing entirely -> `unknown`, not union_absent)",
          union_state("def something_else():\n    pass\n") == "unknown")

    # 14. A call to _symbols_for_account ELSEWHERE in the file must not be read
    #     as the union landing. `_apply_per_account_leverage` has called it
    #     since 2026-05, so a file-wide grep would have reported `union_landed`
    #     against every version of main.py ever written — the guard would have
    #     been born giving the wrong answer.
    src_decoy = (
        "def _apply_per_account_leverage():\n"
        "    syms = _symbols_for_account(acct, cfg)\n"
        "\n"
        "def _resolve_tick_symbols(settings):\n"
        "    return list(acct.symbols or [])\n"
    )
    check("14 (the SAME call in another function -> still union_absent; the "
          "check is scoped to the one function's body)",
          union_state(src_decoy) == "union_absent")

    # 15. The consequence text must INVERT between the two states, and the
    #     pre-union one must tell a promoting session the leg will not trade.
    absent_msg, landed_msg = _consequence("union_absent"), _consequence("union_landed")
    check("15 (union_absent says the leg does NOT trade and to declare the "
          "symbol in THIS PR)",
          "DOES NOT TRADE" in absent_msg and "THIS PR" in absent_msg)
    check("15b (union_landed says the leg DOES trade — the opposite claim)",
          "TRADES regardless" in landed_msg
          and "DOES NOT TRADE" not in landed_msg)
    check("15c (`unknown` assumes the WORSE of the two, never the cheerful one)",
          "COULD NOT TELL" in _consequence("unknown")
          and "does not trade" in _consequence("unknown"))
    check("15d (no state's message ever proposes demoting the leg or editing "
          "a roster)",
          not any(w in m.lower() for m in (absent_msg, landed_msg,
                                           _consequence("unknown"))
                  for w in ("demote", "remove the leg", "shadow")))

    print("self-test OK — a planted stale pull list goes red, a correct one "
          "and an orphan-carrying pull list stay green, an unmeasurable axis "
          "stays silent instead of firing, and the consequence text follows "
          "src/main.py rather than asserting a fix that has not merged."
...
def main() -> int:
    ...
    print(f"roster-symbol-reachability: {len(accounts)} accounts, "
          ...
          f"fetch-set derivation READ FROM src/main.py: {state}.")
    ...
    print("\n::error::THE PULL LIST IS OUT OF DATE. "
          ...
          "resolve this by changing a roster or by demoting a leg.\n"
          f"  {_consequence(state)}")
