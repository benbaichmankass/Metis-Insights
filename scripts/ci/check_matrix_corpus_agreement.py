...
CORPUS_EXEMPT_LEVERS: dict[str, str] = {
    ...
    "stop_geometry": (
        "NOT A LEVER — it is the ENTRY BRACKET's STOP WIDTH "
        "(`atr_sl_buffer_mult`, the `ict_scalp` family's "
        "`sl = sweep_extreme +/- mult * ATR`), added as a column 2026-09-12 "
        "(MI-278 U3, run 34689877269). The exemption is the SAME IN-PRINCIPLE "
        "one `bracket_geometry` holds directly above, for the same structural "
        "reason: this corpus holds (leg, LEVER) rows, one per POST-ENTRY "
        "override applied to a bracket held FIXED, and varying the bracket "
        "itself has no lever to key a row on. "
        "⚠️ THAT IS NOT A JUDGEMENT CALL — the repo already ruled on it. "
        "`scripts/research/e35_corpus_extract.py` says in its own header that "
        "`m20-sweep-corpus.jsonl` 'holds only M20 **lever** cells' and writes "
        "bracket-geometry evidence to 'A SEPARATE FILE, DELIBERATELY'. This "
        "column is the same category as the file that decision created. "
        "WHERE THE EVIDENCE ACTUALLY IS, so the exemption is checkable rather "
        "than taken on trust: the per-leg verdicts are posted as run-34689877269 "
        "comments on PR #11933 and tabulated in "
        "docs/research/m20-u3-scalp-target-sweepable-2026-09-12.md section 4q "
        "(42 cells, 7 legs, 4 candidates, one walk-forward survivor). "
        "⚠️ AND THAT LOCATION IS WEAKER THAN `bracket_geometry`'s, which points "
        "at committed report.json files. A PR comment is not a committed "
        "artifact, and run 34684742080 has already demonstrated that this "
        "workflow's comment path can FAIL — six verdicts had to be recovered "
        "from job logs. Tracked as "
        "BL-20260912-EVERY-M27-ICT-SCALP-VERDICT-IS-CARRIED-BY-A-PR-COMMENT-THAT-HAS-ALREADY-FAILED-TO-POST-ONCE; "
        "the workflow now uploads verdicts.json as an artifact, which is the "
        "cheap half and not the fix. Do not read a passing run as 'this column "
        "is verified'. "
        "⚠️ NOTE WHAT IS DELIBERATELY *NOT* EXEMPTED HERE: `breakeven_ratchet`. "
        "It IS a post-entry override applied to a fixed bracket — exactly the "
        "shape this corpus is built for — so it does not qualify for this "
        "exemption and its column is NOT being added until its rows reach the "
        "corpus by the sanctioned route (scripts/research/m20_corpus_extract.py "
        "over the sweep's out/ directory). Exempting it to make this guard pass "
        "would be the cheaper-to-lie-to-than-to-satisfy failure the "
        "new-table-wiring-guard already cost this repo."
    ),
    "exit_head_ml": (
        ...
        "and 10 DISAGREED IN SIGN — eth/xrp/ada_pullback_2h, "
        ...
        "AS OF 2026-08-23 THE FIGURE WAS **9 DISAGREE IN SIGN**, and the ONE "
        "cell that left is a real re-grade, not attrition: ict_scalp_eth_15m "
        "moved to `blocked:no_lever_consumer_in_unit` (the ict_scalp unit has "
        "no lever consumer, so the cell claims no outcome and an "
        "honest_negative round verdict no longer contradicts it). "
        "AS OF 2026-09-06 THE FIGURE IS **2 DISAGREE IN SIGN** (MI-145), AND "
        "THE DROP FROM 9 IS THE SAME RE-GRADE APPLIED SEVEN MORE TIMES — NOT "
        "ATTRITION AND NOT A PARSING ARTIFACT. All nine were the identical "
        "shape: round verdict `candidate` against a matrix `honest_negative`. "
        "Seven moved to `blocked:no_lever_consumer_in_unit` because their "
        "MONITOR UNIT IMPLEMENTS NO EXIT HEAD — ada/eth/xrp_pullback_2h, "
        "eth_pullback_prop_2h, iaum_pullback_1d (all `htf_pullback_trend_2h`) "
        "and ict_scalp_{avax_5m,xrp_15m} (`ict_scalp`). Resolved by CALLING "
        "src.runtime.pipeline.monitor_unit_for and "
        "scripts.ops.exit_mechanism_coverage.module_implements, both imported "
        "rather than re-derived, not by reading an audit's list. "
        "`candidate offline` and `unshippable for want of a consumer` are "
        "COMPATIBLE — the cell claims no outcome, so it contradicts neither "
        "verdict — whereas `candidate offline` against `measured and lost` was "
        "a real contradiction. That is precisely the 2026-08-23 argument one "
        "line up, and the count is now MORE informative, not weaker. "
        "⚠️ THE TWO THAT REMAIN ARE THE GENUINE ONES AND MUST NOT BE SWEPT "
        "UP WITH THEM: trend_donchian_{ada,xrp}_4h resolve to the "
        "`trend_donchian` unit, which DOES implement the exit head, so they "
        "were deliberately NOT re-statused and their `candidate` vs "
        "`honest_negative` contradiction is live and unexplained. "
        "The seven are the state MI-146 named — passed their gate, blocked on "
        "wiring alone (docs/research/exit-lever-wiring-audit-2026-09-06.md); "
        "the derivation is docs/research/exit-sweep-positive-control-2026-09-06.md "
        "section 4. "
        "⚠️ A FOURTH APPARENT CHANGE WAS AN ARTIFACT AND IS RECORDED SO IT IS "
        "NOT RE-INTRODUCED: the honesty test recomputing this count matched the "
        "status by EXACT membership against the literal \"blocked\", so the "
        "moment three ict_scalp cells gained a REASON suffix "
        "(`blocked:no_lever_consumer_in_unit`) they stopped matching and read "
        "as fresh disagreements — briefly making this count look like 13. "
        "`blocked:<reason>` is the repo convention, not an exception: 51 of the "
        "53 blocked cells in the matrix carry one, across nine distinct "
        "reasons. This guard was always right (`find_stale_blocks` normalises "
        "via `_base_status`); only the test was not, and it now uses the same "
        "helper. RECORDING WHY a cell is blocked must never flip whether it "
        "AGREES — a qualified state read as an unknown state is the collapsed-"
        "state class, and bumping the count to 13 would have enshrined a "
        "parsing artifact as evidence, which is the exact stale-number failure "
        "this exemption's own history is about. "
        "AS OF 2026-09-06 (MI-150, PR #11140) THE FIGURE IS **4 DISAGREE IN "
        "SIGN**, AND THE RISE FROM 2 IS REAL — IT WAS CAUSED BY LANDING THE "
        "CONSUMER, NOT BY A NEW ROUND AND NOT BY A PARSING ARTIFACT. MI-145's "
        "argument one paragraph up is CONDITIONAL and its condition has now "
        "expired for one unit: `candidate offline` does not contradict "
        "`blocked:no_lever_consumer_in_unit` *because the cell claims no "
        "outcome while no consumer exists*. MI-150 makes "
        "src/units/strategies/ict_scalp.py implement the exit head, so that "
        "status is factually false for the ict_scalp legs and the cells were "
        "corrected. Of the five ict_scalp legs present in the rounds corpus: "
        "THREE stay compatible on a NEW blocked reason — "
        "ict_scalp_{sol_5m,sol_15m,xrp_5m} are `blocked:no_scalp_artifact_"
        "published`, since every published exit-head artifact is 1h against "
        "their 5m/15m, so they still claim no outcome; ONE agrees — "
        "ict_scalp_eth_15m is `honest_negative` against an `honest_negative` "
        "round verdict; and TWO become live contradictions — "
        "ict_scalp_{avax_5m,xrp_15m} are `honest_negative` against a "
        "`candidate` round verdict, which is EXACTLY the "
        "trend_donchian_{ada,xrp}_4h shape: a unit that DOES implement the "
        "head, with the sign disagreement live and unexplained. "
        "⚠️ SO THIS IS THE ONE DIRECTION THE COUNT MAY RISE WITHOUT NEW "
        "EVIDENCE, and it must not be read as evidence appearing: two of "
        "MI-145's seven were parked behind an excuse that landing the "
        "consumer removes, so part of the 9 -> 2 drop was always temporary. "
        "Recomputed by CALLING `_base_status` over the committed rounds file "
        "and the matrix, not read off an audit list. ⚠️ NO CELL WAS PROMOTED "
        "TOWARD `shipped`/`passed_unshipped` — the 2026-08-23 SHIP BLOCKED "
        "operator decision stands, and MI-150 lands annotate-only and "
        "disarmed. "
