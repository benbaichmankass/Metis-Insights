    ...
    for obj_id, data in objects.items():
        ...
        for raw in (data.get("decision_requests") or []):
            ...
            if not rid:
                continue
            declared.add(rid)
            if normalise_answer(raw.get("answer")) is not None:
                answered[rid] = obj_id
    ...
    for obj_id, data in sorted(objects.items()):
        ...
        for edge in (data.get("blocked_on") or []):
            ...
            if ref in answered:
                where = answered[ref]
                spent.append(
                    f"{obj_id} (lifecycle: {lifecycle}) is blocked_on {ref}, which carries a "
                    f"READABLE answer on {where}. The decision was taken; the edge was not "
                    f"released."
                )
            elif ref not in declared:
                unresolvable.append(
                    f"{obj_id} (lifecycle: {lifecycle}) is blocked_on a ref that is not any "
                    f"declared request id: {ref!r}. A typed edge whose ref is prose is untyped "
                    f"-- nothing matching on ids can ever discharge it."
                )
...
def _self_test() -> int:
    ...
    if failures:
        ...
        return 1
    print("decision-answers: self-test OK (12 cases)")
    return 0
