...
def _summarize(trades: List[Trade], df: pd.DataFrame, *, timeframe: str,
    ...
    base.update({
        ...
        # fee-only (NOT total cost): _fee_r now returns fee+slippage+funding, so the
        # fee-labeled key must use _fee_only_r to keep the label honest. The full-cost
        # totals live in net_total_r / mean_cost_r.
        "total_fee_r": round(sum(_fee_only_r(t) for t in trades), 4),
...
def main(argv: List[str]) -> int:
    ...
    p = argparse.ArgumentParser(description="Failed-breakout fade backtest (net-of-cost: fee+slippage+funding).")
