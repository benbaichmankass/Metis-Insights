#!/usr/bin/env python3
"""Build a pretrained-TSFM quantile-forecast side-stream for a target symbol (M19 T0.4).

Tier-1 trainer-side tooling: writes the `data.jsonl` (+ a small `metadata.json`)
that `market_features` joins as-of to populate the `fc_*` columns. Like
`build_embeddings.py` it fetches nothing external — it reads the target's OHLCV
from an EXISTING `market_raw` dataset already on the build host, runs a frozen
`amazon/chronos-bolt-tiny` (9M, CPU) over each (strided) bar's trailing close
window as a QUANTILE FORECASTER (predicting the next `--horizon` bar's price
quantiles), and derives the scale-free `fc_*` features (log-return space vs the
last close).

**Optional trainer-side deps** (NOT on the live money-box): this script needs
`chronos-forecasting` + `torch` (`pip install -r requirements-backtest.txt`). The
`market_features` build WITHOUT `forecast_path` never imports them — T0.4 heads
stay at `candidate` stage (offline only), so the dep is trainer-side only.

Run on the trainer VM:

    python -m scripts.ml.build_forecasts \
      --target datasets-out/market_raw/BTCUSDT/15m/v002 \
      --out datasets-out/forecasts/BTCUSDT/15m/v001 \
      --context-len 64 --stride 4 --horizon 1

Then build the target's market_features WITH `forecast_path=<out>`:

    python -m ml build-dataset market_features --output-dir datasets-out \
      --version v001 --source <btc_market_raw_dir> --symbol-scope BTCUSDT \
      --timeframe 15m --overwrite market_raw_path=<btc_market_raw_dir> \
      forecast_path=datasets-out/forecasts/BTCUSDT/15m/v001 \
      vol_window_n=20 forward_window_m=5
"""
from __future__ import annotations

import argparse
import json
import sys
from datetime import datetime, timezone
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from ml.datasets.forecast_features import (  # noqa: E402
    DEFAULT_CONTEXT_LEN,
    DEFAULT_HORIZON,
    DEFAULT_MIN_CONTEXT,
    DEFAULT_STRIDE,
    FORECAST_FEATURE_COLUMNS,
    FORECAST_MODEL_ID,
    FORECAST_QUANTILES,
    chronos_forecast_fn,
    compute_forecast_feature_rows,
    forecast_available,
)


def _load_market_raw(path: Path) -> list[dict]:
    data_path = path / "data.jsonl"
    if not data_path.is_file():
        raise FileNotFoundError(f"market_raw data.jsonl not found at {data_path}")
    rows: list[dict] = []
    with data_path.open("r", encoding="utf-8") as fh:
        for line in fh:
            line = line.rstrip("\n")
            if line:
                rows.append(json.loads(line))
    return rows


def _symbol_of(path: Path, rows: list[dict]) -> str:
    for r in rows:
        if r.get("symbol"):
            return str(r["symbol"])
    parts = path.resolve().parts
    return parts[-3] if len(parts) >= 3 else path.name


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--target", required=True, type=Path,
                    help="Target symbol's market_raw dataset dir.")
    ap.add_argument("--out", required=True, type=Path, help="Output dataset dir.")
    ap.add_argument("--model-id", default=FORECAST_MODEL_ID)
    ap.add_argument("--context-len", type=int, default=DEFAULT_CONTEXT_LEN)
    ap.add_argument("--stride", type=int, default=DEFAULT_STRIDE)
    ap.add_argument("--min-context", type=int, default=DEFAULT_MIN_CONTEXT)
    ap.add_argument("--horizon", type=int, default=DEFAULT_HORIZON,
                    help="how many bars ahead to forecast (default 1 = next bar).")
    args = ap.parse_args(argv)

    if not forecast_available():
        ap.error(
            "TSFM forecast deps not installed. On the trainer VM run "
            "`pip install -r requirements-backtest.txt` (chronos-forecasting + torch)."
        )

    target_rows = _load_market_raw(args.target)
    target_symbol = _symbol_of(args.target, target_rows)

    rows = compute_forecast_feature_rows(
        target_rows,
        context_len=args.context_len,
        stride=args.stride,
        min_context=args.min_context,
        forecast_fn=chronos_forecast_fn(args.model_id),
        horizon=args.horizon,
        quantile_levels=FORECAST_QUANTILES,
    )

    args.out.mkdir(parents=True, exist_ok=True)
    data_path = args.out / "data.jsonl"
    with data_path.open("w", encoding="utf-8") as fh:
        for r in rows:
            fh.write(json.dumps(r) + "\n")

    meta = {
        "family": "tsfm_forecasts",
        "target_symbol": target_symbol,
        "model_id": args.model_id,
        "feature_columns": list(FORECAST_FEATURE_COLUMNS),
        "context_len": args.context_len,
        "stride": args.stride,
        "min_context": args.min_context,
        "horizon": args.horizon,
        "quantile_levels": list(FORECAST_QUANTILES),
        "row_count": len(rows),
        "source": "tsfm_forecasts_from_market_raw",
        "generated_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
    }
    (args.out / "metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    print(json.dumps(
        {k: meta[k] for k in ("family", "target_symbol", "model_id", "row_count")},
        sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
