...
| 2026-06-03 | S-MLOPT-S7 (M14 Session 1.3) | Backtest-augmented per-trade labels: `trade_outcomes` + `setup_labels` gain a `source` column (`live`/`backtest`) + `include_backtest` flag (default off → unchanged). New `ml/datasets/backtest_recorder.py` maps `sim.ledger.SimTrade`→trades rows + writes them `is_backtest=1` (opt-in; excluded from every live/stats/default path). `live_holdout` split generalized (`live_flag_true_value`) → recipe "train live+backtest, eval REAL only". `execution_quality` intentionally NOT augmented (learns real slippage). | None — additive Tier-1 (families read `is_backtest`; recorder is opt-in tooling, writes only `is_backtest=1` rows, never runs against the production DB autonomously). Wiring the recorder into a harness's money-DB write is a follow-up. |
| 2026-06-03 | S-MLOPT-S8 (M14 Session 1.4) | Cross-symbol transfer: `setup_candidates` gains a `market_raw_paths` multi-symbol build (per-symbol CUSUM + vol-bucketing, concatenated, `symbol` column carried) → one joint BTC+MES dataset. New `ml/configs/setup-candidates-metalabel-xsym-v1.yaml` (Tier-3 proposal) adds `symbol` as a categorical transfer feature. | None — additive Tier-1 dataset capability (read-only over built `market_raw`). The manifest + any promotion past `shadow` are Tier-3 (operator-gated). |
| 2026-06-06 | S-MLOPT-S8 EVAL (MB-20260606-002) | Trained + evaluated the joint xsym meta-label (trainer-vm-diag #2891–#2894). **MES has ZERO closed trades** (all 354 real are BTCUSDT) ⇒ no real MES holdout, BTC→MES transfer unmeasurable; only the reverse (does MES synth + `symbol` help the real-BTC floor) is testable. Clean ablation: **JOINT acc 0.7571 / precision 0.5417** vs **BTC-only acc 0.6808 / precision 0.2093**, majority baseline 0.7514 — joint is the first meta-label to clear the baseline (precision 2.2× base rate). **Qualified positive, NOT a decisive floor-break** (win on BTC not MES; purged-WF doesn't corroborate; n=354 noisy). Daily build wired opt-in/default-off (`ICT_BUILD_XSYM`). | None — Tier-1 trainer tooling + docs; build default-off (cycle byte-identical, manifest skips via #2886). `research_only → shadow` is a Tier-3 **proposal**, not flipped. |
| 2026-06-03 | S-MLOPT-S6-FU (MB-20260603-002) | Signal-log event source: `setup_candidates` gains a `signal_log_db` kwarg sampling candidates from the strategies' real decision points (`trade_journal.db::signals` buy/sell rows) with the same triple-barrier label as CUSUM; new `event_source ∈ {cusum, signal_log, live}` schema column; `include_cusum` preferred over the legacy `include_synthetic` alias. New manifest `setup-candidates-metalabel-siglog-v1.yaml` (research_only). **Eval (#2716) honest negative** — signal-log acc 0.526 < CUSUM 0.670 < 0.756 baseline; win rate 0.469 ≈ CUSUM 0.457 ⇒ the synthetic *label* (not the sampler) is the train↔eval gap. | None for the Tier-1 family/manifest (additive, read-only). Manifest + promotion past `shadow` Tier-3. |
| 2026-06-03 | S-MLOPT-S9 (M14 Phase 2.1, MB-20260603-004) | Range-based vol estimators: new `ml/datasets/volatility_estimators.py` (Parkinson / Garman-Klass / Rogers-Satchell / Yang-Zhang variance estimators over an OHLC window) + four new past-window `market_features` columns (`parkinson_vol` / `garman_klass_vol` / `rogers_satchell_vol` / `yang_zhang_vol`), `builder_version v2 → v3` (metadata-only). New manifest `btc-regime-1h-lgbm-yz-v1.yaml` (research_only) — a clean A/B vs `btc-regime-1h-lgbm-v2` selecting the range-vol features + freezing the regime spec on `yang_zhang_vol`. Attacks gap G5 (no range-based vol estimators). | None for the Tier-1 family/estimators (additive, read-only, past-only → leakage-safe). The manifest + promotion past `shadow` are Tier-3 (operator-gated); ships `research_only`. |
| 2026-06-03 | S-MLOPT-S6-FU-2 (MB-20260603-003) | Backtest event source: `setup_candidates` gains a `backtest_trades_db` kwarg (+ `include_backtest` convenience reusing `live_trades_db`, + `backtest_strategies` filter) reading the `is_backtest=1` rows the S7 recorder writes from the standalone harnesses; rows tagged `event_source="backtest"` + `is_live_trade=False` (train side) carry the harness's real-execution outcome (`won` from the recorded R-proxy `pnl`), not a synthetic barrier. New bridge `scripts/ml/record_harness_trades.py` (harness trade-JSONL → recorder rows). New manifest `setup-candidates-metalabel-backtest-v1.yaml` (research_only) — the apples-to-apples backtest-train + real-eval. | None for the Tier-1 family/bridge/manifest (additive, read-only; bridge writes a TEMP DB, `is_backtest=1` only). Manifest + promotion past `shadow` Tier-3. |
| 2026-06-04 | S-MLOPT-S9 ext (MB-20260603-004) | Range-vol A/B extended to the remaining regime timeframes: new `btc-regime-{5m,15m}-lgbm-yz-v1.yaml` + `mes-regime-{5m,15m}-lgbm-yz-v1.yaml` (research_only), each a clean A/B clone of its v2 champion with the range-vol feature set + regime spec frozen on `yang_zhang_vol`. | None — additive Tier-3 A/B manifests at `research_only`; promotion past `shadow` operator-gated. |
| 2026-06-04 | S-MLOPT-S11 (M14 Phase 2.3) | Crypto funding-rate + open-interest features: new `ml/datasets/funding_oi_features.py` (rolling z-score, extreme magnitude \|z\|, log-change, change-z — signal is in the EXTREMES, not the level) + `ml/datasets/adapters/bybit_funding_oi.py` (off-VM Bybit V5 funding/OI history fetcher) + `scripts/ml/fetch_funding_oi.py` (side-stream builder). `market_features` gains an optional `funding_oi_path` join → five new past-only, as-of-aligned columns (`funding_rate` / `funding_rate_zscore` / `funding_rate_abs_z` / `open_interest_change` / `open_interest_change_zscore`), `builder_version v3 → v4`; **default-preserving** (no `funding_oi_path` → columns emit 0.0, every existing build + non-crypto symbol unchanged). New manifest `btc-regime-1h-lgbm-funding-v1.yaml` (research_only) — A/B vs `btc-regime-1h-lgbm-v2`. Attacks gap G5. | None for the Tier-1 family/estimators/adapter/fetcher (additive, read-only, past-only → leakage-safe; the fetcher is off-VM-guarded). The manifest + promotion past `shadow` are Tier-3 (operator-gated); ships `research_only`. |
| 2026-06-04 | S-MLOPT-S10 (M14 Phase 2.2) | Order-flow / microstructure features: new `ml/datasets/orderflow_features.py` (Cont OFI, Easley-LdP-O'Hara VPIN + bulk-volume classification, Stoikov micro-price, relative spread) + `scripts/ml/orderflow_capture.py` (a forward live-capture side-car, Bybit REST polling, off-VM-guarded) + `deploy/trainer/ict-orderflow-capture.service` (trainer-only systemd unit, kept OUT of the live-VM installer glob). `market_features` gains an optional `microstructure_path` join → six new past-only, as-of-aligned columns (`ofi` / `ofi_zscore` / `vpin` / `order_imbalance` / `rel_spread_mean` / `microprice_dev`), `builder_version v4 → v5`; **default-preserving** (no `microstructure_path` → 0.0). New manifest `btc-regime-5m-lgbm-flow-v1.yaml` (research_only). **Key constraint:** no L2 history to backfill — the capture runs FORWARD on the trainer VM and the A/B can only run once enough data accrues (`MB-20260604-002`). Attacks gap G5. | None for the Tier-1 family/estimators (additive, read-only, past-only → leakage-safe). The capture side-car runs on the TRAINER VM only (off-VM-guarded; WS9-safe). The manifest + promotion past `shadow` are Tier-3 (operator-gated); ships `research_only`. |
