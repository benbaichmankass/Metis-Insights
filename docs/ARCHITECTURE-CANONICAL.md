...
| 2026-05-27 | (TP-cap + journal-bleed scrub + reconciler-IB-wiring) | **Three Tier-2/3 fixes shipped + deployed in one session, all triggered by the live trader's same-day outages.** (1) **TP cap on the 50R sentinel strategies** (PR #2156, Tier-3 operator-approved): trend_donchian / fade_breakout_4h / squeeze_breakout_4h now compute `tp = min/max(entry*(1±0.099), entry ± tp_r*risk)` on both branches. Yesterday's PR #2141 had clamped the SHORT branch to `entry*0.01` to satisfy the in-process `tp>0` gate, but Bybit then rejected those at the wire with `ErrCode 10001 'TakeProfit < 10pct of base'` (TP 749 vs entry 74940 is ~99% away; Bybit's distance tolerance is ~10%). The symmetric cap stops the rejection cluster on shorts AND defensively caps the long branch (which would have hit the same wall on the next sufficient-risk MES/BTC long). Sentinel intent (Chandelier trail is the real exit, fixed TP must never fill) unchanged. Supersedes BL-20260525-007's "expected unclamped sentinel" framing. (2) **`scrub-env-noncompliant` system-action** (PR #2158, Tier-2 operator-approved): a pre-`set-mobile-push-secrets` setup pasted a multi-line FCM service-account JSON straight into `.env`, and systemd's `EnvironmentFile` parser rejects every continuation line by echoing it into `journalctl` as `Ignoring invalid environment assignment '<line>'` — so each service restart re-published the PEM private key (caught visible on the deploy comments of issues #2157 and #2159). New autonomous action strips every `.env` line that doesn't match systemd's `^[A-Za-z_][A-Za-z0-9_]*=...$` contract, backs up to `${REPO_DIR}/.env.bak.<UTC-ts>` (mode 600), restarts the trader, and logs only counts (never content). 12 lines stripped on the live VM; warning count post-deploy = 0 (verified via diag-request #2161). The remaining exposure surface (the leaked-key issue comments + the on-disk Google Cloud key) is operator-only, logged as BL-20260527-006. (3) **IB-fields in monitor reconciler** (PR #2167, Tier-2 operator-approved): `_load_account_cfgs_for_reconcile()` in `src/runtime/order_monitor.py` was forwarding only Bybit-relevant keys, dropping `ib_host` / `ib_port` / `ib_account` / `ib_client_id`. Four reconciler call sites (`1523`, `1877`, `2491`, `3463`) each tick called `account_open_positions(cfg)` for `ib_paper` → IB branch → `ib_read_client_for(account)` → `ib_client_for(...)` → `account.get('ib_port') = None` → warning + skip. Effect: every monitor tick spammed `ib_client_for(ib_paper): no ib_port set`, AND IB accounts were silently invisible to all four reconciler passes (orphan detection / close-on-disappear never ran on MES). Forwarding the four IB fields mirrors the dict shape `Coordinator.multi_account_execute` already builds at `coordinator.py:1019` so the read/reconcile and write/dispatch layers stay in lockstep. Resolves BL-20260525-002 (the original report attributed the warning to the order-client path; the actual emitter was the reconciler). (4) **Operational incidents handled in-session**: IB Gateway recovered via the `vm-ib-gateway-recover` workflow after a 21h `Up`-but-upstream-dead wedge (Warning 2103/2105/2157, daily-reset that IBC's auto-restart didn't recover — logged as BL-20260527-003); MES leg verified post-recovery (`Warning 2104/2106/2158` all `OK`, real MES candle data flowing into all strategies). Stale-FCM-token 404 cluster on the new VWAP signal post-deploy logged as BL-20260527-004. | `src/units/strategies/{trend_donchian,fade_breakout_4h,squeeze_breakout_4h}.py`, `tests/test_{trend_donchian,fade_breakout_4h,squeeze_breakout_4h}.py`, `scripts/ops/scrub_env_noncompliant.sh` (new), `scripts/ops/notify_run.sh`, `.github/workflows/system-actions.yml`, `docs/claude/system-actions.md`, `tests/ops/test_system_actions_workflow.py`, `src/runtime/order_monitor.py`, `tests/test_load_account_cfgs_for_reconcile.py` (new), `docs/claude/{health-review,performance-review}-backlog.json`, this file | Live VM (deployed in three pull-and-deploy rounds 2026-05-27): trend/fade/squeeze shorts no longer rejected by Bybit ErrCode 10001 (next-tick MES/BTC longs also safe under the same cap); journal no longer bleeds the FCM PEM private key on service restart; monitor reconcilers now read IB positions on `ib_paper` (orphan detection / close-on-disappear active for MES, was silently skipped). Operator-only follow-ups (BL-20260527-006): delete the two leaked issue comments, rotate the FCM service-account key in Google Cloud Console, optionally vacuum the systemd journal. |
