...
| 2026-06-17 | (Breakout prop capability — `breakout` revived as a manual browser-bridge ticket emitter; trend_donchian alt variants) | **`breakout` is NO LONGER a deprecated inert stub** (SUPERSEDES the "breakout stays a deprecated inert stub" note in the 2026-06-15 account_class row above). It is now the **manual browser-bridge** execution path for the Breakout 1-Step prop account: a prop-routed strategy emits a paste-ready DXTrade **ticket** (Telegram/FCM `prop_signal`) instead of an exchange order, and `execute_pkg` (breakout branch §6a, before the client-None dry forcing) returns a `prop-manual-<uuid>` marker so the order package journals **WITHOUT a phantom exchange position** (design: `docs/integrations/breakout-poc-manual-bridge-DESIGN.md`). The capability is the validated `trend_donchian`-on-alts finding (PB-20260616-004): re-validated on REAL Bybit linear-perp 5m + funding + a Breakout **daily-swap** cost model (~0.09%/day — Breakout is a perp-futures-STYLE sim, not Bybit 8h funding) + a 4-fold walk-forward — SOL robust PASS, ETH marginal PASS, BNB excluded (negative realised edge). Wires `trend_donchian_sol` (execution: live) + `trend_donchian_eth` (execution: shadow) reusing the `trend_donchian` unit on SOLUSDT/ETHUSDT, routed to the new `config/accounts.yaml::breakout_1` account. The account is **`account_class: prop`** (a THIRD funding category alongside paper/real_money — tracked separately, excluded from both real-money AND paper KPIs), **`type: prop`** (mission-aware `PropRiskManager` tracking the `account_state: evaluation→funded` lifecycle), and **`mode: live`** (ALWAYS-LIVE ping — the bot always emits the Telegram ticket; the operator decides per-signal whether to place it; there is no programmatic order). POC = Telegram-ping bridge only (no DXTrade API). | `src/prop/{funding,breakout_executor}.py`, `src/units/accounts/{execute,integrator}.py`, `src/runtime/{pipeline,intents,intent_multiplexer,strategy_signal_builders}.py`, `config/{strategies,accounts}.yaml`, `config/prop_rulesets/breakout_routing.yaml`, `config/strategy_{descriptions,changelog}.json`, `scripts/prop/{validate_alt_prop,account_compat_matrix,run_real_validation.sh}`, `scripts/ops/fetch_bybit_funding.py`, `runtime_logs/prop_eval/2026-06-17-validate-real/`, this file, + tests | Tier-3 (config/strategies.yaml + accounts.yaml + order-path executor). Draft PR #3840; operator approval + live smoke test required before merge/go-live. |
