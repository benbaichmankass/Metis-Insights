...
remains gated. Models at `shadow` never touch the order package (the
WS7 non-negotiable still holds for shadow predictions). Models at
`advisory` and above **can** influence the order, but only through the
single reductive path shipped 2026-05-25 (see change log) — and only
when the operator has both promoted the model past shadow **and** set
`ADVISORY_MODE=true` with a per-strategy `advisory_policy`. With either
switch off (the default) the order package is unaffected. An operator who
...
| 2026-05-25 | S-AI-WS7-INFLUENCE (#2012/#2013/#2014) | **First model→order influence path shipped — advisory downsize, default-off.** The full advisory-influence stack landed in three Tier-3 PRs: (#2012) decision-support — per-model attribution, gate-check readiness, and the advisory-stage guard; (#2013) the influence operator `apply_advisory_influence` + `parse_policy`/`AdvisoryPolicy` in `src/runtime/advisory_influence.py` — reductive-only (downsize toward `size_floor` on a bearish quorum, or annotate), with a `_assert_reductive` invariant; (#2014) the **live wiring** — new `src/runtime/advisory_sizing.py` (`compute_advisory_factor` + `apply_advisory_downsize` + `discover_advisory_stage_model_ids`) resolves advisory-stage models, scores them at execution time on an `OrderPackage`-derived feature row, and returns a factor in `[size_floor, 1.0]`; `Coordinator.multi_account_execute` applies it at the per-account sizing point (one gated call; factor computed once per package, cached on `pkg.meta`). **Gated by `ADVISORY_MODE` (default false) AND a per-strategy `advisory_policy`; reductive-only (can shrink, never enlarge); any error → qty unchanged.** Applied downsizes log to `advisory_decisions.jsonl`. `ADVISORY_MODE` is the influence-activation switch, **not** a trade-capability gate — with it off, every strategy still trades fully at full size; it pairs with the operator `shadow → advisory` promotion gate (see CLAUDE-RULES rule 6 carve-out). Enablement (promote a model to `advisory` + set `advisory_policy` + flip the flag) remains operator-gated; **no model qualifies yet** (all data-starved, ~73 trades vs the 200 gate), so the path is dormant. | `src/runtime/advisory_influence.py`, `src/runtime/advisory_sizing.py`, `src/core/coordinator.py`, `docs/sprint-plans/ai-traders/ws7-advisory-influence-operator-DESIGN.md`, `ROADMAP.md`, this file, many `tests/`. (`advisory_policy` is *read* from `config/strategies.yaml` via `parse_policy` when an operator adds it — no schema block was committed this session.) | Live VM: **none today** — `ADVISORY_MODE` off + no advisory-stage model means `apply_advisory_downsize` returns every qty unchanged. The path activates only after the operator's explicit promotion + flag + policy. |
