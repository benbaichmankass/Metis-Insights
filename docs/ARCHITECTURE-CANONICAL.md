...
|---|---|---|---|---|
| 2026-06-01 | (regime-router phases 1+2 + bar-close debounce + diag audit history) | **Regime-aware routing shipped log-only; per-bar entry debounce; historical audit query.** (1) **Regime router phase 1** (#2582): new `src/runtime/regime/{detector,policy}.py` — `detect_regime(candles)` returns `{regime: chop\|transitional\|trending, adx, source}` via Wilder ADX-14 (chop<20 / transitional 20–25 / trending≥25), one source of truth consolidating the per-strategy fade/fvg ADX gates + the research matrix. Every `*_eval` audit row now carries `regime`/`adx_14`/`regime_source` (21 stamp sites across 10 strategies via `_stamp_regime`); `StrategyIntent` gains `regime`/`adx_14`. Verified live: 119/119 eval rows stamped. (2) **Regime router phase 2** (#2583): `config/regime_policy.yaml` (regime×strategy×direction OFF cells seeded from the roster matrix) + `aggregate_intents` evaluates it per tick and emits a `regime_shadow_gate` audit event for any OFF-cell intent — **`enforced: false`, the aggregator decision is UNCHANGED** (observability-only; pinned by `test_phase_2_does_not_change_aggregator_decision`). Verified live: 85 gate events on real data, all `enforced:false`, cells matching the policy (vwap OFF all regimes; htf OFF chop). Phase 3 (hard gates behind `REGIME_ROUTER_ENABLED`) + phase 4 (soft weights + classifier-v0 detector) are Tier-3, operator-gated (PERF-20260601-006/007; operator chose **Option 2** — proceed to phase 3 after the base-rate check passes). (3) **Bar-close debounce** (#2548): one entry attempt per closed bar (`bar_debounce_blocked` audit event), verified live. (4) **Diag historical audit** (#2606): `GET /api/diag/audit_query?since=&until=&event=&strategy=&symbol=&side=&limit=&offset=` reads the `trade_journal.db::signals` dual-write (full payload in `meta`) so off-VM sessions can pull an arbitrary historical window / one event type past the 1000-line `/audit` + `/log_file` tail cap; read-only, reachable via the existing diag relay. | `src/runtime/regime/{__init__,detector,policy}.py`, `config/regime_policy.yaml`, `src/runtime/{strategy_signal_builders,intents,pipeline}.py`, `src/web/api/routers/diag.py`, `docs/research/regime-router-design-2026-06-01.md`, `CLAUDE.md`, `docs/claude/vm-operator-mode.md`, `docs/api-tier-policy.md`, `tests/test_regime_*.py`, `tests/test_web_api_diag_audit_query.py` | Live VM (deployed 2026-06-01, web-api sha `7692294`): the trader stamps regime on every eval row and logs would-gate events for OFF-cell intents **without changing any order decision** (phase 2 is observe-only); per-bar entry debounce active; off-VM sessions can query arbitrary audit history. No order-path behaviour change — phase-3 hard gates remain operator-gated. |
