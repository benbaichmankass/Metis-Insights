...
and since **E35 (2026-09-25)** the election is literally per account: the
regime gate runs once per tick (`intents.gate_intents`), then EVERY account
elects from its own declared candidates
(`arbitration_fanout.plan_per_account_election` → `intents.elect_from_gated`)
and `pipeline._dispatch_rounds` routes each account its own winner. Before E35
one GLOBAL winner per symbol was elected across all accounts and a scoped
fan-out repaired the accounts it starved (see the Change log). So a single
account running several strategies routes every tick through a **decider** — today
...
| 2026-09-06 | MI-139 / WO-20260906-A-BYBIT-REDUCE-ONLY-CLOSE-IS-REJECTED | **The qty-legalization seam covered SIZING and never CLOSING — a Bybit reduce-only close could not be placed.** `execute.close_open_position` builds its Bybit payload as `{"qty": str(qty)}` from the journal's `position_size` and calls `exchange_client.place_order` **directly**, bypassing `_submit_order` — the only Bybit submission path wired to `qty_legalize`. MEASURED: a `bybit_1`/SOLUSDT close was refused 3 consecutive times at 2026-09-06T03:37:22Z on qty `33.299999999999955` (the IEEE-754 residue of `289.4 − 256.1`; Bybit `Qty invalid`, ErrCode 10001) and the position could not be flattened by the normal path. POPULATION for the incidence: `bybit_*`, non-backtest, `position_size > 0`, `trades` ids 4519–5518 (newest 1000 rows, `/api/diag/journal?table=trades&limit=1000`, read 2026-09-06 ~11:02Z), **n = 631 → 9 off-grid = 1.43%**, including a real-money row on `bybit_2`. **Two parts, neither subsuming the other.** (1) `apply_intent_reduce_partial_close` persisted the residual as a raw float subtraction — the ONLY unrounded `position_size` writer, the two siblings that also subtract already round at 8dp/10dp — now `round(x, 8)`, the identity on every legal quantity (finest Bybit step in play is 0.001). (2) NEW `qty_legalize.snap_artifact_qty` repairs an artifact at the wire, **inside the seam** so `qty-legalization-guard` is satisfied rather than worked around, returning three never-collapsed states `snapped` / `unchanged` / `not_graded` (the last covering both *no rule resolved — we did not look* and *genuinely off-step*, both passing through untouched so a real off-step value keeps failing LOUDLY). ⚠️ **IT SNAPS TO NEAREST AND MUST NEVER BE HARMONISED WITH `legalize_qty`, WHICH FLOORS.** Flooring is correct for an ENTRY (realised risk must not exceed the sized cap) and is the OPPOSITE for a CLOSE: flooring `33.299999999999955` to `33.2` under-closes by a full step and orphans 0.1 SOL with no journal row — a loud failure converted into a silent one. The wire string changes on the `snapped` path and NOWHERE ELSE, so the blast radius is structurally bounded to orders the venue rejects today. Also: `config/instruments.yaml` XRPUSDT `min_qty`/`qty_step` 1.0 → **0.1**, MEASURED from Bybit `/v5/market/instruments-info` (read 2026-09-06T11:03:10Z, trainer-vm-diag #11120, run 34029110078). ⚠️ **AVAXUSDT was suspected wrong and is NOT** — its sub-0.1 journal rows all carry `status='rejected'`, i.e. pre-legalization sizes that never reached the venue. **NOT widened** to the two Bybit sibling paths (`modify_open_order`'s `slSize`/`tpSize`; `_bybit_top_up_partial_sl`'s `uncovered`) — no observed failure and both are *protective* paths where a rejection fails a tightening silently; filed instead. | `src/units/accounts/{execute,qty_legalize}.py`, `config/instruments.yaml`, `tests/test_bybit_close_qty_artifact.py` (NEW), `docs/sizing-legalization-DESIGN.md` (§ 3.4, NEW), `docs/claude/work/OPEN-PRS.json`, `docs/claude/health-review-backlog.json`, this file | PR #11125 — **Tier-3, operator-approved 2026-09-06 (both parts, no condition), DRAFT pending the manager routing the merge.** No live-VM behaviour change until deployed. Nothing is currently unflattenable (0 of 29 open rows, read 2026-09-06 ~10:12Z), so this is a fix for the next occurrence. Rollback is a revert; both parts are fail-safe (unresolvable rule → passthrough, byte-identical to the pre-fix line). |
| 2026-09-22 | E27 | **The `/api/diag/journalctl` allowlist gains `ssh.service`, so the `/health-review` breach sweep's MANDATORY sshd/auth source exists at all.** `_CANONICAL_UNITS` in `src/web/api/routers/diag.py` held only this project's own units, and it is EXACT-MATCH, so `unit=ssh` / `ssh.service` / `sshd` / `sshd.service` all returned HTTP 400 `unknown_unit` (measured 2026-09-22, control `unit=ict-web-api.service` -> 200 with journal lines) and that mandatory source had never been satisfiable. The entry is `ssh.service`, MEASURED on ict-bot-arm (Ubuntu 22.04.5): `sshd.service` is a systemd ALIAS and `journalctl -u sshd.service` returns `-- No entries --`, so the alias would answer 200 with `lines: []` — a sweep reporting CLEAN having read nothing. Only that one unit is added; no new route. Like `caddy.service`, it ships no `deploy/` unit of ours, so `check_diag_unit_allowlist.py` does not cover it — hand-maintained. ⚠️ **Read-surface/privacy change:** sshd journal lines carry source IPs and usernames, readable by anything holding the diag bearer — the cost was stated to the operator and accepted. No WRITE route and no secrets returned, so the closed diag-token rotation decision (`docs/reference/diag-access.md`) is not reopened. | `src/web/api/routers/diag.py`, `tests/test_web_api_diag.py`, `.claude/skills/health-review/SKILL.md`, `docs/ARCHITECTURE-CANONICAL.md` | Tier-2 — it changes a live service's read surface, so it is not in effect until `ict-web-api.service` is pulled and restarted. Closes when the probe that 400s today returns 200 with journal lines against the RUNNING web-api, with the `ict-web-api.service` control alongside. No order path, no config gate, no strategy or risk change. |
