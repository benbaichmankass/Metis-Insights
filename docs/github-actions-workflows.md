...
| `vm-devnull-source-diagnose.yml` | Ops relay | AUTONOMOUS | label `vm-devnull-diagnose` / dispatch | Identifies *what* strips `/dev/null`'s write bit on the live VM — the recurring perms regression behind the `ict-devnull-guard` self-heal. |
