...
| `claude-pr-automerge.yml` | Control path | AUTONOMOUS | push (`.github/pr-automerge-requests/*.txt`) | Opens (or finds) a `claude/**` branch's PR, marks it ready and enables native squash auto-merge — the durable path when the GitHub MCP is read-only. CI is never bypassed; branch protection still gates. ⚠️ **The warning this row carried CAME TRUE on 2026-09-02 and the trigger is now narrowed** — do not re-quote the old trigger path. It read *"merging `main` can inherit another PR's request"*, and that is exactly what happened: the legacy shared file `.github/pr-automerge-request` reached `main`, so every branch carried it, and a `git merge origin/main` un-drafted PR #10788 and armed auto-merge on it against an explicit operator instruction (`BL-20260902-MERGING-MAIN-ARMS-AUTOMERGE-ON-A-DRAFT-PR-AND-NOTHING-COULD-DISARM-IT`). Only per-request `*.txt` files fire it now; the stale file is left on disk, inert. ⚠️ It still *touches* the path check only — it never reads a request's contents — so the OFF switch below is what undoes a mistaken arm. |
