...
- **Operator action (blocks T0):** Bybit Spot Margin Trading must be
  *enabled* on the `bybit_2` account via the Bybit web UI (Account →
  Margin Mode). Until that toggle is on, every `isLeverage=1` order
  returns retCode 110007 ("MARGIN_TRADING_NOT_ENABLED"). Operator
  notebook `notebooks/operator/enable_bybit_spot_margin.ipynb` to be
  added in T0 and run by the operator before T1 starts.
- **Bybit margin tier confirmation (blocks T2):** Spot Margin uses tiered
  borrowing limits (per-coin, per-account). Operator must confirm the
  current BTC borrow cap on `bybit_2` so the RiskManager's
  `max_borrow_btc` field gets a non-fictitious default. Surfaced
  via the same notebook as a read-only diagnostic in cell 2.
...
| D2 | `config/accounts.yaml` schema: new `is_leverage` boolean + `margin:` sub-block carrying `max_borrow_btc`, `borrow_fee_apr_pct`, `liquidation_buffer_pct`. Plumbed through to `account_cfg`. | `feat(accounts): isLeverage + margin sub-block in accounts.yaml schema` | strategy / model |
| D3 | `RiskManager.position_size()` upgrade: when `is_leverage=True`, use USDT collateral as balance for *both* directions; cap qty so liquidation distance > stop distance + buffer; deduct expected daily borrow fee from per-trade risk budget. New regression tests cover: spot-long (no borrow), spot-short (BTC borrow), liquidation cap, fee-budget cap, daily-loss-budget interaction. | `feat(risk): spot-margin sizing — collateral, liquidation, borrow fees` | strategy / model |
| D4 | `execute.py`: pass `isLeverage=1` to Bybit V5 spot place_order when account is margin-enabled. Spot-sell pre-flight bypassed when borrowing. New retCode handling: 110007 (margin not enabled) and 110095 (insufficient borrow available). | `feat(exec): route spot-margin orders via isLeverage=1` | deploy / live |
| D5 | `coordinator.multi_account_execute`: direction-aware balance returns USDT collateral for both directions when `is_leverage=True`. Loaded from accounts.yaml. | `feat(coordinator): direction-aware balance for spot-margin accounts` | deploy / live |
...
| **T1** | accounts.yaml schema extension | D2 merged. `bybit_2` has `is_leverage: true` + a `margin:` sub-block with operator-confirmed defaults. Loader tests pass; legacy non-margin accounts unchanged. | strategy / model | 2h | T2, T3 |
| **T2** | RiskManager spot-margin sizing | D3 merged. New unit tests prove: short sizing uses USDT collateral, liquidation cap fires before stop cap when too aggressive, borrow-fee buffer reduces sizable budget, daily-loss-budget gate still wins on conflict. | strategy / model | 4h | T3, T4 |
...
- `bybit_1` and `prop_velotrade_1` MUST be unchanged in observable behavior. Every PR includes a regression assertion that non-margin accounts route exactly as before (no isLeverage flag set, no borrow checks, no spot-margin reconciler call).
...
- The Spot Margin liquidation buffer is **not optional**. T2 must refuse to size any short whose stop distance is closer than `liquidation_buffer_pct × liquidation_distance`. Default 30 % until the operator tunes it.
...
the toggle, it does not flip it (per Tier-3 rule).
