...
| Backstop (broker) | naked-position re-arm (journal-driven) | `run_monitor_tick` → `_check_naked_positions` → `_attempt_naked_autoprotect` → `IBClient.place_protective` | **Unconditional** since PR #3674 (was the `NAKED_POSITION_AUTOPROTECT` flag; removed) |
| Backstop (broker-VERIFY) | broker-side resting-bracket check + re-arm — catches a position whose journal SL/TP is intact but whose *broker* bracket was dropped (Alpaca RTH day-TIF cancel / IB never-placed or breaker-flap) | `run_monitor_tick` → `_check_broker_naked_equity_positions` (Alpaca, `has_protective_orders`, every tick) **and** `_check_broker_naked_ib_positions` (IB, `IBClient.has_protective_orders` = account-wide `reqAllOpenOrders`, cadence-gated `IB_BROKER_NAKED_CHECK_SECONDS`) → `_attempt_naked_autoprotect` | **Unconditional** (Alpaca, BL-20260629); **cadence-gated on-by-default** (IB, BL-20260709 — the account-wide read is the pacing cost). Fail-safe: a could-not-confirm read never re-arms |
