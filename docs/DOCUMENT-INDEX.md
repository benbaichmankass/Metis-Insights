...
| `docs/research/m20-u25-performance-sign-agreement-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u25:self-measured-2026-09-12 / live-endpoint-read / population-stated / agreement-declared-not-a-pass` | MI-278 U25: **`/api/bot/performance` STILL publishes an `expectancyR` that contradicts its own PnL, six days after `PB-20260906-R-CONTAMINATION-QUANTIFIED-AND-THE-HEADLINE-SIGN-IS-WRONG` filed it.** Read live 2026-09-12 over the Caddy host (endpoint defaults `demo=False`, real money only): on **`30d`**, `totalPnl −14.5552` / `expectancy −0.355` / `profitFactor 0.8251` all say LOSING while **`expectancyR` reads +0.1113**. ⚠️ **`rCoverage` reads `1.0` on the same payload**, so the surface asserts full R coverage beside the contradicted number while its own `rProvenance` grades **11 of 41** rows `contaminated`. ⚠️ **THE FLIP IS SHARE-DRIVEN, NOT COUNT-DRIVEN, AND THAT ROW'S FRAMING BY COUNT POINTS THE WRONG WAY**: the `all` window carries MORE contaminated rows (14) than `30d` (11) and its sign **agrees** — 3.3 per cent against 26.8 per cent — so the long windows are the SAFEST and the corrupted figure sits on the short window an operator opens by habit. **Three distinct windows exist** (`7d` / `30d` / `all`); the endpoint serves `all` for any unrecognised value and **honestly echoes `window: "all"`**, credited as correct behaviour rather than filed. ⚠️ **AGREEMENT IS DECLARED NOT-A-PASS** on every graded window — `7d` agrees on zero contaminated rows at `profitFactor 0.0`, `all` agrees by dilution. **Criterion (2) (the `|R|>10` control) is NOT tested and not claimed.** ⚠️ **Closes neither row** — the fix is Tier-2 in `src/web/api/routers/performance.py`; what this adds is that criterion (1) is now RUNNABLE in one command instead of a full re-derivation. **Nothing enacted; read-only.** |
