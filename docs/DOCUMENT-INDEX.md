...
| status | rows | meaning |
|---|---|---|
| `live` | 45 | Current. A session may act on it today. |
...
| `docs/claude/high-severity-class-map-2026-09-12.md` | unknown | unknown | — | never | `not-assessed / not-assessed` | — |
...
| `docs/design/open-items-loud-triage-PROPOSAL-2026-09-12.md` | architecture | unknown | — | never | `dir:design-declares-contracts / not-assessed` | — |
...
| `docs/research/m20-u17-arbitration-fallback-inversion-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u17:self-measured-2026-09-12 / population-stated / reproduces-the-audit-exactly / corrects-the-rows-contention-clause / ships-two-detectors` | MI-278 U17: ships **part 1** of `BL-20260909-UNKNOWN-STRATEGY-PRIORITY-NOW-BEATS-45-OF-50-DECLARED-LEGS-AND-THE-CONTENTION-IS-LIVE` (the two CI detectors, Tier-1) and **proposes part 2** (`src/runtime/intents.py`, Tier-3) rather than taking it. **Population: AST over `src/runtime/intents.py` + YAML load of `config/strategies.yaml` at `origin/main` 4a91b834e, 2026-09-12 — `DEFAULT_PRIORITIES` n=50, histogram {0:41, 1:1, 2:1, 3:1, 5:1, 10:1, 20:1, 30:1, 40:1, 50:1}, `_UNKNOWN_STRATEGY_PRIORITY`=10, 45 of 50 legs strictly below it, 55 declared / 45 execution:live / 44 enabled, five absent legs, 0 orphan entries.** Every figure REPRODUCES the 2026-09-09 audit's F-29 exactly three days on — the inversion has not moved. ⚠️ **Both new invariants FAIL on `main` as it stands**, so they ship armed against a **pinned, self-clearing waiver** (pins `observed_constant: 10` / `observed_min: 0`; re-fires if either term moves; **fails if the waiver is still present once the constant is fixed**) — arming hard would have turned every open PR red for a condition none of them caused. Self-test **8 → 38 controls**, every new refusal path planted in isolation with a negative control beside it; **seven never-collapsed read states, all of which REFUSE**, because a guard that cannot find the constant it watches is not watching it. ⚠️ **Corrects the row's contention clause and the correction changes the remedy**: the SLV rival `slv_trend_1h` is `execution: shadow`, not live — contention is real (a shadow leg is not excluded from the election, `intents.py:182-187`) but **today the inversion makes the LIVE leg beat the DATA-ONLY one**, so mapping `slv_pullback_1d` at its siblings' 0 creates a tie that can hand SLV to a shadow leg. Filed as `BL-20260912-MAPPING-AN-UNLISTED-LEG-AT-ITS-SIBLINGS-VALUE-CAN-HAND-ITS-SYMBOL-TO-A-SHADOW-RIVAL`. The other four absent legs have **no rival at all** on their symbol, so their priority is inert today. **No misrouting observed — the arbitration outcome is not journalled per-tick. Nothing deployed, nothing enacted.** |
...
| `docs/research/bleed-attribution-2026-09-11.md` | evidence | unknown | — | never | `dir:research-is-measurement / not-assessed` | — |
...
| `docs/research/m20-u1-winner-close-inventory-2026-09-12.md` | evidence | unknown | — | never | `dir:research-is-measurement / not-assessed` | — |
...
| `docs/research/winner-size-collapse-2026-09-12.md` | evidence | unknown | — | never | `dir:research-is-measurement / not-assessed` | — |
...
| `docs/sprint-logs/S-MI195-QLD-TQQQ-EXAMINED-2026-09-08.md` | history | historical | — | 2026-09-08 | `dir:sprint-logs-are-session-records / dir:record-of-a-completed-session` | — |
| `docs/sprint-logs/S-MI215-SOAK-DOCTRINE-REACHES-M7-GATE-2026-09-09.md` | history | historical | — | 2026-09-09 | `dir:sprint-logs-are-session-records / dir:record-of-a-completed-session` | — |
