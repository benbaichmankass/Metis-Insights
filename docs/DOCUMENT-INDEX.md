...
| `docs/claude/work/BYBIT-HEDGE-BOOK-FLAT-READ-2026-09-08.md` | evidence | live | — | 2026-09-08 | `mi-204:self-measured-2026-09-08 / population-stated` | MI-204: the bybit_1 ETHUSDT "orphan" (trade 5569) is the same position the netting reconciler false-closed 4 min earlier. Root cause `_bybit_position_protection` grades a symbol off `get_positions` rows[0], so a zero-size hedge book reads FLAT — 3 provable false closes, +$115.86 fabricated ESTIMATED pnl, real money in the exposed population. Fixes proposed here were APPLIED 2026-09-08 in `46e1efb1f` (PR #11435) and are DEPLOYED (verified 2026-09-12) — this row read "NOT applied" until 2026-09-12 and that is stale; field beats comment. Observation: `HEDGE-BOOK-FLAT-READ-OBSERVED-2026-09-12.md`. |
| `docs/claude/work/HEDGE-BOOK-FLAT-READ-OBSERVED-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-281:self-measured-2026-09-12 / population-stated` | MI-281: the hedge-book flat read is repaired AND OBSERVED on the fleet. All three clauses of OI-20260908 carry evidence, reported separately: landed+DEPLOYED (c5a0ba993) with 4 mutation checks; two post-deploy soak rows on a hedge-armed symbol with NON-ZERO `exchange_qty` and `position_idx: 1`; zero netting_attributed→same-size-adopt pairs against a positive control finding 2 pre-deploy. ⚠️ The NINE unestablished pre-deploy flat reads are PERMANENTLY unadjudicable (the fields did not exist) — a gap in the evidence, NOT a clean bill. Observation only; no `src/`/`config/` change. |
| `docs/claude/work/BYBIT-SYMBOL-DEDUPE-REPAIR-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-283:self-measured-2026-09-12 / population-stated` | MI-283: `account_open_positions` deduped Bybit positions by SYMBOL alone, dropping the second live hedge book — MEASURED 376 of 376 `bybit_1` reads (100.0%) over 2026-09-12T03:20:49Z→07:36:53Z, and the cause of two false closes worth -$762.496 (#11867). Repairs the dedupe to `(symbol, position_idx)` at BOTH sites (the second, `account_bybit_open_orders`, is not named by the backlog row). Tier-2 order path — PREPARED AND PROPOSED, not merged on a session's judgement. Also resolves MI-281's handed-on U2 finding: the two unexplained adopts are the SAME defect seen from the other side, not an adopt-attribution defect. |
...
| `docs/research/bleed-attribution-2026-09-11.md` | evidence | unknown | — | never | `mi-271:self-measured-2026-09-11 / population-stated` | MI-271: attribution of the directional-leg bleed. THE BLEED BEGINS 2026-08-27, THREE DAYS BEFORE the e35 geometry shipped (08-30T08:53Z), and the worst day (2026-08-28, -$6,654) is two days before it — 16 consecutive losing days, -$36,997 on bybit_1. Two-thirds of the post-deploy loss (-$30,057, 66%) sits in `ict_scalp_*` legs that declare no bracket geometry at all and carry the BEST provenance (0.65); e35 legs carry 23% at coverage 0.30. Mechanism is WINNER-SIZE COLLAPSE (avg win $404 -> $112, -72%) not a win-rate collapse (-4.9pp). e35 does carry a real provenance-clean secondary effect — stop-out rate 0/5 -> 6/7 on broker fills, p=0.015 — but that it made its legs worse OVERALL is REFUSED (p=0.334 on measured rows). Control decontaminated of the 08-29 B4 change; exit location ADJUDICATED (the `reconciler_filled` frozen label is 40.5% of rows) with an 88.4%/100% positive control. MFE-at-stop NOT ANSWERABLE (telemetry arrived 2026-09-07, eight days late). Real-money loss -$20.36. All Tier-3 remedies are PROPOSALS. |
| `docs/research/stop-width-counterfactual-2026-09-11.md` | evidence | live | — | 2026-09-11 | `mi-275:self-measured-2026-09-11 / population-stated` | MI-275: the stop-width counterfactual MI-271 named as the precondition for any e35 revert. **DO NOT REVERT e35.** FORWARD: of the 5 post-deploy packages ended by their own entry-declared e35 stop, **5 of 5 would also have hit the pre-e35 2.5-ATR stop** and **0 of 5 reached a take-profit at any horizon** (2 within 9 minutes of the real exit, 3 deferred 4h-4.4d then stopped). REVERSE (no horizon needed — each trade's own lifetime bounds it): the tighter stop would have killed **1 of 14 pre-era winners, $5.61 of $24,967.77 = 0.02%**; the top ten winners all have MAE ≤ 0.90 ATR. Sizing is `(balance × risk_pct) / abs(entry − sl)` and the target is `tp_r × risk_distance`, so **dollar risk AND reward are invariant to stop width** — reverting buys the same losses later. WHAT CHANGED, truncation confound removed (fixed windows from entry): adverse excursion **1.4–1.8×** larger, favourable **0.19–0.58×** smaller, favourable:adverse 1.04–4.59 → 0.33–0.54, measured in ATR units so invariant to the multiplier. ATR/price did NOT collapse (0.75–1.14×) — chop at unchanged volatility, a statement about ENTRIES not exits. Dose-response INVERTS (20% cut → stop rate 0.80, p<0.0001; 40% cut → 0.25, p=0.47) but is corroboration only: **both designed controls have ZERO observations**. 3 positive controls ran first; PC2 caught a geometry error (the declared stop is anchored to the PACKAGE entry, not the fill) and all 33 packages' declared distances then land exactly on their era's multiplier, confirming the deploy reached the trader. **MI-271 is ROBUST, not corrected** — recomputed per ORDER PACKAGE (fan-out inflates n 1.27–1.64×, unequally) every conclusion survives and only the p-values weaken (e35 stop rate p=0.023; PRE falsifier still passes p=0.304). The MI-271 memo-vs-summary contradiction is a mis-compression in one field: the merged memo is right. All Tier-3 items are PROPOSALS. |
| `docs/research/m20-u1-winner-close-inventory-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u1:self-measured-2026-09-12 / population-stated / call-sites-asserted` | MI-278 U1: **what can end a WINNING trade, per leg, and what arms it** — the denominator MI-277's lever refutation lacked (its own memo scopes that to `bybit_1` in its window). Inventory over **55 declared / 44 enabled+live** legs, monitor unit **IMPORTED** from `pipeline.monitor_unit_for`, **every claimed call site ASSERTED against the unit source** (the script FAILS rather than reporting a stale table), cross-checked against 1000 journal rows: **24 mechanism/leg pairs confirmed, 0 contradictions**. **THE TWO GATES:** a mechanism fires only if the unit implements the call site AND the YAML declares the key. Armed over the 44: `tp_cross` 44 · `trail_base` 35 · `trail_decay` 16 · `breakeven_ratchet` 8 · `stale_stop` 3 · `exit_head` 3 · **`giveback_stop` 1** — the only lever that fires by construction on a trade that WAS winning. Seven of the eight `ict_scalp_*` legs arm exactly `{tp_cross, sl_cross, breakeven_ratchet}` and **the unit reads no `trail_mult` at all**. ⚠️ **THE HEADLINE IS ABOUT THE INSTRUMENT:** `reconciler_filled` is **84 of 294 closes (28.6%) and 35 of 112 winners**, all stamped `exit_reason_source: unresolved`, and it can never name a mechanism **by construction** — `_close_trade_from_order_status` watches the **ENTRY** order. `_classify_broker_exit`'s back-fill refuses on **27 of 27** broker-truth rows because each fill lands **SHORT** of the level its strict inequality tests (median **−0.79bp** stop side / **−0.55bp** target side; 25 of 27 within 25bp; **zero went through**) — market-on-trigger, not a resting limit. So exit labels **undercount** stop and target hits, and MI-277 §2.2's *reached a declared target* cell (n=7/9) is drawn from that censored population. **The reader that would settle it already exists and is UNWIRED:** `account_bybit_raw_order_history` returns `stopOrderType`/`cancelType` and its only non-test caller is a diag route. **REFUTED WITH CONTROLS:** the netting shared-bracket cascade (4% co-timing vs a 3% `sl` control) and the break-even ratchet (the family *without* it moved further into the scratch band). ⚠️ Corrects MI-277 §3.5 — `stale_stop` fired **6×** inside its exact population (−$4,742.85, all losses), so its zero is **winner-scoped**; its conclusion is untouched. No coverage-matrix cell changes (U1 ran no sweep). All Tier-3 items are PROPOSALS; nothing enacted. |
| `docs/research/m20-u5-excursion-regime-instrument-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u5:self-measured-2026-09-12 / population-stated / validated-on-a-known-event` | MI-278 U5: the **favourable:adverse excursion instrument in ATR units**, closing the COMPUTATION half of `BL-20260911-MFE-OVER-MAE-IN-ATR-UNITS-IS-A-LEADING-INDICATOR-NOBODY-COMPUTES`. Needs **no exit price, no PnL provenance, no stop, and no knowledge of which lever closed the trade** — which matters more than when the row was filed, because U2b established **R is contaminated too** (the break-even ratchet amends `order_packages.sl` on winners only), leaving ATR units the only basis immune to both. **VALIDATED at 4 of 4 windows** on MI-275's own population: F:A **2.889 → 0.548** at 48h (MI-275: 4.585 → 0.542), MFE tracking closely (48h pre **6.564** vs **6.690**) and the post-era ratio within 1%. ⚠️ **One delta stated, not smoothed:** my pre-era MAE reads higher (2.272 vs 1.459), so the pre-era F:A is lower — MFE agrees and MAE does not, and the cause is unresolved. ⚠️ **I graded ALL packages first and it silently changed the answer** (48h pre F:A 0.865 on n=49 vs 2.889 on n=17); the script now warns when `--trades` is omitted. **NEW FINDING:** the excursion collapse is far more an **e35-leg** phenomenon than a scalp-family one — on `ict_scalp` 15m the move is slight at 4–24h and **absent at 48h**, against a 5.3× fall on the e35 legs, which is not what MI-275's *reaches legs across the book* framing predicts. **Fetch costed** as the row demanded: `api.bybit.com` is **403 geo-blocked** from these containers, `data.binance.vision` answers 200 in 0.8s, crypto perps only (equity/futures legs NOT covered). ⚠️ **NO CADENCE IS SHIPPED** — MI-276's `losing_streak_alert` is merged and the row warns that two cadences in one window is how they disagree, so the carrier is put to the operator as a Tier-1 decision with three options costed. The backlog row stays **OPEN**: *a session is OBSERVED reading it* cannot be satisfied by the session that wrote it. |
| `docs/research/m20-u9-netting-close-venue-adjudication-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u9:self-measured-2026-09-12 / population-stated / matcher-graded-against-an-independent-field` | MI-278 U9: discharges option (b) of `BL-20260912-THE-NETTING-ATTRIBUTED-PATH-CARRIES-THE-LARGEST-SHARE-OF-THE-WINNER-COLLAPSE-AND-IS-100-PERCENT-ESTIMATED` for **25 of 50 rows**, against `/api/diag/bybit_raw_closed_pnl` — the SECOND venue endpoint, chosen because `OI-20260908`'s mechanism is a misread of the POSITION path. **Population: 50 closed `netting_attributed` rows in the newest 1000-row `/api/diag/journal` tail read 2026-09-12 (bybit_1 39, bybit_2 6 REAL MONEY, bybit_portfolio 5; closes 2026-08-18T06:25:02Z→2026-09-11T08:30:54Z) against 27 venue windows / 301 records, 0 truncated, 0 errored.** ⚠️ **The matcher's own control flipped the headline's sign** — side+qty matches graded on `avg_entry_price` give 25 corroborated / 8 weak / 5 refuted, and journal-minus-venue is **−1,955.86 over all 38 matches vs +4,416.65 over the 25 corroborated**. **THE CLOSES ARE REAL on 25 of 25** (a blanket 'these never happened' reading is refuted); the gap decomposes exactly into **price +2,040.68 (46.2%, the estimation error)** and **fees +2,375.97 (53.8%, a basis difference — the journal figure is gross on 25 of 25)**; **5 of 25 disagree on the SIGN** (largest +30.23 journal vs −953.15 venue); and **3 were booked by the venue 61–173 min AFTER the journal closed the row** against a median of −0.3, one of them **trade 5568, the exact row `OI-20260908` names** — venue-side timestamped corroboration of that item, which it does NOT clear. Dollars are almost all paper (bybit_2 differs by $2.12 over 2 rows, and that is n=2, not 'harmless'). The unadjudicated half is carried by `BL-20260912-HALF-THE-NETTING-ATTRIBUTED-POPULATION-CANNOT-BE-ADJUDICATED-ONE-TO-ONE-AGAINST-THE-VENUE`. |
| `docs/research/m20-u16-netting-unmatched-remainder-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u16:self-measured-2026-09-12 / population-stated / imports-u9s-matcher / headline-is-a-refusal` | MI-278 U16: closes out option **(b)** of `BL-20260912-THE-NETTING-ATTRIBUTED-PATH-CARRIES-THE-LARGEST-SHARE-OF-THE-WINNER-COLLAPSE-AND-IS-100-PERCENT-ESTIMATED` **as UNREACHABLE for the remainder**, leaving the row on option (a). **Population: 50 closed `netting_attributed` rows from `/api/diag/journal` limit=1000 pulled 2026-09-12T16:0xZ, against `/api/diag/bybit_raw_closed_pnl` over 27 windows / 12 (account,symbol) pairs — 0 truncated, 0 errored, 0 load problems. U9's split reproduced exactly (38 matched / 10 different-qty / 2 no-rows); the 12 U9 left are what this grades.** ⚠️ A newer tail than the row's original 39 — not the same rows, and its numbers are not restated. **Hypothesis tested and mostly refuted: `sum_matched_unique` = 0 of 12** (8 `no_subset_sums`, 2 `venue_silent`, 2 `ambiguous_multiple_subsets`), so U9's 1:1 side+qty key is NOT the dominant cause and a sum-aware matcher does not rescue this population. ⚠️ **The more useful result is that the obvious extension is UNSOUND: subset-sum is under-determined.** Trade 5079 admits 5 distinct venue-PnL answers and trade 4930 admits **74**, spanning $37.01 — the same trade resolves to $199.09 at a 360-minute window and $180.53 at 1440. Both are graded as refusals with `n_alternatives` and the spread published; **fixing the window would pick one arbitrary answer and hide it.** ⚠️ **Corrects its own author's intermediate pass**, which reported '2 of 10 explained' — an artifact of taking the FIRST combination found. **Largest unadjudicated figure: trade 5451, `bybit_portfolio`/BTCUSDT, journal −$2,250.34, venue silent on the closing side.** Names but explicitly does NOT test the candidate that would explain the whole remainder — a `netting_attributed` row's `position_size` is assigned by `_reconcile_netting_partial_closes`, not read from a fill. **Establishes no false close, disturbs none of U9's 38, and n is 12 throughout. Nothing enacted.** |
| `docs/research/m20-u12-fanout-denominator-surfaces-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u12:self-measured-2026-09-12 / population-stated / every-candidate-classified-by-reading` | MI-278 U12: discharges the **"name the surfaces"** clause of `BL-20260911-A-RATE-PER-TRADE-ROW-COUNTS-ACCOUNT-FANOUT-AS-INDEPENDENT-AND-INFLATES-N-UNEQUALLY`. **Population: `scripts/{research,ops,reports,ml}/` + `src/web/api/routers/` at `origin/main` read 2026-09-12 — 57 candidates (reads `trades` AND forms a rate), of which 36 read the LIVE journal and 21 only a backtest corpus (no accounts, no fan-out); all 36 classified by READING the code, evidenced to a `path:line`; journal `/api/diag/journal?table=trades&limit=1000`, 1000 rows / 791 packages / 0 without a package id.** **21 of 36 publish a rate whose numerator is what PRICE did over a denominator of ROWS**; only 2 already dedupe by package; 0 undeclared. ⚠️ **Three are LIVE DASHBOARD surfaces** — `/api/bot/stats` `winRate` (the first number both apps render), `/api/bot/performance` `winRate`/`expectancy`/`expectancyR`/`rCoverage`/`reachedRatio`, and `/api/bot/attribution`'s real-money win rate — and **`performance.py` already guards this same hazard in the OPPOSITE direction, its own comment naming `winRate` as the casualty** (it de-dupes packages-per-trade and leaves trades-per-package open). ⚠️ **`e35_break_attribution.py` builds a one-sided binomial p-value on a ROW count** and never reads `order_package_id`. ⚠️ **And MI-275's own script breaks its own stated universal rule in `dose_response()`** — the function that produced the inverted dose result cited elsewhere as evidence against a stop-width mechanism; **not re-graded here**. **Two inflations, never pooled** (overall 1.264× · cross-account **1.238×** · intra-account 1.021×, and a naive package-dedupe removes both), plus the discriminator the row lacks: **restricted to one account it is ~1.00×**, so two surfaces are safe only because they require `--account`. Ships a **declared registry, not a guard**, on the row's own reasoning. **Nothing is fixed; the dashboard routes are Tier-2 and no change is proposed.** |
| `docs/research/m20-u18-e35-dose-response-still-not-gradeable-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u18:self-measured-2026-09-12 / population-stated / imports-u10s-recover-and-u9s-load-venue / headline-is-a-refusal / corrects-the-rows-next-action` | MI-278 U18: answers the arithmetic question MI-278 U11 left on `BL-20260911-BOTH-OF-THE-E35-EXPERIMENTS-DESIGNED-CONTROLS-HAVE-ZERO-OBSERVATIONS` — **does venue recovery return enough of the unnamed closes to make the e35 dose-response gradeable? NO.** **Population: `/api/diag/journal?table=trades&limit=1000` pulled 2026-09-12T20:0xZ (1000 rows, 2026-08-18T02:07:30Z → 2026-09-12T20:00:15Z — a NEWER TAIL than U11's, not the same rows); treated set re-derived from ship commit `892c9a2c8`'s own diff (9 legs, 10 field edits, reproducing U11 independently); control `trend_donchian_eth` + `trend_donchian_sol` re-verified HELD at 2.5 at `892c9a2c8^`, `892c9a2c8` and `HEAD`; 27 treated / 12 control closes kept with every exclusion censused; 28 of 28 venue windows fetched, 0 errors, 0 truncations; every arm row on a bybit account so recovery is applicable to all 21 unnamed closes.** **Recovery returns 7 of 21 (33%) — 4 of 17 treated (24%) against 3 of 4 control (75%)**, an asymmetry that narrows but survives a like-for-like restriction (38% vs 75%, n=8 vs n=4). ⚠️ **The arm the outcome depends on is the arm recovery fails on.** The failure is at the MATCH step (12 of 13 grade `unnameable_no_match`), and `netting_attributed` recovers 1 of 5 against `reconciler_filled`'s 6 of 16 — consistent with U16's finding that such a row's `position_size` is ASSIGNED rather than read from a fill. **After recovery: treated 14 of 27 gradeable (13 still unnameable, 48%), control 11 of 12; stop rate 0.786 vs 0.364, two-sided Fisher p=0.0486.** ⚠️ **THAT p IS NOT A FINDING**: imputing the unnameable at both extremes puts treated in [0.407, 0.889] and control in [0.333, 0.417], the bands OVERLAP, and the worst case **reverses the sign** at a flat p=1.0 (range 0.0009..1.0000). The criterion is sign-survival, the weakest reasonable test, so failing it is decisive; the instrument refuses to print the p without the band. **Corrects the row's `next_action`**, which still pointed at the exit U11 refuted — the field a session reads first. **Adds a measured ceiling on the retrospective route**, so U10's Tier-2 persistence build cannot be deferred on the theory that a join would do. ⚠️ Explicitly **NOT** evidence for `BL-20260912-FOR-55-PERCENT-OF-WINNERS-THAT-MISSED-TARGET-THE-MECHANISM-THAT-STOPPED-THE-RUN-IS-NOT-IN-THE-JOURNAL`, whose criterion excludes a retrospective re-grade by name. **No config change proposed. Nothing enacted.** |
| `docs/research/m20-u19-package-denominator-rederivation-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u19:self-measured-2026-09-12 / population-stated / imports-cf-and-ba-unmodified / additive-change-proven-key-by-key / corrects-its-own-authors-prediction` | MI-278 U19: pays **clause 1** of `BL-20260911-A-RATE-PER-TRADE-ROW-COUNTS-ACCOUNT-FANOUT-AS-INDEPENDENT-AND-INFLATES-N-UNEQUALLY` for the surface MI-278 U12 named as most load-bearing and left explicitly unpaid — `dose_response()`, whose file promises per-package rates and whose p<0.0001 the e35-controls row leans on. **Population: `/api/diag/journal?table=trades&limit=1000` pulled 2026-09-12T20:0xZ; the bucketing, era split and adjudicator are CF's/BA's own, IMPORTED UNMODIFIED, so any difference is the denominator and cannot be a different population.** **THE NAMED CELL: `tightened_to_2_from_2.5` Fisher p goes ~0 (rows) → 0.021 (packages)** — pre 0/17→0/7, post 12/15=0.800→5/7=0.714, inflation 1.889× pre and 2.143× post. **The conclusion survives; its strength does not.** ⚠️ **The dose INVERSION survives** (20% tightening 0.714 vs 40% tightening 0.250), so the evidence against a stop-width mechanism is **not** a fan-out artifact — worth checking because those arms are inflated 1.000× against ~2×. ⚠️ **A finding row-counting hides: the PRE cell holds 2 of 9 packages whose OWN ROWS DISAGREE about the exit**, so the perfect `0/17` sits in the most inflated cell in the table. A disagreement is **reported, never arbitrated** (U15 measured row choice as worth 4.3–28.2pp). ⚠️ **AND IT AUDITS ITS OWN AUTHOR: MI-278 U18's arms are inflated 1.421× treated against 1.000× control — maximally unequally — and re-derived per package its headline goes p 0.0486 → 0.1984 (no longer significant) while the sensitivity band NARROWS and sign-survival PASSES by +0.0044. The two criteria disagree**, which shows U18's sign test is insensitive to n; filed as `BL-20260912-THE-SIGN-SURVIVAL-CRITERION-IS-INSENSITIVE-TO-N-SO-IT-CAN-PASS-A-COMPARISON-WITH-NO-POWER`. The author's own prediction, recorded on the board before measuring, was the opposite and is corrected in the memo. **The change to `dose_response` is PROVEN ADDITIVE key-by-key against the pre-patch function; no published figure moves. The three live dashboard routes are Tier-2 and untouched. 19 of the 21 surfaces remain. Nothing enacted.** |
| `docs/research/m20-u24-exit-label-state-census-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u24:self-measured-2026-09-12 / population-stated / positive-control-on-the-zero / refutes-its-own-hypothesis` | MI-278 U24: **the 'unattributable' winners are in TWO categorically different states and `BL-20260912-FOR-55-PERCENT-OF-WINNERS-THAT-MISSED-TARGET-THE-MECHANISM-THAT-STOPPED-THE-RUN-IS-NOT-IN-THE-JOURNAL` counts them as one.** **Population: one /api/diag/journal pull of 1000 rows → 68 graded** (closed, non-backtest, pnl NOT NULL, since 2026-08-27, winners, pairs excluded per the row's own scoping); the field probe runs over all 362 closed non-pairs rows. **`reconciler_filled` carries `notes.exit_reason_source: unresolved` on 29 of 29** — the classifier RAN, LOOKED and declined — **while `netting_attributed` carries NO KEY on 7 of 7**, which `order_monitor`'s own comment calls *'a 100% signature that none had ever reached the classifier'*. **Different remedies follow**: the first ran against an unusable PRICE (a venue `cancelType` is independent evidence, and its target list is already enumerable); the second runs no classifier at all, so a column written by a path that writes nothing stays empty. **The field premise is CONFIRMED WITH A POSITIVE CONTROL** — `stop_order_type` / `cancel_type` / camelCase = **0** occurrences against `exit_price_source` 357, `pnl_source` 325, `close_exec_type` 186, and the probe REFUSES rather than reporting a zero when no control field is present. ⚠️ **IT REFUTES ITS OWN AUTHOR'S HYPOTHESIS AND THE ROW'S IMPLIED GAP: the reader already exists and is correct** — `src/web/api/routers/performance.py` separates `label_unattested` / `label_refused` / `label_unresolved` / `label_attested`, so the absent-key netting rows are NOT silently counted as clean. What is missing is only that nobody has read it for this population. ⚠️ **Most of the `unattested` column is BY DESIGN** — the classifier is scoped to reconciler-derived buckets, so `tp_cross`/`sl_cross`/`giveback_stop` being unattested is correct; the 12/12 split naming which reasons are mechanism-silent is a stated JUDGEMENT. ⚠️ **CANNOT close the row**: the durable surface is Tier-2, and the other clearing condition needs a LATER window. **Read-only `GET /api/diag/*`; nothing enacted.** |
...
| `docs/research/m20-u15-stop-integrity-both-arms-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u15:self-measured-2026-09-12 / population-stated / reproduction-control-passes / corrects-its-own-authors-published-number` | MI-278 U15: discharges `BL-20260911-A-TRAILED-STOP-OUT-IS-ATTRIBUTED-TO-THE-DECLARED-GEOMETRY-THAT-DID-NOT-END-IT` **for the analyses that exist today** (its criterion says *any* analysis, a standing obligation, so the row is updated and NOT resolved). **Population: `/api/diag/journal` trades+order_packages limit=1000 pulled 2026-09-12T16:0xZ; unit = ORDER PACKAGE; population rule and split instant imported from MI-271/MI-275; e35 18 pre / 17 post, untouched_control 62 / 71; ZERO packages skipped in either arm.** **THE SPLIT: e35 pre 2 stop-outs (2 declared / 0 amended), post 8 (6 / 2); control pre 31 (22 / 9), post 41 (33 / 8) — every amendment TIGHTER, zero wider, zero ungradeable.** ⚠️ **Trailing is NOT an e35 phenomenon and only the control arm could show it** — the control amends 29% of pre-era and 20% of post-era stop-outs, so MI-275's correctly-e35-scoped 2-of-7 reads as an e35 characteristic when it is simply what trailing does. ⚠️ **IT CORRECTS A NUMBER ITS OWN AUTHOR PUBLISHED FORTY MINUTES EARLIER**: MI-278 U5's stop-rate DiD of **+21.2pp** is **+7.6pp** declared-only on U5's own row selection — the split removes about two-thirds — because the treated delta falls 35.9→24.2pp while the control's RISES 7.7→11.0pp. **And a SECOND undeclared degree of freedom was found while measuring the first**: which fan-out row is `rows[0]` moves the ADJUDICATION, not just the excursion window U5 identified, so the DiD ranges **4.3–28.2pp on identical data** — a factor of 6.5 — with the treated arm insensitive (35.9pp under all three selections) and **the entire swing arriving through the CONTROL**, which is why single-arm studies never saw it. Filed as its own high row. **Sign positive in all six combinations, so the e35 stop-side effect is real; magnitude a third to a half of what was published, on an e35 pre cell of TWO stop-outs, and no p-value is offered because one computed on n=2 would be decoration.** ⚠️ **U5's excursion conclusion is untouched** — a fixed window from entry does not depend on stop adjudication. Reproduction control PASSES (7 shared, 0 disagreements) against MI-275's recorded verdicts, the classification being a forced second copy. **Nothing enacted.** |
| `docs/research/m20-u21-fanout-exit-unit-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u21:self-measured-2026-09-12 / population-stated / harness-shown-able-to-fail / does-not-close-the-row-it-advances` | MI-278 U21: **declares what a fan-out package's EXIT is, so nothing decides it by input order.** Advances and deliberately does **NOT** close `BL-20260912-WHICH-FANOUT-ROW-IS-ROWS-0-MOVES-THE-STOP-OUT-ADJUDICATION-AND-SWINGS-THE-HEADLINE-DID-BY-10-POINTS`. **THE RULE: the unit is the PACKAGE and no row is selected** — verdict `unanimous` / `disagreement` / `ungradeable`, a disagreement REPORTED and never arbitrated, which removes input order by construction rather than by pinning a sort in one caller (which the row's own criteria say closes nothing). **Population: one `/api/diag/journal` trades pull of 1000 rows joined to 1000 indexed order packages, tol 0.001; arms/eras/leg membership imported UNMODIFIED from `stop_integrity_both_arms.build`, so a difference is attributable to the unit rule and cannot be a different population.** **RESULT: the DiD is a BAND of +3.8pp to +29.6pp, sign survives every resolution of the disagreements; it CONTAINS all three of U15's published points (17.9 / 21.2 / 28.2pp) and is wider than their spread**, because each point silently resolved the contradictions. ⚠️ **12 packages contradict themselves and 10 of the 12 are in the CONTROL arm** (2 / 0 / 3 / 7 across treated_pre / treated_post / control_pre / control_post) — the mechanism behind U15's finding that the treated arm read stable while the whole swing came through the control. ⚠️ **UNPLANNED: U15's own code, unmodified, does not reproduce U15's own table** — `control_pre` stop-outs read **30** today against a published **31**, every other cell identical. **Row-order flip vs a re-stamped exit price cannot be distinguished without U15's original pull, which was not retained, and the memo says so rather than picking one.** ⚠️ **`sign_survives` invents no alpha and is insensitive to n** — the treated_pre cell holds 2 stop-outs in 18 packages; a direction, not an estimate. ⚠️ **The band bounds DISAGREEMENTS ONLY, never the `ungradeable` exclusion** (0 on this population — a property of this week's data, not a guarantee, which is why the count is reported). ⚠️ **Does NOT close the row**: its criteria require the analyses to SHARE the declaration and the two callers stay unwired — `stop_width_counterfactual_2026_09_11.py` is open under PR #12092 and editing it here would guarantee a conflict. **Nothing enacted; no `src/`, no config, no order path.** |
| `docs/research/m20-u22-wire-fanout-exit-unit-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u22:self-measured-2026-09-12 / population-stated / additivity-proven-not-asserted / held-on-unmerged-pr` | MI-278 U22: **wires BOTH known callers to U21's single declared fan-out-package exit rule**, which is the structural half of `BL-20260912-WHICH-FANOUT-ROW-IS-ROWS-0-MOVES-THE-STOP-OUT-ADJUDICATION-AND-SWINGS-THE-HEADLINE-DID-BY-10-POINTS` — a declaration nothing imports closes nothing. `stop_width_counterfactual_2026_09_11.py::_per_package_rates` carried a **VERBATIM SECOND COPY** of the rule and now calls `fanout_exit_unit.package_exit_verdict`; the CLASSIFICATION moved and the POLICY did not (that helper still excludes a contradicting package from numerator and denominator, which is the caller's question). `stop_integrity_both_arms.py` gains `package_verdict`/`package_exit` per unit and a `pkg_stop_rate_low`/`_high` BAND plus `pkg_disagreement` per cell, stated **beside** its published row-selected figures so the `closed_at` sort is visible as a CHOICE. **BOTH PROVEN ADDITIVE, not asserted — pre- and post-patch modules loaded side by side over one 1000-row journal pull, tol 0.001, compared key-by-key: the counterfactual is BYTE-IDENTICAL (0/0/0) and the integrity module adds 356 keys with 0 changed and 0 removed.** ⚠️ **Byte-identical cuts both ways and the memo says so**: the duplicate had not yet DRIFTED, so this removes a FUTURE hazard and fixes no PRESENT disagreement. **CROSS-CHECK: the four new cell bands and disagreement counts reproduce U21's restatement exactly through a different code path.** ⚠️ **It materially narrows `BL-20260912-U15S-OWN-CODE-NO-LONGER-REPRODUCES-U15S-OWN-PUBLISHED-TABLE-ONE-CONTROL-PACKAGE-FLIPPED-AND-NOBODY-WOULD-HAVE-KNOWN`: the published `31` is EXACTLY control_pre's band-high (31/62), so it is reachable with NO data change** — 28 unanimous + all 3 contradictions, against today's 30 = 28 + 2 of 3. A re-stamped price is no longer REQUIRED, is not eliminated, and the search is 3 named packages rather than 62. **Two further hazards were measured and found ZERO** (string-sort vs numeric-sort disagreement; siblings straddling the era split) — latent, not filed as findings. ⚠️ **HELD, `landing: hold` / `depends_on_unmerged_pr` on PR #12103**, which ships the module; against `main` it would import a file that is not there, established by the first attempt failing at import. **Changes no published number; nothing enacted; no `src/`, no config.** |
| `docs/research/m20-u14-the-bleed-record-is-wrong-in-three-places-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u14:self-measured-2026-09-12 / population-stated / positive-control-on-every-negative` | MI-278 U14: discharges `BL-20260911-THE-BLEED-PREDATES-E35-BY-THREE-DAYS-AND-THE-BRIEFED-08-30-ONSET-IS-WRONG`, `BL-20260911-THE-PROFITABLE-FORTNIGHT-BASELINE-RESTS-ON-A-WEEK-WITH-ZERO-MEASURED-ROWS` (clause **(b)**) and `BL-20260912-MI-271-S-HEADLINE-DECOMPOSITION-TABLE-INCLUDES-THE-PAIRS-SLEEVE-EVERY-OTHER-SECTION-EXCLUDES`. **Population: `/api/diag/journal?table=trades&limit=1000` pulled 2026-09-12T16:0xZ (`git_sha 855c7cc8d`), ids 4728–5727; `bybit_1` + `status=closed` + `NOT is_backtest` + `pnl NOT NULL` = 462 rows, bucketed on `closed_at`; §4 reproduces over ids 4701–5700 = 441 of the memo's 442.** **The onset is 2026-08-27, not 2026-08-28 — 17 consecutive losing days to 09-12, −$38,851.81 — and it is bounded from BELOW by data (2026-08-26 closed +$4,172.64), so it is not a tail artifact; the fix widens the gap to the e35 deploy `2026-08-30T08:53:19Z` (`892c9a2c`) from two days to THREE.** **The '+$17,951 profitable fortnight' is WITHDRAWN as ungradeable, not restated**: pre-period broker-truth coverage **3.4%** (5/149) vs the streak's **20.4%** (64/313), and on the memo's own reproducer `pre.pnl_sum_measured_only` = **−$4,178.35** — which does NOT establish it was unprofitable (n=13), which is why the disposition is ungradeable. Clause (a) is **established unreachable, with a positive control**: `/api/diag/journal` has no `offset` and caps at 1000; `/api/bot/db/table/trades` does and **401s through the relay** (#12011) while `/api/bot/performance` and `/api/diag/version` both return 200 in the same run (#12012). ⚠️ **`bleed-attribution-2026-09-11.md` §4.3 computes its headline table with the pairs sleeve INCLUDED (264 of 441 rows, 59.9%) while every other section excludes it**; on the declared filter the win-rate fall is **19.0–21.2pp, not 4.9pp** (three measurements, two split conventions, corroborated independently by MI-279's `winner-size-collapse-2026-09-12.md` at −19.5pp), and average loss **improves** 39% rather than worsening 20%. **The positive claim SURVIVES every cell — average win −71% to −74% — so PROPOSAL A and §5 of that memo are untouched; only the negative claim inverts.** ⚠️ **`CLAUDE.md` was RIGHT and is not the register to reconcile down** (its 16 days from 2026-08-27 checks out: 16 days, all losing, −$36,274.67 over its own window, the $722 residual being one day of re-pricing) — so this was two wrong registers agreeing while a third held the answer, filed as its own row. ⚠️ **And §4.3 is the one table its own reproducer does not compute**, which is the mechanism behind the drift and why the original cells reproduce on no tested split. **Nothing is enacted: no `src/`, no config, no order path, and the attribution itself is untouched.** |
| `docs/research/m20-u10-venue-mechanism-recovery-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u10:self-measured-2026-09-12 / population-stated / matcher-imported-not-rebuilt` | MI-278 U10: settles the question that must precede the Tier-2 remedy named in `BL-20260912-FOR-55-PERCENT-OF-WINNERS-THAT-MISSED-TARGET-THE-MECHANISM-THAT-STOPPED-THE-RUN-IS-NOT-IN-THE-JOURNAL` — **does the venue hold the mechanism at all?** **Population: the 17 winners in the three U2 classes that name no mechanism (`unattributable_price` 9, `contaminated` 7, `unattributable_level` 1), re-derived at `--tolerance 0.05` over a 50-winner re-run of the same classifier, all `bybit_1` PAPER on four symbols, closes 2026-08-27T08:04:59Z→2026-09-09T15:38:36Z, against `/api/diag/bybit_raw_closed_pnl` 8 windows / 121 records + `/api/diag/bybit_raw_order_history` 8 windows / 235 orders, 0 truncated, 0 errored.** **IT DOES: the venue answers for 11 of 17 (64.7%)** — `named` 10, `plain_order` 1, `no_corroborated_match` 6, and **`absent_from_order_history` ZERO**, the load-bearing count: the failure is never the order history, all six unanswered rows fail earlier at the closed-PnL join, which a close-time writer would not be doing at all. **7 of the 11 are stop fires** (6 `PartialStopLoss`, 1 `StopLoss`) against 3 take-profit fires. ⚠️ **Five of the eleven — every one a stop fire — are LOSSES at the venue while the journal calls them winners: journal +$358.94 against venue −$2,095.68** (trades 5515, 5437, 5442, 5610, 5554), sitting inside the population MI-277 and U2 measure winner SIZE over; 6 of 11 were booked 52–249 min after the journal closed the row. ⚠️ **This does NOT re-grade U2's headline** — a stop fire on a row the venue books as a loss is a MISCLASSIFIED LOSS, not a lever cutting a winner short. `cancel_type: UNKNOWN` on all ten named rows is CORRECT (every one `Filled`), not a gap. Matcher **imported** from U9's `netting_close_venue_adjudication`, never rebuilt. The Tier-2 persistence change is proposed in § 5 and **not taken**. |
| `docs/research/m20-u11-e35-controls-that-can-report-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u11:self-measured-2026-09-12 / population-stated / treated-set-diffed-from-the-ship-commit` | MI-278 U11: discharges `BL-20260911-BOTH-OF-THE-E35-EXPERIMENTS-DESIGNED-CONTROLS-HAVE-ZERO-OBSERVATIONS` by **correcting its premise rather than taking either exit it offered**. **Population: 55 legs / 11 accounts at `origin/main` `01aa8d367`; the e35 treated set diffed out of the ship commit `892c9a2c8` itself (9 legs, 10 field edits) rather than off the inline comments; family from the e35 sweep's own corpus (41 of 55); `/api/diag/journal?table=trades&limit=1000` read 2026-09-12, 1000 rows all non-backtest, split on `created_at` at the 2026-08-30T08:53Z deploy.** **NOT exit (a):** a within-family control that CAN report exists and has traded throughout — **`trend_donchian_eth`**, same family/symbol/timeframe and the same held `atr_stop_mult` 2.5 as the declared control `trend_donchian_eth_prop`, differing only in routing to `bybit_1` instead of the prop bridge (**8 closed after the deploy against the declared control's 0 ever**); `trend_donchian_sol` likewise. Both verified HELD across the boundary — only two commits touched the file since, and the other is runtime-identical. That also refutes the row's *"no way to remove [the symbol×timeframe confound] from this window"*: the controls give **crossing** within-symbol and within-timeframe comparisons. **NOT exit (b) either, and that is the finding:** the sample is already sufficient (23 v 11 detects 27 points; MI-275's 55-point effect needs ≥17/≥8), but **16 of the 23 treated closes do not record what ended them**, collapsing the usable arms to 7 and 7 and the resolution to **71 points** — ⚠️ and that subset must not be quoted as a rate, since an unlabelled stop is exactly what a `reconciler_filled` close looks like. **The binding constraint moved from the control to the outcome**, which is `BL-20260912-FOR-55-PERCENT-OF-WINNERS-THAT-MISSED-TARGET-THE-MECHANISM-THAT-STOPPED-THE-RUN-IS-NOT-IN-THE-JOURNAL`'s already-written Tier-2 remedy. ⚠️ **Third structurally-blocked arm the row lacks: `avax_pullback_2h` is `execution: shadow`**, so the AVAX dose comparison is permanently unavailable rather than missing from this window — **2 of e35's 9 treated legs are shadow legs that cannot act on their own treatment**. No config change proposed; routing the widened leg live is Tier-3 and explicitly not proposed. |
| `docs/research/m20-u2-winner-close-attribution-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u2:self-measured-2026-09-12 / population-stated / sensitivity-published` | MI-278 U2: **which mechanism actually ENDED each winning trade**, provenance-graded. Population: journal pull 2026-09-12T05:2xZ, MI-277's `population()` restated, `created_at >= 2026-08-27` = **183 closes / 49 winners**; risk on the ENTRY-FROZEN `order_packages.sl`; positive control `{measured 99, estimated 84}`. **NO EXIT LEVER IS CUTTING WINNERS SHORT** — the whole lever family is **3 of 49**, and the two `giveback_stop` fires banked **+$2,471.70**, the largest lever contribution in the window. **THE HEADLINE IS THE GAP:** of the 31 winners that fell short of ≥98% of their declared `tp_r`, **17 (55%) carry an unattributable or contaminated label** — the journal does not record what stopped the run. Five classes, never pooled (`attributed` / `account_level` / `unattributable_price` = an ESTIMATED anchor, *we could not look* / `unattributable_level` = a broker-truth fill near no level, a real negative / `contaminated` = `netting_attributed`, `OI-20260908`'s live-defect population). ⚠️ **THE UNATTRIBUTABLE SHARE IS INVARIANT to the label-recovery tolerance** across 0.000–0.250R (`unattributable_price` 9, `contaminated` 7 never move); only `attributed` shifts, **23→27 of 49 (47–55%)**, so it is quoted as a range. 18 of 49 ended at their declared target; the 6 stop-family exits banked a median **12–13%** of target. ⚠️ **MI-277 §2.2 IS NOT DECISION-GRADE:** its *achieved R nearly doubled* ratio swings **1.18–1.98** on the tolerance at n≈10 — reported as ungradeable, NOT as a refutation (the first draft said *the doubling disappears* and that was wrong). ⚠️ **MI-277 §3.3's `cap_r` figures could NOT be reproduced** (16.7%/0.0% vs my 55%/38%), so that table is **not adjudicated**; separately, against the DECLARED `tp_r`, `bybit_1` winners reached ≥98% of target 43% pre → 32% post (median achieved/declared 0.848 → 0.328). **Changes U3:** the briefed counterfactual was on *the mechanism U2 implicates*, and U2 implicates none — so U3 tests target geometry and the trailing stop, at the `exit-refinement` gate. All Tier-3 items are PROPOSALS; nothing enacted. |
| `docs/research/m20-u2b-risk-denominator-ratchet-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-278-u2b:self-measured-2026-09-12 / population-stated / control-asserted` |  MI-278 U2b: **`order_packages.sl` is NOT entry-frozen — the break-even ratchet amends it in place, and MI-277's unattributed `0.0015` clamp IS that ratchet** (`be_offset_bps: 15` = 15bp = 0.00150000 exactly). MEASURED over 299 `ict_scalp` packages: the declared `tp_at_r: 1.5` reproduces as a median ratio of **exactly 1.500** on all 8 legs while the max reaches 36.5, and **65 of 65** rows in that tail sit at `risk/entry == 0.00150000` (vs **1 of 234** off it), with **93%** of their linked closed trades winners (vs 9%). ⚠️ **THE CONSEQUENCE IS ASYMMETRIC:** the ratchet fires only at 1R, so it shrinks the denominator **selectively on winners and never on losers** — exactly the asymmetry a winner-vs-loser R comparison rests on. The entry risk is **exactly recoverable** from the frozen `tp`, ASSERTED not claimed: the recovery reproduces ` · entry−sl · ` on **231 of 233** un-ratcheted rows (median rel err **1.38e-10**) and the script FAILS if that control does not hold. ⚠️ **IT DOES NOT OVERTURN MI-277** — on its own population the winner-R collapse goes **−63% → −49%**, still large, and the loser rows are **bit-identical** under both bases (the control). But the LEVELS move: its pre-era winner median R **3.298 → 1.445**, inflated 2.3×. ⚠️ On the `ict_scalp` family alone (a DIFFERENT population, n=27 post) the repaired median is flat (+11%) — **reported, explicitly not the headline**, because carrying it forward would repeat the population error this workstream has now paid for three times. §3.3: mean R falls while median does not, so **the big winners stopped happening rather than the typical winner shrinking** — a different diagnosis, pointing a remedy at the right tail. Supersedes the R figures in U2 §4/§5 (its ATTRIBUTION is unaffected). All Tier-3 items are PROPOSALS; nothing enacted. |
| `docs/research/winner-size-collapse-2026-09-12.md` | evidence | live | — | 2026-09-12 | `mi-277:self-measured-2026-09-12 / population-stated` | MI-277: WHY the average win fell — the decomposition MI-271 left open. Identity `mean(pnl for winners) = mean(risk_usd)·mean(R) + cov`, risk taken from the **ENTRY-FROZEN** `order_packages.sl` (`trades.stop_loss` is the TRAILED stop; in a denominator it moved a mean R from 7.3 to **102.6** — a RECURRENCE of the trap that withdrew `BL-20260908-SL-CROSS-LABELS-NINE-CLOSES-THAT-EXITED-IN-PROFIT`). **THE R TERM IS 97.4% OF THE FALL.** Every in-system mechanism tests NEGATIVE: **size did not fall** — median risk over the FULL book ROSE $303.60 → $438.10 (the winners-only halving is a selection effect); the **venue clamp truncated 16.7% of pre-era winners and 0.0% of post-era ones** with `cap_r` unmoved at 66.0; **no exit lever** (0 packages declare a trailing stop or rung, `stale_stop`/`giveback_stop` absent); **composition explains ~1–7%** against ~75% from the legs, all 7 common legs falling (exact p=0.0156). The **loser side is the control**: median R −0.891 → −0.939, unchanged, so sizer and stops are intact. **PRICE-FREE and therefore artifact-proof:** winner hold time **5.17h → 1.90h** while loser hold time ROSE, win rate 0.537 → 0.256 pooled. Volatility **ROSE 29%** — more movement, less follow-through, corroborating MI-275 on a different leg family. ⚠️ **TWO CORRECTIONS TO MI-271:** its §4.3 table is the ONLY one computed with the **pairs sleeve INCLUDED**; excluded, win rate falls **19.5pp not 4.9pp** and avg loss **improves** 36%, so *"not a win-rate collapse"* does not survive its own population (the −72% RATIO is robust). ⚠️ **MAGNITUDE NOT ESTABLISHED:** −38.5% on broker-measured rows, −81.4% standardised, and on measured trades reaching a declared target achieved R **ROSE**; binding constraint n=15 pre-era measured winners. The largest share sits in `netting_attributed`, **100% ESTIMATED across 39 rows** — `OI-20260908`'s own population, so that doubt is inherited and stated. A **0.0015 stop-distance floor** binds 27% of scalp rows and its source was **NOT found in `src/`**. All Tier-3 items are PROPOSALS; nothing enacted. |
