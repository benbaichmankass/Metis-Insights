...
| `docs/research/m20-u23-tier3-proposal-target-geometry-2026-09-12.md` | evidence | unknown | — | never | `mi-278-u23:self-measured-2026-09-12 / population-stated / harness-shown-able-to-fail / refuses-to-satisfy-its-own-done-condition` | MI-278 U23: **the object's `done_condition` deliverable — a Tier-3 proposal the operator can accept or reject.** **IT PROPOSES CHANGING NO TARGET PARAMETER**, and asks instead for two named research arms. **Why: MI-278 U2 established the brief's premise is false — NO EXIT LEVER IS CUTTING WINNERS SHORT** (the lever family is 3 of 49 winners over 183 closes since 2026-08-27, `giveback_stop` is armed on 1 of 44 legs, and its two fires BANKED +$2,471.70), so the only lever with attributed mass is the TAKE-PROFIT (18 of 49 winners ended exactly there). ⚠️ **AND THE M20 WALK-FORWARD EVIDENCE STRUCTURALLY CANNOT GRADE IT** (population: 246 arms, 33 legs, 3 families, `m20-fold-dispersion-arms-consolidated.jsonl`): `tp_geometry` is **PERFECTLY CONFOUNDED** with `family` AND `block_unit` — `live_parity_uncapped` ⟺ scalp ⟺ per_leg, `live_parity_capped` ⟺ {donchian, pullback} ⟺ family_pooled — and **NO family carries both geometries**, so the within-family contrast does not exist. **The seductive 75%-vs-30% candidate-rate gap IS the scalp-vs-others gap: the same 28 arms.** There is **no dose** either (all 246 arms carry `tp_cap_pct == 0.099`, the venue clamp), and a **third confound** rides along (`candidate` arms have median 23 folds / 288 OOS against 16 / 222). **THE ASK: Arm A, a within-family geometry contrast; Arm B, a cap dose — with the acceptance criterion stated IN ADVANCE so it cannot be chosen afterwards.** ⚠️ **IT EXPLICITLY DOES NOT SATISFY THE `done_condition` IT SERVES** and says so: that condition wants a lever with IS/OOS + walk-forward behind it, and the finding is that the walk-forward cannot support the lever — so **the object stays OPEN**. ⚠️ **It also WITHDRAWS a claim its own author repeated in several board posts and PR bodies this session** — *'three decision requests on the work object are unanswered'* — the object file contains the string `decision` **zero** times. **Nothing enacted; no `config/`, no `src/`, no parameter proposed.** |
