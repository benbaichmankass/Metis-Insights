...
| `enable-mobile-push` | 2 | `scripts/ops/enable_mobile_push.sh` | `.env` (`MOBILE_PUSH_ENABLED=1`) + restart `ict-trader-live.service` — activates the M12 S1 mobile push notifier so `Database.update_trade(status='closed', ...)` fans out to registered Android devices via FCM. Prereqs: `FCM_SERVICE_ACCOUNT_JSON` set in the same `.env`, at least one device registered via `POST /api/bot/devices/register`, `google-auth` installed. Failing any prereq leaves the notifier inert + a WARNING log line — no trader impact. |
| `disable-mobile-push` | 2 | `scripts/ops/disable_mobile_push.sh` | `.env` (`MOBILE_PUSH_ENABLED=0`) + restart `ict-trader-live.service` — silences mobile push fan-out. Router at `/api/bot/devices/*` remains available for register/revoke. |
...
| `pull-mes-ibkr-history-daily` | 2 | `scripts/ops/pull_mes_ibkr_history.sh` | Same wrapper + the same live-gateway guards as `pull-mes-ibkr-history`, but baked to a **DAILY multi-year** pull (`MES_TIMEFRAMES=1d`, `MES_HIST_START=2019-05-06` ≈ MES inception, `DATASET_VERSION=v002-daily` so it never collides with the intraday v002 shards) → `/data/bot-data/ibkr_datasets/market_raw/MES/1d/v002-daily/data.jsonl`. The `ibkr_offvm` adapter stitches dated MES expiries for depth (~1,750 daily bars). Added 2026-06-01 to validate `mes_trend_long_1d` (the execution:shadow daily long-only diversifier) on **native MES** rather than the SPX-CFD proxy before any shadow→live. |
