...
> **Doc status:** `live` · category `unknown` · last verified `2026-09-25` · registered in [`docs/DOCUMENT-INDEX.md`](../../docs/DOCUMENT-INDEX.md)
...
| `mark-operator-flattened` | 2 | `scripts/ops/mark_operator_flattened_action.sh` | `UPDATE trades SET exit_reason='operator_flatten_reconciled', notes=?` for the trade ids named in the issue's `trade_id:` line (digits, comma-separated). **Marks a close as OPERATIONAL so exit analyses can exclude it.** The flatten scripts deliberately leave the journal row to the reconciler's close-on-disappear, which stamps `reconciler_filled` — a correct description of HOW the row closed and a misleading one about WHY, leaving it byte-indistinguishable from an ordinary close the reconciler happened to book. `perExitPath` then buckets it with genuine strategy exits and the exit-refinement corpus reads its entry→exit geometry as evidence about the strategy's exit quality, when a human chose the exit time for a reason unrelated to the market. `close-stranded-journal-row` already owns the operator-flatten convention but hard-filters `WHERE status='open'` — it is the tool for when the reconciler FAILS; this is the missing half for when it SUCCEEDS. ⚠️ **Targets are NAMED, never derived** — no column carries "an operator flattened this", which is the bug itself, so a filter-based version would invent the signal it exists to record. Touches **no** monetary field and **not** `notes.exit_price_source`: the PRICE's provenance and the CLOSE's CAUSE are different questions, and stamping `operator_flatten_fill` over a `bybit_closed_pnl` row is exactly the clobber `BL-20260824-RECORDED-EXIT-PRICE-OUTNUMBERS-ALL-BROKER-TRUTH-COMBINED` records. Records the prior label under `notes.pre_mark_exit_reason`, so it is reversible per row. REFUSES (writing nothing at all) if any id is missing, a backtest row, or not `status='closed'`. Wrapper runs `--self-test` (12 planted controls, two non-vacuity controls verified) as a PRECONDITION, then a before/after table around a dry-run and `--apply`. Idempotent (a row carrying `notes.closed_by_operator` is skipped). No service touched. Added 2026-08-30 after the bybit_2 hedge-switch flatten of trades 4904/4934 landed as `reconciler_filled`. |
...
| `pull-bybit-transaction-log` | 2 | `scripts/ops/pull_bybit_transaction_log_action.sh` | Pulls Bybit's **own wallet ledger** (`/v5/account/transaction-log`) for every live Bybit account over a `days:`-selectable window (**default 7**) into the venue-truth store (`runtime_state/exchange_fills.sqlite :: bybit_transaction_log`), feeding `/api/diag/bybit_wallet_truth`. **This is the live replacement for a hand-pasted CSV.** `comms/broker_truth_ledger.json` — the authoritative real-money figure for `bybit_2` — is populated BY HAND from an operator's UM export, so it froze on 2026-07-13 while the account kept trading, leaving 48 days and 59 closed real-money trades unreconciled and the journal under-recording by **11.0x** (-$23.77 against -$262.52 of wallet truth) (`BL-20260830-BROKER-TRUTH-LEDGER-STALE-59-REAL-MONEY-CLOSES-UNRECONCILED`). **WHY THE ACTION EXISTS BESIDE THE HOURLY TIMER:** the timer runs `--days 7`, which can never close a HISTORICAL gap; only this path can ask for the deep `days:` window a backfill needs. ⚠️ **A DEEP WINDOW IS WALKED, NOT ASKED FOR IN ONE CALL** — Bybit V5 caps the queryable RANGE at 7 days while retaining far more, so `startTime = now-60d` returns the 7-day slice at the START of the range: the window MOVES rather than widens and the run reports success over almost nothing. The fills puller already paid for this exact bug (`BL-20260808-FILLS-WINDOW-TOO-SHORT-TO-REPAIR-HISTORY`), and it is the open UNVERIFIED exposure in `pull-exchange-funding`'s row above (`BL-20260808-FUNDING-PULLER-SAME-RANGE-CAP-EXPOSURE`); this puller walks `<= MAX_RANGE_DAYS` chunks, so it is correct whether or not the cap applies to this endpoint. Asserted by `tests/test_pull_bybit_transaction_log.py` with a CONTROL proving a single call would have returned only a slice. **COST:** `ceil(days/7)` requests per account plus pagination — `days: 60` is ~9 chunks x 3 accounts; keep deep pulls deliberate. ⚠️ **Wallet truth is the sum of `change`, NOT `cashFlow`** (which excludes fees), EXCLUDING `TRANSFER_IN`/`TRANSFER_OUT` — dropping the transfer exclusion makes `bybit_2` read **+$4,737.48** instead of -$262.52, a $5,000 sign flip, and fails 3 tests. Read-only on the exchange side; idempotent (the store keys on the venue's own row id, so overlapping windows insert nothing and a re-run cannot move an account-level P&L figure); touches NO service and NO `trade_journal.db` table. States are never collapsed: `not_pulled` / `no_rows_in_window` / `unreadable` / `measured_api`, and `realized_usd` is `None` — never `0.0` — unless measured. Added 2026-08-31. |
...
| ~~`enable-mes` / `disable-mes`~~ | — | *removed 2026-05-22* | **Deleted — these were a forbidden second gate.** The `MULTI_SYMBOL_ENABLED` env they flipped no longer exists; the tick's fetch set is derived from config (`_resolve_tick_symbols` unions each configured account's **roster-implied** symbols with its declared `symbols:` — updated 2026-09-22, E42). Per the "one switch per account" rule, the only way to gate MES is the account's `mode:` (via `set-account-mode` on `ib_paper` → stops execution, signals still log) or removing it from the account's `strategies:` roster in a PR. ⚠️ **Removing a symbol from `symbols:` is NOT a way to gate anything** — that list is an additive data-pull list and the roster decides what trades. |
...
| `send-ping` | 1 | `scripts/ops/send_ping_action.sh` | **No mutation, no restart.** Enqueues one immediate Telegram message via `scripts/send_ping.py` (`target=claude` default → @claude_ict_comms_bot; the bridge drains within ~5 s). This is the autonomous "Claude wants to say something NOW" path — far faster than the ≤5-min `pending-pings.jsonl` git-relay. Params: `message:` (required), `priority:` (low\|normal\|high\|urgent, default normal), `target:` (claude\|trader, default claude), and — added 2026-09-01 — `kind:` (decision\|state_change\|lifecycle), `why:`, `unproven:`. **`kind:` blank is the DEFAULT and is the passthrough shape**, which sends your text unaltered; setting it renders Format B via `src/runtime/claude_ping.py` and applies that class's rate limit. ⚠️ Passthrough is not a legacy mode — it carries the OPERATOR's own words, and Format B is a house style for MACHINE-generated events. `why:` is REQUIRED whenever `kind:` is set (Format B's second line is the point) and the workflow refuses the dispatch without it; `why:`/`unproven:` WITHOUT a `kind:` is also refused, because the producer only reads them on the Format-B path and would otherwise accept and silently drop them. ⚠️ A `lifecycle` ping inside its 300 s window is **withheld**: `send_ping.py` exits 0, nothing is queued, and the audit records `status: withheld` with the limiter's reason — do not read that run's success as a delivered ping. Before 2026-09-01 the wrapper passed no `--kind` at all, so the three classes were implemented and unreachable from this action. The transparency notify is skipped for it (the action IS the message). Added 2026-05-24. |
...
| `unset-env` | 2 | `scripts/ops/unset_env.sh` | The DELETE half `set-env` cannot provide (its empty-value guard refuses to blank a key). Removes a key's `KEY=` (or `export KEY=`) line(s) from an env file and restarts `service:` — but only for a key on the fixed `RETIRABLE_KEYS` allowlist inside the script (starts with `ARBITRATION_FANOUT_MODE` / `ARBITRATION_FANOUT_ACCOUNTS`, retired by E35 / #12924), never a freeform key. Backs up the file first (mode 600, timestamped, same convention as `scrub-env-noncompliant`); every other line is preserved byte-for-byte; an absent key is a clean no-op (no backup, no restart). Params: `env_key:` (required, must be allowlisted), `service:` (default `ict-trader-live`, or `none` to skip restart), `env_file:` (optional — `shared` (default) | `web-api`, same targets as `set-env`). |
...
| `switch-bybit-position-mode` | 2 | `scripts/ops/bybit_switch_position_mode_action.sh` | Read — and with `apply: true`, **SWITCH** — one Bybit symbol's **VENUE position mode** (one-way netting ⇄ hedge). Body params: `account: <id>` + `symbol: <SYM>` (both required); apply additionally requires `position_mode: one_way|hedge` AND `confirm_account: <id>` echoing `account`. Default **report-only**. The venue half of **T.2** (`BL-20260821-PAIRS-SOL-ETH-STRANDS-ON-EVERY-OPEN`): `src/runtime/bybit_position_mode.py` made the CODE hedge-capable and deliberately stopped short of the venue, and measured 2026-08-30 `switch_position_mode` had **zero** call sites anywhere in `src/` or `scripts/` — so nothing performed this step. Under one-way netting a market-neutral pairs leg opposite a concurrent directional position does not open a book, it REDUCES the standing one: **8 of 8** SOL/ETH pairs stranded since 2026-08-18, arithmetic closing exactly. ⚠️ **THE MODE IS READ FROM `positionIdx`, NOT AN ACCOUNT SETTING** — one-way returns a single row at `positionIdx 0`, hedge returns two at `1`/`2`, so the SET of indices IS the mode and it stays readable while the symbol is FLAT. That matters because a symbol must BE flat to switch, i.e. this reads exactly the state `BL-20260826-BYBIT-POSITION-MODE-UNREADABLE-FOR-A-FLAT-SYMBOL` records as unreadable on the existing surfaces (both skip `size <= 0`). ⚠️ **THE BYBIT ENUM IS VERIFIED, NOT ASSERTED** — v5 documents `mode` 0 = Merged Single, **3 = Both Sides** (3, not the obvious wrong guess of 1). Rather than trust that, apply **re-reads** afterwards and reports `switch_verified` only when the observed `positionIdx` set actually changed, so a silent venue no-op exits **5** (`notify_run.sh` routes that to `urgent` with *do NOT arm the allowlist*) instead of reporting success. ⚠️ **ORDER OF OPERATIONS IS LOAD-BEARING AND NEITHER ORDERING IS SAFE WHILE THE TRADER RUNS**: a hedge venue with an empty `BYBIT_HEDGE_MODE_SYMBOLS` sends no `positionIdx` (or `0`) and Bybit refuses; a one-way venue with an armed allowlist sends `1`/`2` and Bybit refuses that too — refusing **entries AND protective SL/TP re-arms** on the symbol. Run it with the trader STOPPED (`stop-bot-service` → this → `set-env` → `start-bot-service`), which converts *orders get refused* into *no orders are attempted*. **Four refusals before any mutation**: symbol not flat; resting orders present; mode unreadable (`error`/`absent`/`ambiguous` — never read as one-way); `confirm_account` not echoing `account`. Already-in-mode is a clean no-op. Per-SYMBOL, because Bybit's position mode is per-symbol. Wraps `scripts/ops/bybit_switch_position_mode.py`. |
...
| `reattach-stranded-package-legs` | 2 | `scripts/ops/reattach_stranded_package_legs_action.sh` | **Audit F-39 / MI-231.** Re-attaches the three `order_packages` rows that were flipped to `closed` while their legs were still OPEN. `order_monitor` drives exits per package and selects `get_order_packages_by_strategy(strategy, status="open")`, so a package closed under an open leg can never be selected again and **that leg is managed by nobody** — no trail, no tighten, no exit. Targets: `pkg-32a5162bbbdd4ef6` (SPY -> leg 4347, `alpaca_paper`), `pkg-785d9a600c894c81` (TLT -> 5265, `alpaca_paper`), `pkg-293021e2e84a48db` (XRPUSDT -> **5474 `bybit_2` REAL MONEY** + 5475 `bybit_portfolio`). The diff is `status` closed->open and `close_reason`->NULL, with both prior values preserved under `meta.reattach_repair`. Body param: optional `apply: true` (default **dry-run**). ⚠️ **IT PLACES, MODIFIES AND CANCELS NOTHING.** The operator's approval (`WO-20260909-DECISION-PACKAGES-CLOSED-WHILE-HOLDING-OPEN-LEGS` `chosen: detector_and_reattach` for the paper pair; `DEC-20260910-F39-REATTACH-XRPUSDT-APPROVED`, 2026-09-10T07:52Z, for the XRPUSDT half) is scoped **in terms** to a JOURNAL/STATE repair and authorises no order action on any account. The standing prohibition on remediating by cancelling resting legs (`BL-20260820-OVERCOVER-REMEDIATION-CANCELLED-THE-JOURNAL-MATCHING-LEG`) applies without exception. ⚠️ **THE INTENDED CONSEQUENCE IS THAT THE EXIT PATH CAN ACT ON THESE LEGS AGAIN, INCLUDING REAL-MONEY 5474** — once the package is `open` the strategy's own `monitor()` may trail, tighten or CLOSE it on the next exit pass. That is the point (they are unmanaged today), it was stated verbatim in the option text the operator chose, and it is recorded here rather than buried. ⚠️ **VENUE EVIDENCE IS MANDATORY AND IS CAPTURED FRESH INSIDE THE RUN.** The wrapper GETs `/api/diag/exchange_positions` over loopback seconds before the write, because the approval's condition is a live re-read *immediately before* `--apply`; a payload passed in through the issue body would be stale by construction. **A failed capture ABORTS** — no `|| echo '{}'` — since an empty payload would read as *every account blind* and produce a refusal that looks like a considered decision instead of a plumbing fault. ⚠️ **A leg FLAT at the venue is REFUSED, not re-attached** (it must be reconciled as CLOSED), and an **unreadable account is a REFUSAL, never a flat** — *we could not look* is not evidence the position is gone. Those refusals are the CORRECT outcome, not obstacles to route around. Every target also pins its expected CURRENT signature (status, close_reason, linked_trade_id, and the exact open legs it must still hold), so a row that has moved is refused and named: the action is idempotent and safe to re-run. ⚠️ **RE-ATTACHING FIXES THE PACKAGE, NOT THE SIBLING GAP** — the loop still manages only `linked_trade_id`, so 5475 stays unmanaged in fact even once its package is open (`BL-20260818-MONITOR-MANAGES-ONLY-THE-LINKED-LEG`, a Tier-3 order-path repair this does not attempt and must not be read as having fixed). Verify the post-state by reading `order_packages` back off `/api/diag/journal` and the legs off `/api/bot/positions` — **never from the run's own output**. No service restart. Wraps `scripts/ops/reattach_stranded_package_legs.py`. |
...
`unset-env` is the DELETE half `set-env` cannot provide by design — its
empty-value guard (2026-08-01 `TELEGRAM_BOT_TOKEN` incident) correctly
refuses to blank a key, which also means it cannot retire one (issue
#12938). It removes a key's `KEY=` line(s) from an env file and restarts
`service:`, but ONLY for a key on the fixed `RETIRABLE_KEYS` allowlist
inside `scripts/ops/unset_env.sh` — never a freeform key, however
well-formed its charset — the same doctrine `get_env.py::ALLOWED_KEYS`
uses for the read half. Backs up the target file first (mode 600,
timestamped, same convention as `scrub-env-noncompliant`); an absent key
is a clean no-op (no backup, no rewrite, no restart). Params: `env_key:`
(required, must be on `RETIRABLE_KEYS`; starts with exactly
`ARBITRATION_FANOUT_MODE` / `ARBITRATION_FANOUT_ACCOUNTS`, retired by E35
/ #12924), `service:` (allowlisted unit, default `ict-trader-live`, or
`none` to skip restart), `env_file:` (optional — `shared` (default) |
`web-api`, same targets as `set-env`).
