...
_Generated 2026-09-21T01:36:03+00:00 · verdict **all_sources_read**_
...
- 🔔 **manager-queue-watch-never-ran-overdue** (manager_queue_watch · 18d) — armed 443.7h ago, on the order of 443 expected firings, and ZERO receipts have ever been committed. The Routine reports SUCCEEDED, so nothing else reads as wrong. This is NOT `it has not fired yet`. Only the Routine can clear it — a hand-written receipt would arm the freshness grading against a file nobody maintains and report a healthy watch forever. Decide: fire it and confirm it can write, fold the escalation into the manager tick (which HAS a persistent session with tools), or RECORD the decision to retire it together with what then grades the manager's inaction.
...
- **ERRFEED-a46d2970** (error_feed · 9d) — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 22 rows 2026-09-12T18:12:44 → 2026-09-20T21:12:15 · symbols=MHG strategies=mhg_pullback_1d — decide: fix now, or file to a backlog
...
- **ERRFEED-c8553cf1** (error_feed · 8d) — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 7 rows 2026-09-13T08:38:35 → 2026-09-20T09:59:28 — decide: fix now, or file to a backlog
...
- **ERRFEED-6da04340** (error_feed · 3d) — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 8 rows 2026-09-18T05:56:33 → 2026-09-20T06:06:30 · symbols=MGC strategies=mgc_pullback_1d — decide: fix now, or file to a backlog
...
- **ERROR-FEED-SUMMARY** (error_feed) — 50 further error group(s) and every warn group are NOT listed above — read `docs/claude/ERROR-FEED-DIGEST.json` for the full set. Digest verdict `all_feeds_read`, page cap hit on `bot_logs`, covers rows after `2026-09-20T22:11:37.158113+00:00`.
  - 93 cause groups over 1396 rows (60 error-level, 33 warn-level, 0 new since the last digest)
