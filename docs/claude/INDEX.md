...
- `../runbooks/health-check.md`: single-workflow health-snapshot pipeline (operator-driven manual review, 2026-05-12 cleanup). Daily cron `health-snapshot.yml` SSHes to the VM, runs `scripts/collect_health_snapshot.sh` + `scripts/run_pipeline_health_test.sh`, and uploads the snapshot as an Action artifact. Operator downloads the artifact and pastes into a Claude session for `/health-review`. No more Layer-1 LLM, no more `comms/requests/REQ-*.json`, no more auto-merging review PR, no more Telegram pings. Required secret: `VM_SSH_KEY`. Read before touching `.github/workflows/health-snapshot.yml`, `.claude/skills/health-review/SKILL.md`, or `scripts/collect_health_snapshot.sh`.
...
- `open-considerations.md`: design questions under evaluation — **not canonical, not directives.** Read before assuming any "we should remove X" idea is decided.
