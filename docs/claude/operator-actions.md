...
| `enable-mes` | 2 | `scripts/ops/set_multi_symbol.sh` | sets `MULTI_SYMBOL_ENABLED=true` + `SYMBOLS=BTCUSDT,MES` in the trader `.env` and restarts `ict-trader-live.service` so the per-symbol tick loop trades MES alongside BTCUSDT. Added with the Interactive Brokers integration (PR #1656/#1670). |
| `disable-mes` | 2 | `scripts/ops/disable_multi_symbol.sh` | sets `MULTI_SYMBOL_ENABLED=false` in the trader `.env` and restarts `ict-trader-live.service`, reverting to the single-symbol (BTCUSDT-only) path. Added with the Interactive Brokers integration (PR #1656/#1670). |
