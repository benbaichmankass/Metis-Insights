...
- Fix: grace window in `_reconcile_open_trades`. Trades whose `created_at` is newer than `RECONCILER_GRACE_SECONDS` (default **60 s**, matches heartbeat cadence) are skipped this tick and counted as `skipped_recent` in the summary. The grace window keeps an old, ripe ghost row eligible for orphan-stamping while shielding rows that are still racing the Bybit open-positions index. Knobs: `RECONCILER_GRACE_SECONDS=0` reverts to pre-fix behaviour for debugging; garbage values fall back to the 60 s default.
- Check: regression tests live in `tests/test_monitor_reconciler.py::TestReconcilerGraceWindow`. In production, grep `journalctl?unit=ict-trader-live` for `skipped_recent=` after a VWAP fire — non-zero on the tick following a dispatch, zero again on the next sweep once the position has indexed. False-positive orphans for rows with a numeric Bybit `trade_id` should be zero in fresh diag pulls.
- Lesson: when introducing a reconciler that compares "DB-open vs. exchange-open", always factor in exchange index lag. The DB write happens synchronously; the exchange's open-positions endpoint is eventually-consistent on the order side of seconds.
