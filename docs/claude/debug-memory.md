...
- **Session can't reach the VM directly** — neither the proxy allowlist (`141.145.193.91:8001` is firewalled) nor `dangerouslyDisableSandbox: true` (which only relaxes the *Bash* sandbox, not the platform firewall). Use the `vm-diag-snapshot` issue-driven relay; see `docs/claude/diag-relay.md`.
