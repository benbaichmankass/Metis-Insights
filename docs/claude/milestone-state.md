...
| **M5** | Strategy testing workflow | ✅ CLOSED | Closed 2026-05-10 across three PRs: P1 #637 (closed loop — `BacktestConsumer` bolted into `CommsPoller.poll_once`, validation log at `runtime_logs/validation.jsonl`, `cmd_test_strategy` registry guard); P2 #639 (hardening — subprocess execution via `run_backtest_m5`, `M5_BACKTEST_TIMEOUT_S` wall clock, single-flight lock, `M5_CONSUMER_ENABLED` env gate, structured error envelope); P3 (this PR — `docs/runbooks/strategy-testing.md` + close-out updates). Optional dashboard backtest-history surface (P4 — Tier-1 `/api/bot/backtests` + UI tab) carved out so the milestone could ship; queued separately. |
...
|---|---|---|---|
| 1 | M6 — Web app UI (dashboard repo) | auto-claude | **In active session 2026-05-08** in `benbaichmankass/ict-trader-dashboard`. |
| 2 | M5 P4 (optional) — backtest-history dashboard surface | auto-claude (cross-repo) | M5 P3 closes (this PR). Tier-1 `GET /api/bot/backtests` + dashboard tab. Carved out of M5 close so the milestone could ship; low priority. |
| 3 | M9 — AI / model roadmap | auto-claude | Independent of M6. Could run in parallel. |
| 4 | M10 — HF / data pipeline | auto-claude | Independent of M6. Could run in parallel. |
