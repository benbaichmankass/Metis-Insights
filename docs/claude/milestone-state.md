...
| **Active checkpoint** | ~~T0~~ DELETED in PR #455 (margin-agnostic correction). ~~T1~~ shipped 2026-05-07 (PR #456 operator-merged). ~~T2~~ shipped 2026-05-07 (PR #459 DRAFT — `feat(risk): spot-margin sizing — collateral, liquidation, borrow fees`, awaiting operator merge reply). **T3 queued** — `feat(exec): route spot-margin orders via isLeverage=1` + `feat(coordinator): direction-aware balance for spot-margin accounts` (D4 + D5 land together — one diff is incoherent without the other), gated on operator merge of #459. |
