...
| **M5** | Strategy testing workflow | ✅ CLOSED | Closed end-to-end 2026-05-10 across five PRs: P1 #637 (closed loop — `BacktestConsumer` bolted into `CommsPoller.poll_once`, validation log at `runtime_logs/validation.jsonl`, `cmd_test_strategy` registry guard); P2 #639 (hardening — subprocess execution via `run_backtest_m5`, `M5_BACKTEST_TIMEOUT_S` wall clock, single-flight lock, `M5_CONSUMER_ENABLED` env gate, structured error envelope); P3 #640 (`docs/runbooks/strategy-testing.md` + close-out doc updates); P4 #689 (Tier-1 `GET /api/bot/backtests` read endpoint, with `silent-empty-guard` allow-silent justifications); dashboard `#12` (Backtests tab consuming the endpoint). Operator runbook: `docs/runbooks/strategy-testing.md`. |
