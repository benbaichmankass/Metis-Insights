...
**M1 status update (S-048 closed 2026-05-07; operator-corrected post-audit):** audit verdict = 🔄 PARTIAL. No P0 surfaced. Operator clarified that the on-disk "ClaudeBot is one-way; intentional" design is correct; the drift is in the workplan, not the implementation. **Adjusted follow-up count: 4 P1 + 1 P2 cluster** (3 P1s dropped: relocate-to-ClaudeBot, merge/hold buttons, S-042 doc correction; 1 reframed as a workplan-correction sprint). Remaining P1s: workplan correction; stuck-request recovery alerts on the trader-bot comms-handler; auto-hourly snapshot timer; `/new-session` + `/test` operator commands. See `docs/audits/M1-comms-audit-followups.md` § "Operator-correction redlines (2026-05-07)". Per § 8 hand-off: next sprint after S-048 close = S-047 T3 (default; no P0 override).
...
| **M1** | Comms infrastructure | 🔄 PARTIAL (S-048 audited 2026-05-07; operator-corrected) | S-048 audited the on-disk telegram-bot implementation; `docs/audits/M1-comms-audit-2026-05-07.md` + `docs/audits/M1-comms-audit-followups.md` (with operator-correction redlines). Verdict: **🔄 PARTIAL**, no P0. Operator-control trio (`/halt`, `/closeall`, `/toggle`) and 7 of 8 info menus on `@bict_trading_bot` are present and tested. S-027's repo-driven request/response system is on the trader bot — and that's the **correct** placement under the operator-clarified architecture (ClaudeBot is intentionally one-way; operator decisions go through GitHub). **Adjusted follow-ups: 4 P1 + 1 P2 cluster** — workplan correction (remove the two-way 5-step language for ClaudeBot); stuck-request recovery alerts on the trader-bot comms-handler; auto-hourly snapshot timer; `/new-session` + `/test` operator commands. M1 stays open until the workplan correction + auto-hourly + recovery-alert + `/test` follow-ups land. |
