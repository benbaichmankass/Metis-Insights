...
ict-ib-executions-pull.service # oneshot, timer-driven (HOURLY :17) — IBKR executions pull into the same exchange-fills store, so an IB close resolves from CommissionReport.realizedPNL (MEASURED) instead of a reconstruction. Hourly rather than daily because IBKR's reqExecutions serves ~the current trading day AND order_monitor's broker-defer grace is 6h — a daily pull would look correct and be inert. In DEFAULT_SKIP so a deploy never opens an unscheduled gateway connection (S-PROVENANCE-IB-EXECUTIONS-2026-07-30)
