...
ict-exchange-funding-pull.service # oneshot, timer-driven (daily) — Bybit funding-fee pull so realised-PnL funding costs are captured on schedule; secondary-priority (#6901, BL-20260719-FUNDING-NO-TIMER)
ict-trainer-git-sync.service  # trainer-VM only — dedicated code-only git-sync timer; fixes the 495-commit trainer-VM drift by pulling code separately from the data sync (#6859)
```
