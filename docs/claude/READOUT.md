...
_Generated 2026-09-19T10:48:49+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
|---|---|---|---|---|
| 37 | 24.3% | -41.4891 | -41.4891 | **73.0%** |

Provenance split — measured 27 · estimated 10 · fabricated 0 · unverified 0.
...
- `WO-20260901-PHASE-C` (waiting) — 18d — Phase C — migration, the WIP ceiling, and the priority that reaches a session
- `WO-20260901-PHASE-D` (waiting) — 18d — Phase D — the constraint, computed rather than judged
- `WO-20260902-DECISION-REVIEW-PACKET-POPULATION` (waiting) — 17d — Which POPULATION does the M7 strategy-review gate decide on — real money, or real blended with paper?
- `WO-20260902-EVIDENCE-TRADE-PRIORITISATION-AB` (waiting) — 17d — Confidence-first is the LIVE ranking key for competing trades and has never been shown to pick the better trade — and the A/B that would settle it cannot be run
- `WO-20260903-CLOSE-WEDGE-LEDGER-ABSENT` (waiting) — 16d — The standing close-wedge ledger does not exist, and the digest that replaced the pager reads a non-canonical path for it -- so a downgraded wedge is in NEITHER…
- `WO-20260903-SUNSET-DISPOSITIONS-OWED` (waiting) — 16d — Ten evidenced retirement candidates have sat undispositioned since 2026-09-01, and retiring a leg is Tier-3 so only the operator can move them
- `WO-20260903-THE-REFUSAL-HALF-OF-MANAGER-CONTROL-BLOCK` (waiting) — 16d — The refusal half of manager control: block a manager action on a PR whose author session is still live
- `WO-20260904-MANAGER-IDLE-IS-UNBOUNDED-AND-NOTHING-WAKES-IT` (waiting) — 15d — a manager that stops acting is not detected by anything -- idle is unbounded, the lease expires into nobody, and the only thing that restarted work was the ope…
- `WO-20260905-PENDING-PINGS-HAS-NO-MERGE-DRIVER-SO-PING-PRS-DIE` (waiting) — 14d — docs/claude/pending-pings.jsonl is the ONE register with no row-aware merge driver, so every ping-carrying PR conflicts unresolvably and the only remaining pat…
...
**Completeness: `all_sources_read`** · **202 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `unlanded_automation` | `read` | 67 |  |
| `error_feed` | `read` | 11 | digest 2026-09-19T06:35:58+00:00, age 4.2h |
| `sunset` | `read` | 12 |  |
| `checklist_unrouted` | `read` | 1 | register_freshness=fresh |
| `stuck_branches` | `read` | 1 | 248 branch(es) standing |
| `settled_disposition_owed` | `read` | 0 | settled record grades settled_graded (117 settled row(s)) |
...
- 🔔 `OI-20260906-RESEARCH-THAT-SPECIFIES-WORK-IS-CARRIED-BY-NOTHING` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- `OI-20260826-MHG-OVER-COVER-MECHANISM-UNVERIFIED` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- `OI-20260826-SESSION-BRIEF-NEVER-READ-BY-A-FRESH-SESSION` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260826-STRAY-OCA-SWEEP-SHIPPED-BUT-UNARMED` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260826-MGC-JOURNAL-QTY-DIVERGENT-UNOWNED` · 6d — monitoring row 6d since last observation (cadence 2d)
...
- 🔔 `OI-20260903-CAPTURE-WATCH-PROVED-ITSELF-ON-DEMAND-AND-ITS-SCHEDULE-HAS-NEVER-SUCCEEDED` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260829-ALPACA-GOLIVE-BLOCKED-ON-T1-SETTLEMENT-MODEL` · 19d — loud row — must be reported on every session
...
- 🔔 `OI-20260830-E35-GEOMETRY-SHIPPED-TO-9-LEGS-NOT-YET-LIVE-VERIFIED` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260829-E35-REVERSED-LEGS-ARE-A-TIER-3-PROPOSAL-SET-NOT-APPLIED` · 19d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-PER-ACCOUNT-ARBITRATION-SHIPPED-NOT-YET-ARMED-OR-EXERCISED` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-LIVE-WALLET-TRUTH-CANNOT-REPRODUCE-THE-LEDGER-WINDOW` · 19d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` · 19d — monitoring row 19d since last observation (cadence 14d)
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` · 19d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` · 7d — monitoring row 7d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-SCOPE-OVERLAP-ATTRIBUTOR-MISREADS-A-SESSIONS-OWN-START` · 19d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LOCAL-LLM-VERDICT-PENDING-A-DIFFERENT-SIZE-ARM` · 18d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-READ-DEBT-11-UNREAD-AND-256-SUPERSEDED-UNREAD` · 7d — monitoring row 7d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-PROBE-READERS-SHIPPED-BUT-THE-ACTIONS-ONE-HAS-NEVER-RUN-FOR-REAL` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-ALPACA-LIVE-FIRST-REAL-MONEY-LEG-ROUTED-BUT-HAS-NEVER-TRADED` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-CYCLE-PRIORITY-IS-RENDERED-BUT-NO-SESSION-HAS-ACTED-ON-IT` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-CONSTRAINT-READOUT-SHIPPED-AND-IT-REFUSES-NOBODY-HAS-ACTED-ON-THE-REFUSAL` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-ALPACA-SHARE-HOLD-CLASSIFIER-SHIPPED-NOT-YET-OBSERVED` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-DECISION-ROUNDTRIP-SHIPPED-AND-NO-DECISION-HAS-EVER-MADE-THE-ROUND-TRIP` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-SUNSET-PASS-SHIPPED-AND-NOTHING-HAS-BEEN-RETIRED` · 13d — monitoring row 13d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-TELEGRAM-DECISION-ROUNDTRIP-SHIPPED-AND-NO-TAP-HAS-EVER-BEEN-MADE` · 13d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260902-DECISION-PROMPTS-MOVED-TO-CLAUDEBOT-AND-NO-TAP-HAS-LANDED-THERE` · 13d — loud row — must be reported on every session
...
- `OI-20260902-ROLE-PACKS-NOW-REACH-THE-OPERATING-LAYER-AND-NO-SESSION-HAS-BEEN-OBSERVED-USING-IT` · 7d — monitoring row 7d since last observation (cadence 7d)
...
- `OI-20260902-SUNSET-CANDIDATES-ARE-NOW-DUE-AND-NINE-ARE-UNDISPOSITIONED` · 13d — monitoring row 13d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-REPLAY-PREGATE-CANNOT-FINISH-BECAUSE-THE-TRAINER-IS-OUT-OF-MEMORY` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260902-REAPER-SHIPPED-AND-THE-KILL-PROVED-A-PARTIAL-LOSS-NOT-A-CLEAN-ONE` · 17d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DECISION-DRAIN-ROUTINE-DOES-NOT-EXIST-AND-NOTHING-HAS-EVER-DRAINED` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE` · 13d — monitoring row 13d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-PR-QUEUE-WATCHER-SHIPPED-AND-ITS-CRON-HAS-NEVER-FIRED` · 8d — monitoring row 8d since last observation (cadence 2d)
...
- 🔔 `OI-20260902-HOURLY-DIGEST-CARRIER-PREPARED-AND-HELD-FOR-A-TIER-2-OK` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260903-OPERATOR-COMMANDS-MOVED-OFF-THE-TRADER-BOT-PREPARED-AND-HELD` · 16d — monitoring row 16d since last observation (cadence 2d)
...
- 🔔 `OI-20260903-CLOSE-WEDGE-LEDGER-MERGED-AND-HAS-NEVER-BEEN-OBSERVED-PRESENT-ON-THE-FLEET` · 7d — loud row — must be reported on every session
...
- 🔔 `OI-20260904-MANAGER-WAKE-BUILT-AND-ITS-SCHEDULER-DOES-NOT-EXIST` · 14d — loud row — must be reported on every session
...
- 🔔 `OI-20260905-MI-128-SWEEP-WINDOW-REKEYED-AND-THE-PR-HOLDS-ON-A-BACKFILL-DECISION` · 14d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-5M-DEMOTED-OFF-BYBIT2-AND-NOTHING-HAS-SEEN-IT-STOP` · 7d — monitoring row 7d since last observation (cadence 1d)
...
- 🔔 `OI-20260906-THE-BRACKET-CALIBRATION-INSTRUMENT-EXISTS-AND-ITS-VERDICT-HAS-NOT-BEEN-ACTED-ON` · 13d — monitoring row 13d since last observation (cadence 7d)
...
- 🔔 `OI-20260906-SCALP-EXIT-HEAD-ARTIFACT-IS-THE-SECOND-MISSING-HALF-NOT-THE-FIRST` · 12d — monitoring row 12d since last observation (cadence 7d)
...
- 🔔 `OI-20260906-ML2-CALIBRATION-PASSED-BUT-THE-SHARPNESS-RESULT-IS-AGAINST-THE-WRONG-BASELINE` · 7d — monitoring row 7d since last observation (cadence 5d)
...
- 🔔 `OI-20260907-CLOSE-CONFIRM-RESCOPE-IS-HELD-AND-MI-140-MUST-NOT-BE-FIXED-BEFORE-IT` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-ALPACA-QUANTITY-COVERAGE-SHIPPED-AND-A-56-SHARE-NAKED-POSITION-IS-STANDING` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-A-HEDGE-BOOK-FLAT-READ-IS-CLOSING-LIVE-BYBIT-POSITIONS-AND-THE-FLAP-GUARD-CANNOT-SEE-IT` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260908-QLD-TQQQ-EXAMINED-LOCALLY-AND-THE-CI-FEED-PATH-HAS-NEVER-RUN` · 11d — monitoring row 11d since last observation (cadence 7d)
...
- 🔔 `OI-20260908-THE-REAL-MONEY-CLOSED-POPULATION-IS-447-NOT-15-AND-TWO-SURFACES-DISAGREE-ON-ITS-EXIT-LABELS` · 7d — loud row — must be reported on every session
...
- 🔔 `OI-20260909-PROP-STATUS-REQUEST-TRIGGER-DEPLOYED-BUT-THE-SUPPRESSION-HAS-NEVER-BEEN-OBSERVED` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260909-THE-FULL-SYSTEM-AUDITS-TIER-2-AND-TIER-3-PROPOSALS-ARE-FILED-AND-CARRIED-BY-NOTHING` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260909-INTENT-REDUCE-LEG-RESIZE-SHIPPED-AND-NO-LEG-HAS-BEEN-OBSERVED-RESIZED` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- 🔔 `OI-20260909-THE-ROLLBACK-REGISTRY-IS-COLLAPSED-AND-THE-ROLLBACK-HAS-NEVER-BEEN-EXERCISED` · 7d — monitoring row 7d since last observation (cadence 7d)
...
- 🔔 `OI-20260910-EXIT-EVAL-NEAR-MISS-BAND-AND-RESTART-GAP-SHIPPED-AND-NEITHER-HAS-GRADED-A-LIVE-BOUNDARY` · 9d — monitoring row 9d since last observation (cadence 7d)
...
- 🔔 `OI-20260910-F39-STRANDED-PACKAGE-LEGS-DETECTOR-BUILT-AND-THE-REATTACH-AWAITS-AN-OPERATOR-YES-NO` · 9d — monitoring row 9d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-WORKFLOW-PAGE-SHIPPED-AND-THE-OPERATOR-HAS-NOT-SEEN-IT-RENDER` · 9d — monitoring row 9d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-ALPACA-LIVE-OPTION-A-FOUR-LEGS-ADDED-TO-REAL-MONEY-AND-THE-ORDER-PATH-HAS-NEVER-BEEN-EXERCISED` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- `OI-20260910-ORPHAN-ADOPT-SIZE-GATE-DEPLOYED-AND-HAS-REFUSED-NOTHING` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- `OI-20260911-THE-UNROUTED-ROW-DETECTOR-IS-SEEDED-AND-HAS-REPORTED-NOTHING` · 8d — monitoring row 8d since last observation (cadence 2d)
...
- 🔔 `OI-20260911-BLOCKED-LANE-WATCH-SHIPPED-AND-NO-LANE-HAS-BEEN-WOKEN-BY-IT` · 8d — monitoring row 8d since last observation (cadence 3d)
...
- 🔔 `OI-20260911-WEBAPP-AUTH-SECRET-MAP-LANDED-AND-HAS-CARRIED-NO-VALUE` · 7d — loud row — must be reported on every session
...
- 🔔 `OI-20260911-THE-DIRECTIONAL-LEGS-BROKE-ON-2026-08-30-AND-THE-CAUSE-IS-UNATTRIBUTED` · 2d — monitoring row 2d since last observation (cadence 1d)
...
- 🔔 `OI-20260911-THE-PROP-ACCOUNT-IS-STARVED-NOT-QUIET-AND-THE-UNBLOCK-IS-A-SEQUENCE-NOT-A-FIX` · 7d — monitoring row 7d since last observation (cadence 1d)
...
- 🔔 `OI-20260911-THE-TWO-DETECTORS-ARE-BUILT-AND-NEITHER-HAS-EVER-FIRED-ON-THE-FLEET` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260912-THE-WINNER-SIZE-COLLAPSE-IS-AN-R-COLLAPSE-AND-ITS-MAGNITUDE-IS-NOT-ESTABLISHED` · 7d — monitoring row 7d since last observation (cadence 7d)
...
- 🔔 `OI-20260911-TIER-2-APPROVED-SELF-LANDING-IS-BUILT-AND-NO-PR-HAS-LANDED-OR-BEEN-REFUSED-BY-IT` · 7d — monitoring row 7d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-IB-PER-PASS-BREAKER-ARMED-AND-HAS-SKIPPED-NOTHING` · 6d — monitoring row 6d since last observation (cadence 2d)
...
- 🔔 `OI-20260912-MI280-SHIPPED-EIGHT-LANDING-AND-REGISTER-INSTRUMENTS-AND-MOST-HAVE-NEVER-BEEN-SEEN-WORKING` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260912-FANOUT-APPLY-PATH-REPAIRED-AND-STILL-HAS-NEVER-DISPATCHED` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260912-BYBIT-SYMBOL-DEDUPE-REPAIRED-AND-NO-FLEET-READ-HAS-BEEN-SEEN-WITHOUT-THE-DROP` · 6d — monitoring row 6d since last observation (cadence 2d)
...
- 🔔 `OI-20260913-A-BYBIT2-HEDGE-BOOK-IS-NEVER-FETCHED-AND-NO-SURFACE-IN-THIS-REPO-CAN-SAY-WHETHER-IT-IS-STILL-THERE` · 6d — monitoring row 6d since last observation (cadence 1d)
...
- 🔔 `OI-20260917-THE-GENERATED-CONFLICT-RECOMPUTE-IS-PROVEN-IN-REPLAY-AND-HAS-NEVER-RUN-ON-A-RUNNER` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260918-THE-TARGET-GEOMETRY-PACKET-EXISTS-AND-ITS-THREE-DECISIONS-ARE-UNANSWERED` · 1d — loud row — must be reported on every session
...
- `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 17d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: verdicts_differ=true. This is a KNOWN, DECLARED g…
  - #10746 ships a soak at runtime_logs/bybit_coverage_soak.jsonl whose rows are the ONLY declared evidence for the operator's conditional widening of the graded-coverage gate to bybi…
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 13d — soak READY — ✅ READY — 40 matching row(s) of 1000 scanned satisfy the declared criterion (decision_state!=not_scored). ⚠️ Ready is not CLEA…
  - MI-150 shipped the M20 exit-head consumer into `ict_scalp` (annotate-only, disarmed) -- the wiring MI-146 identified as the only passed-gate work blocked on missing code. ⚠️ IT CA…
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` · 10d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: dropped_symbol_dedupe_count>=1. This is a KNOWN,…
...
- `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 10d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: examined>=1. This is a KNOWN, DECLARED gap (`prob…
  - The closed->exchange-flat invariant -- THE ONE MECHANISM that can independently contradict 'this trade is closed' -- was silent for four independent reasons, each alone sufficient…
...
**`unlanded_automation` — 67 due**

- 🔔 `#12584` · 0d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12529` · 1d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12511` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12502` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12498` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12493` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12489` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12486` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12484` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12477` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12474` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12469` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12466` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12459` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12456` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12451` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#12385` · 2d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12363` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12362` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#12361` · 2d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#12360` · 2d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#12359` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#12358` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#12357` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12356` · 2d — producer output opened a PR that has not landed
  - chore(m1): economic-calendar PIT snapshots (auto)
- 🔔 `#12353` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12343` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12337` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12331` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12330` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12328` · 3d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12326` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12320` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12316` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12312` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12306` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12299` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12298` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12297` · 4d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12295` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12289` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12284` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12279` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12276` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12269` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12268` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12265` · 5d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12263` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12257` · 5d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12253` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12249` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12245` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12239` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12231` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12230` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12229` · 6d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12228` · 6d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12225` · 6d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11998` · 7d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#11956` · 7d — producer output opened a PR that has not landed
  - chore(m28): valuation snapshots (auto)
- 🔔 `#11953` · 7d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11952` · 7d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11943` · 7d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11939` · 7d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#11919` · 7d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#11916` · 7d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#11912` · 7d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)

**`error_feed` — 11 due**

- `ERRFEED-72e2ad1e` · 5d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 70 rows 2026-09-14T10:02:36 → 2026-09-18T23:14:31 · accou…
...
- `ERRFEED-5402e495` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 47 rows 2026-09-16T23:54:03 → 2026-09-17T01:25:29 · accou…
...
- `ERRFEED-fa77ad80` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 24 rows 2026-09-12T18:12:44 → 2026-09-18T21:49:18 · symbo…
...
- `ERRFEED-9da85381` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 21 rows 2026-09-12T18:18:11 → 2026-09-18T04:51:30 · symbo…
...
- `ERRFEED-a46d2970` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 16 rows 2026-09-12T18:12:44 → 2026-09-19T05:08:13 · symbo…
  - [error] x16 ⚠️ MONITOR BLIND — open position has no live dynamic exit Order package: pkg-<hex> Strategy: mhg_pullback_Nd | Symbol: MHG Reason: candles_u
- `ERRFEED-22c7c0be` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 14 rows 2026-09-16T19:29:11 → 2026-09-16T19:56:10 · accou…
...
- `ERRFEED-ae533209` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 7 rows 2026-09-13T14:15:14 → 2026-09-19T01:54:22 · accoun…
  - [error] x7 🔔 Broker close detected by reconciler Account: bybit_N Symbol: AVAXUSDT | Side: long DB trade id: N Package: pkg-<hex> Reason: reconciler Cl
- `ERRFEED-516a1b2a` · 5d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 6 rows 2026-09-14T06:10:03 → 2026-09-18T13:39:27 · accoun…
...
- `ERRFEED-99d11bfe` · 5d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 5 rows 2026-09-14T18:21:48 → 2026-09-19T01:23:16 · accoun…
  - [error] x5 🔔 Broker close detected by reconciler Account: bybit_N Symbol: XRPUSDT | Side: short DB trade id: N Package: pkg-<hex> Reason: reconciler Cl
- `ERRFEED-bfbfcb97` · 5d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 5 rows 2026-09-14T08:01:34 → 2026-09-18T08:01:17 · accoun…
  - [error] x5 🛑 Position CLOSE failing — won't flatten Account: alpaca_paper Symbol: GLD | Side: long | Qty: N Consecutive close failures: N share_hold: b
- `ERROR-FEED-SUMMARY` — 50 further error group(s) and every warn group are NOT listed above — read `docs/claude/ERROR-FEED-DIGEST.json` for the full set. Digest ve…
  - 91 cause groups over 1354 rows (60 error-level, 31 warn-level, 1 new since the last digest)

**`sunset` — 12 due**

- `strategy:avax_pullback_2h` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · avax_pullback_2h
- `strategy:fade_breakout_4h` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · fade_breakout_4h
- `strategy:fvg_range_15m` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · fvg_range_15m
- `strategy:htf_pullback_trend_2h` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · htf_pullback_trend_2h
- `strategy:ief_pullback_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · ief_pullback_1d
- `strategy:iwm_trend_long_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · iwm_trend_long_1d
- `strategy:mhg_pullback_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · mhg_pullback_1d
- `strategy:qld_trend_long_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · qld_trend_long_1d
- `strategy:qqq_trend_long_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · qqq_trend_long_1d
- `strategy:slv_pullback_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · slv_pullback_1d
- `strategy:slv_trend_1h` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · slv_trend_1h
- `strategy:tlt_pullback_1d` · 18d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
...
- 🔔 `stuck-branches-changed` — automation/reconcile-open-prs-35308264155-1, automation/replay-pregate-35326495386-1. A stranded automation branch is a producer whose outp…
  - 2 automation branch(es) newly stranded, 0 cleared
...
- 🔔 `manager-queue-watch-never-ran-overdue` · 16d — armed 404.9h ago, on the order of 404 expected firings, and ZERO receipts have ever been committed. The Routine reports SUCCEEDED, so nothi…
