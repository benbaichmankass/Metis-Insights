...
_Generated 2026-09-12T10:39:09+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
**Completeness: `all_sources_read`** · **109 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `probes` | `read` | 9 | freshness=fresh age=24.8h cadence=daily (cron 20 5 * * *) \| 4 probe result(s) deferred to the `soaks` source,… |
| `red_crons` | `read` | 6 |  |
| `unlanded_automation` | `read` | 3 |  |
| `error_feed` | `read` | 11 | digest 2026-09-12T06:27:27+00:00, age 4.2h |
| `sunset` | `read` | 0 |  |
