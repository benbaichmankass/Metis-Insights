...
_Generated 2026-09-10T11:08:35+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
71 of 709 objects (10.0%) have an ASSESSED `blocked_on` basis, below the declared floor of 50%. **No stage is named.** 638 objects carry an empty `blocked_on` that is NOT a claim that nothing blocks them — it is nobody having looked. A stage computed over this graph would describe the 71 rows somebody assessed, not the system.

| population | assessed | coverage | floor |
|---|---|---|---|
| 709 objects | 71 | **10.0%** | 50.0% |

**Edge basis, never collapsed** — `blocked` 28 · `declared_none` 43 · **`unstated` 638** · `malformed` 0.
...
Objects by stage: `QUESTION` 16 · `EVIDENCE` 95 · `DECISION` 17 · `DEPLOYMENT` 24 · `OBSERVATION` 6 · `CAPABILITY` 25 · `INTEGRITY` 519 · `(unstated)` 7

⚠️ **576 of 709 of those stages were assigned in BULK FROM THE SOURCE FILENAME, not by reading the row.** The Phase C migration maps `health-review-backlog.json` → `INTEGRITY` and `{ml,performance,research}-review-backlog.json` → `EVIDENCE`, with no per-row judgement, so `INTEGRITY 519` is a census of ONE filename. Only **130** stage(s) in the whole store were chosen per object — and choosing one is not a claim it is RIGHT, only that a filename did not decide it.

Stage by how the stage was arrived at: `bulk_by_source_file` → EVIDENCE 78, INTEGRITY 498 · `per_object` → (unstated) 4, CAPABILITY 25, DECISION 17, DEPLOYMENT 24, EVIDENCE 17, INTEGRITY 21, OBSERVATION 6, QUESTION 16 · `unstated` → (unstated) 3

**The assessed subgraph — every object that declares an edge (28 of 709):**
...
- **`WO-20260907-CLOSE-CONFIRMATION-IS-SYMBOL-SCOPED`** (EVIDENCE · waiting) — The close-confirmation gate is symbol-scoped while the close is trade-scoped, so a succeeded close is graded a failure and its journal row left open
  - `external_event` → `a real IB close on a symbol holding a sibling trade's lots, OBSERVED confirming on the fleet` · ref `not_in_store_by_design` · hold **`unverifiable_outside_store`** · since 2026-09-08
...
- **`WO-20260908-TRADE-SCOPE-THE-ALPACA-CLOSE-OPERATION-AND`** (CAPABILITY · waiting) — Trade-scope the Alpaca close operation (and modify_protective)
  - `operator_decision` → `The Alpaca close-confirmation MEASUREMENT — reserved to #11312 by #11279's own approval scope` · ref `not_in_store_by_design` · hold **`unverifiable_outside_store`** · since 2026-09-08
- **`WO-20260908-WIRE-MORE-STRATEGIES-TO-ALPACA-LIVE-WHICH`** (DECISION · waiting) — Wire more strategies to alpaca_live, which runs one leg on a 200 dollar cash account
  - `operator_decision` → `DEC-20260910-ALPACA-LIVE-ROSTER-EXPANSION` · ref `not_in_store_by_design` · hold **`unverifiable_outside_store`** · since 2026-09-10
...
- **`WO-20260909-DECISION-INTENT-REDUCE-ORPHANS-A-PROTECTIVE-LEG`** (DECISION · waiting) — intent_reduce leaves a stop up to 101.8x the row it protects - operator answered: resize the leg AND add the per-row invariant
  - `external_event` → `a real intent-reduce firing on a Bybit account under BYBIT_TPSL_MODE=partial, after the 7b8b6b51d deploy reaches the running trader - and the shrunk row's tracked leg then read back off /api/diag/bybit_open_orders at that row's NEW position_size` · ref `not_in_store_by_design` · hold **`unverifiable_outside_store`** · since 2026-09-09
- **`WO-20260910-ROOT-CAUSE-THE-EXIT-EVAL-60S-BREACHES`** (DIAGNOSIS · waiting) — The exit-eval 60s breaches have TWO different causes - a closed within-process episode plus a live IBKR-reset-window residual, and a structural restart gap wit…
  - `operator_decision` → `DEC-20260910-EXIT-EVAL-60S-REMEDY` · ref `not_in_store_by_design` · hold **`unverifiable_outside_store`** · since 2026-09-10
...
|---|---|---|---|---|
| 40 | 35.0% | -9.0666 | -9.0666 | **70.0%** |

Provenance split — measured 28 · estimated 12 · fabricated 0 · unverified 0.
...
**6 in flight against a ceiling of 8** (headroom 2) · 25 waiting.
...
**From the work store: 15 `operator_decision` edge(s).**
...
- `WO-20260908-TRADE-SCOPE-THE-ALPACA-CLOSE-OPERATION-AND` (CAPABILITY) → The Alpaca close-confirmation MEASUREMENT — reserved to #11312 by #11279's own approval scope · since 2026-09-08
- `WO-20260908-WIRE-MORE-STRATEGIES-TO-ALPACA-LIVE-WHICH` (DECISION) → DEC-20260910-ALPACA-LIVE-ROSTER-EXPANSION · since 2026-09-10
- `WO-20260910-ROOT-CAUSE-THE-EXIT-EVAL-60S-BREACHES` (DIAGNOSIS) → DEC-20260910-EXIT-EVAL-60S-REMEDY · since 2026-09-10

⚠️ Zero here does NOT mean no decision is pending — it means no object DECLARES one, and 638 of 709 objects have never been assessed for edges at all.

**From `docs/claude/operator-owed-register.json`: read state `read`, 1 OPEN item(s)** (carry limit 2; 5 terminal, not listed).

- `OO-20260910-F39-REATTACH-EXACT-DIFF-YES-NO` (`open` · medium · judgement · 0d) — Yes/no on the EXACT re-attach diff for the four F-39 stranded package legs (3 order_packages rows, one on real money)
...
**Completeness: `all_sources_read`** · **156 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `probes` | `read` | 10 | freshness=stale age=72.6h cadence=daily (cron 20 5 * * *) \| 2 probe result(s) deferred to the `soaks` source,… |
| `red_crons` | `read` | 7 |  |
| `unlanded_automation` | `read` | 54 |  |
| `error_feed` | `read` | 11 | digest 2026-09-10T10:00:58+00:00, age 1.1h |
| `sunset` | `read` | 0 |  |
...
- 🔔 `OI-20260906-BYBIT-CLOSE-QTY-SNAP-MERGED-BUT-NEVER-OBSERVED-ON-THE-FLEET` · 4d — monitoring row 4d since last observation (cadence 4d)
...
- 🔔 `OI-20260906-RESEARCH-THAT-SPECIFIES-WORK-IS-CARRIED-BY-NOTHING` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-OPERATING-LAYER-BUILD-IS-IN-FLIGHT-AND-CARRIED-ONLY-BY-THIS-ROW` · 9d — monitoring row 9d since last observation (cadence 1d)
...
- 🔔 `OI-20260826-STRAY-OCA-SWEEP-SHIPPED-BUT-UNARMED` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260829-ALPACA-GOLIVE-BLOCKED-ON-T1-SETTLEMENT-MODEL` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260829-E35-REVERSED-LEGS-ARE-A-TIER-3-PROPOSAL-SET-NOT-APPLIED` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LIVE-WALLET-TRUTH-CANNOT-REPRODUCE-THE-LEDGER-WINDOW` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-SCOPE-OVERLAP-ATTRIBUTOR-MISREADS-A-SESSIONS-OWN-START` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LOCAL-LLM-VERDICT-PENDING-A-DIFFERENT-SIZE-ARM` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-READ-DEBT-11-UNREAD-AND-256-SUPERSEDED-UNREAD` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-ALPACA-LIVE-FIRST-REAL-MONEY-LEG-ROUTED-BUT-HAS-NEVER-TRADED` · 0d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-CYCLE-PRIORITY-IS-RENDERED-BUT-NO-SESSION-HAS-ACTED-ON-IT` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-ALPACA-SHARE-HOLD-CLASSIFIER-SHIPPED-NOT-YET-OBSERVED` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-SUNSET-PASS-SHIPPED-AND-NOTHING-HAS-BEEN-RETIRED` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-CLAUDE-CHANNEL-SEPARATION-SHIPPED-BUT-UNPROVEN` · 9d — monitoring row 9d since last observation (cadence 1d)
...
- 🔔 `OI-20260902-TELEGRAM-DECISION-ROUNDTRIP-SHIPPED-AND-NO-TAP-HAS-EVER-BEEN-MADE` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DECISION-PROMPTS-MOVED-TO-CLAUDEBOT-AND-NO-TAP-HAS-LANDED-THERE` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-REPLAY-PREGATE-CANNOT-FINISH-BECAUSE-THE-TRAINER-IS-OUT-OF-MEMORY` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-REAPER-SHIPPED-AND-THE-KILL-PROVED-A-PARTIAL-LOSS-NOT-A-CLEAN-ONE` · 8d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260903-OPERATOR-COMMANDS-MOVED-OFF-THE-TRADER-BOT-PREPARED-AND-HELD` · 7d — monitoring row 7d since last observation (cadence 2d)
...
- 🔔 `OI-20260904-MANAGER-WAKE-BUILT-AND-ITS-SCHEDULER-DOES-NOT-EXIST` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260905-MI-128-SWEEP-WINDOW-REKEYED-AND-THE-PR-HOLDS-ON-A-BACKFILL-DECISION` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-5M-DEMOTED-OFF-BYBIT2-AND-NOTHING-HAS-SEEN-IT-STOP` · 2d — monitoring row 2d since last observation (cadence 1d)
...
- 🔔 `OI-20260906-THE-BRACKET-CALIBRATION-INSTRUMENT-EXISTS-AND-ITS-VERDICT-HAS-NOT-BEEN-ACTED-ON` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-SCALP-EXIT-HEAD-ARTIFACT-IS-THE-SECOND-MISSING-HALF-NOT-THE-FIRST` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260907-CLOSE-CONFIRM-RESCOPE-IS-HELD-AND-MI-140-MUST-NOT-BE-FIXED-BEFORE-IT` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260907-TELEMETRY-HOOK-IS-APPROVED-AND-GREEN-IN-AN-UNMERGED-PR-AND-OBSERVED-ON-NOTHING` · 3d — monitoring row 3d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-ALPACA-QUANTITY-COVERAGE-SHIPPED-AND-A-56-SHARE-NAKED-POSITION-IS-STANDING` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-A-HEDGE-BOOK-FLAT-READ-IS-CLOSING-LIVE-BYBIT-POSITIONS-AND-THE-FLAP-GUARD-CANNOT-SEE-IT` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-QLD-TQQQ-EXAMINED-LOCALLY-AND-THE-CI-FEED-PATH-HAS-NEVER-RUN` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-CANCEL-ACCEPTED-INEFFECTIVE-MERGED-AND-NEVER-EMITTED-ON-THE-FLEET` · 2d — loud row — must be reported on every session
  - PR #11433 merged 2026-09-08 as 2547a6a8 (squash), Tier-2, operator-approved 2026-09-08 with no conditions, and HAND-MERGED BY THE MANAGER rather than self-landed (the branch decla…
- 🔔 `OI-20260908-THE-REAL-MONEY-CLOSED-POPULATION-IS-447-NOT-15-AND-TWO-SURFACES-DISAGREE-ON-ITS-EXIT-LABELS` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260909-PROP-STATUS-REQUEST-TRIGGER-DEPLOYED-BUT-THE-SUPPRESSION-HAS-NEVER-BEEN-OBSERVED` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260909-THE-FULL-SYSTEM-AUDITS-TIER-2-AND-TIER-3-PROPOSALS-ARE-FILED-AND-CARRIED-BY-NOTHING` · 1d — loud row — must be reported on every session
...
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` — monitoring row has NEVER been observed
...
- 🔔 `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` — monitoring row has NEVER been observed
...
- 🔔 `OI-20260909-INTENT-REDUCE-LEG-RESIZE-SHIPPED-AND-NO-LEG-HAS-BEEN-OBSERVED-RESIZED` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-EXIT-EVAL-NEAR-MISS-BAND-AND-RESTART-GAP-SHIPPED-AND-NEITHER-HAS-GRADED-A-LIVE-BOUNDARY` · 0d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-F39-STRANDED-PACKAGE-LEGS-DETECTOR-BUILT-AND-THE-REATTACH-AWAITS-AN-OPERATOR-YES-NO` · 0d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-WORKFLOW-PAGE-SHIPPED-AND-THE-OPERATOR-HAS-NOT-SEEN-IT-RENDER` · 0d — loud row — must be reported on every session
...
- `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 8d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: verdicts_differ=true. This is a KNOWN, DECLARED g…
  - #10746 ships a soak at runtime_logs/bybit_coverage_soak.jsonl whose rows are the ONLY declared evidence for the operator's conditional widening of the graded-coverage gate to bybi…
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 4d — soak READY — ✅ READY — 37 matching row(s) of 754 scanned satisfy the declared criterion (decision_state!=not_scored). ⚠️ Ready is not CLEAR…
  - MI-150 shipped the M20 exit-head consumer into `ict_scalp` (annotate-only, disarmed) -- the wiring MI-146 identified as the only passed-gate work blocked on missing code. ⚠️ IT CA…
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` · 1d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: dropped_symbol_dedupe_count>=1. This is a KNOWN, …
  - MI-222 Tier-2 (operator-approved observable-first, 2026-09-09) made the collapsed position read at src/units/accounts/clients.py COUNTABLE without changing what any caller receive…
- `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 1d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: examined>=1. This is a KNOWN, DECLARED gap (`prob…
  - The closed->exchange-flat invariant -- THE ONE MECHANISM that can independently contradict 'this trade is closed' -- was silent for four independent reasons, each alone sufficient…
...
- 🔔 `PROBE-REPORT-STALE` · 3d — EVERY probe verdict below was observed at 2026-09-07T10:31:05.344186+00:00, NOT today. The probes job did not land a fresh report, so treat…
  - probe report is 72.6h old — expected daily (cron 20 5 * * *)
- `OI-20260903-CAPTURE-WATCH-PROVED-ITSELF-ON-DEMAND-AND-ITS-SCHEDULE-HAS-NEVER-SUCCEEDED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
...
- `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
  - probe could not run (exit_2)
- `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
...
- `OI-20260902-SUNSET-CANDIDATES-ARE-NOW-DUE-AND-NINE-ARE-UNDISPOSITIONED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-DECISION-DRAIN-ROUTINE-DOES-NOT-EXIST-AND-NOTHING-HAS-EVER-DRAINED` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
...
- `OI-20260902-DIGEST-MOVED-OFF-A-CRON-THAT-DOES-NOT-FIRE-AND-THE-NEW-TRIGGER-HAS-NEVER-FIRED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 72.6h ago]
  - probe could not run (exit_2)
...
**`unlanded_automation` — 54 due**

- 🔔 `#11672` · 0d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)
- 🔔 `#11664` · 0d — producer output opened a PR that has not landed
  - gld-compat: per-account verdicts (run 34456197195)
- 🔔 `#11662` · 0d — producer output opened a PR that has not landed
  - gld-compat: per-account verdicts (run 34455459855)
- 🔔 `#11654` · 0d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11620` · 0d — producer output opened a PR that has not landed
  - chore(m1): economic-calendar PIT snapshots (auto)
- 🔔 `#11595` · 1d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#11541` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11536` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11525` · 1d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)
- 🔔 `#11518` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11512` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11493` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11489` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11476` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11475` · 1d — producer output opened a PR that has not landed
  - chore(m1): economic-calendar PIT snapshots (auto)
- 🔔 `#11471` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11470` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11468` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11465` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11462` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11459` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11458` · 2d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11456` · 2d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#11454` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11453` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11452` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11449` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11448` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11445` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11444` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11442` · 2d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11439` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11438` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11432` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11429` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11428` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#11422` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11420` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11416` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11408` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11404` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11402` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11399` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11397` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11393` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11386` · 2d — producer output opened a PR that has not landed
  - chore(m28): valuation snapshots (auto)
- 🔔 `#11382` · 2d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11376` · 2d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11368` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the due-list (auto)
- 🔔 `#11367` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11366` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#11362` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11361` · 2d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)
- 🔔 `#11353` · 2d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)

**`error_feed` — 11 due**

- `ERRFEED-86505576` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 60 rows 2026-09-04T12:34:09 → 2026-09-08T17:52:08 · accou…
...
- `ERRFEED-02a919f1` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 56 rows 2026-09-07T08:06:36 → 2026-09-08T14:05:20 · accou…
...
- `ERRFEED-a46d2970` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 35 rows 2026-09-04T19:13:36 → 2026-09-10T05:45:10 · symbo…
  - [error] x35 ⚠️ MONITOR BLIND — open position has no live dynamic exit Order package: pkg-<hex> Strategy: mhg_pullback_Nd | Symbol: MHG Reason: candles_u
- `ERRFEED-fa77ad80` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 30 rows 2026-09-04T21:55:38 → 2026-09-10T05:45:09 · symbo…
  - [error] x30 ⚠️ MONITOR BLIND — open position has no live dynamic exit Order package: pkg-<hex> Strategy: mes_trend_long_Nd | Symbol: MES Reason: candles
- `ERRFEED-b3c9b0e2` · 6d — error-level condition on `bot_logs`, STANDING (predates the last digest) — 24 rows 2026-09-04T12:28:34 → 2026-09-10T04:26:34 · accounts=byb…
  - [error] x24 bybit_over_cover detected: bybit_N/ADAUSDT: position N. SAME-BOOK LEG OVER-ACCUMULATION: legs that REDUCE THIS position total N across N leg
- `ERRFEED-72e2ad1e` · 2d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 19 rows 2026-09-08T19:46:55 → 2026-09-09T23:18:17 · accou…
  - [error] x19 🧱 Position CLOSE wedged BROKER-SIDE — carried in the digest Account: alpaca_paper Symbol: GLD | Side: long | Qty: N Consecutive close failur
- `ERRFEED-6da04340` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 17 rows 2026-09-04T19:13:36 → 2026-09-08T14:16:09 · symbo…
...
- `ERRFEED-9da85381` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 16 rows 2026-09-07T08:38:27 → 2026-09-10T09:07:19 · symbo…
...
- `ERRFEED-f754bb5d` · 2d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 16 rows 2026-09-08T19:22:45 → 2026-09-10T05:45:10 · symbo…
...
- `ERRFEED-1ef305c7` · 8d — error-level condition on `bot_logs`, STANDING (predates the last digest) — 5 rows 2026-09-02T09:06:11 → 2026-09-06T00:32:43 · symbols=AVAXU…
  - [error] x5 api_call bybit_place_order_failed: <REDACTED_TOKEN>: The number of contracts exceeds maximum limit allowed: too large, order_qty:N > max_qty
- `ERROR-FEED-SUMMARY` — 58 further error group(s) and every warn group are NOT listed above — read `docs/claude/ERROR-FEED-DIGEST.json` for the full set. Digest ve…
  - 100 cause groups over 1370 rows (68 error-level, 32 warn-level, 0 new since the last digest)
