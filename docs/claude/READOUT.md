...
_Generated 2026-09-11T11:08:16+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
75 of 163 objects (46.0%) have an ASSESSED `blocked_on` basis, below the declared floor of 50%. **No stage is named.** 88 objects carry an empty `blocked_on` that is NOT a claim that nothing blocks them — it is nobody having looked. A stage computed over this graph would describe the 75 rows somebody assessed, not the system.

| population | assessed | coverage | floor |
|---|---|---|---|
| 163 objects | 75 | **46.0%** | 50.0% |

**Edge basis, never collapsed** — `blocked` 26 · `declared_none` 49 · **`unstated` 88** · `malformed` 0.
...
Objects by stage: `QUESTION` 16 · `EVIDENCE` 26 · `DECISION` 17 · `DEPLOYMENT` 24 · `OBSERVATION` 6 · `CAPABILITY` 28 · `INTEGRITY` 39 · `(unstated)` 7

⚠️ **23 of 163 of those stages were assigned in BULK FROM THE SOURCE FILENAME, not by reading the row.** The Phase C migration maps `health-review-backlog.json` → `INTEGRITY` and `{ml,performance,research}-review-backlog.json` → `EVIDENCE`, with no per-row judgement, so `INTEGRITY 39` is a census of ONE filename. Only **137** stage(s) in the whole store were chosen per object — and choosing one is not a claim it is RIGHT, only that a filename did not decide it.

Stage by how the stage was arrived at: `bulk_by_source_file` → EVIDENCE 9, INTEGRITY 14 · `per_object` → (unstated) 4, CAPABILITY 28, DECISION 17, DEPLOYMENT 24, EVIDENCE 17, INTEGRITY 25, OBSERVATION 6, QUESTION 16 · `unstated` → (unstated) 3

**The assessed subgraph — every object that declares an edge (26 of 163):**
...
|---|---|---|---|---|
| 41 | 34.1% | -14.1006 | -14.1006 | **73.2%** |

Provenance split — measured 30 · estimated 11 · fabricated 0 · unverified 0.
...
**7 in flight against a ceiling of 8** (headroom 1) · 26 waiting.
...
⚠️ Zero here does NOT mean no decision is pending — it means no object DECLARES one, and 88 of 163 objects have never been assessed for edges at all.
...
**Completeness: `all_sources_read`** · **99 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `probes` | `read` | 9 | freshness=fresh age=1.3h cadence=daily (cron 20 5 * * *) \| 4 probe result(s) deferred to the `soaks` source, … |
| `red_crons` | `read` | 4 |  |
| `unlanded_automation` | `read` | 0 |  |
| `error_feed` | `read` | 11 | digest 2026-09-11T05:11:11+00:00, age 6.0h |
| `sunset` | `read` | 0 |  |
...
- 🔔 `OI-20260901-OPERATING-LAYER-BUILD-IS-IN-FLIGHT-AND-CARRIED-ONLY-BY-THIS-ROW` · 10d — monitoring row 10d since last observation (cadence 1d)
...
- 🔔 `OI-20260829-ALPACA-GOLIVE-BLOCKED-ON-T1-SETTLEMENT-MODEL` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260829-E35-REVERSED-LEGS-ARE-A-TIER-3-PROPOSAL-SET-NOT-APPLIED` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LIVE-WALLET-TRUTH-CANNOT-REPRODUCE-THE-LEDGER-WINDOW` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` · 11d — monitoring row 11d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-SCOPE-OVERLAP-ATTRIBUTOR-MISREADS-A-SESSIONS-OWN-START` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LOCAL-LLM-VERDICT-PENDING-A-DIFFERENT-SIZE-ARM` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-READ-DEBT-11-UNREAD-AND-256-SUPERSEDED-UNREAD` · 11d — monitoring row 11d since last observation (cadence 7d)
...
- 🔔 `OI-20260901-SUNSET-PASS-SHIPPED-AND-NOTHING-HAS-BEEN-RETIRED` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-CLAUDE-CHANNEL-SEPARATION-SHIPPED-BUT-UNPROVEN` · 10d — monitoring row 10d since last observation (cadence 1d)
...
- 🔔 `OI-20260902-TELEGRAM-DECISION-ROUNDTRIP-SHIPPED-AND-NO-TAP-HAS-EVER-BEEN-MADE` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DECISION-PROMPTS-MOVED-TO-CLAUDEBOT-AND-NO-TAP-HAS-LANDED-THERE` · 5d — loud row — must be reported on every session
...
- `OI-20260902-ROLE-PACKS-NOW-REACH-THE-OPERATING-LAYER-AND-NO-SESSION-HAS-BEEN-OBSERVED-USING-IT` · 9d — monitoring row 9d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-REAPER-SHIPPED-AND-THE-KILL-PROVED-A-PARTIAL-LOSS-NOT-A-CLEAN-ONE` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DIGEST-MOVED-OFF-A-CRON-THAT-DOES-NOT-FIRE-AND-THE-NEW-TRIGGER-HAS-NEVER-FIRED` · 3d — monitoring row 3d since last observation (cadence 1d)
...
- 🔔 `OI-20260903-OPERATOR-COMMANDS-MOVED-OFF-THE-TRADER-BOT-PREPARED-AND-HELD` · 8d — monitoring row 8d since last observation (cadence 2d)
...
- 🔔 `OI-20260904-MANAGER-WAKE-BUILT-AND-ITS-SCHEDULER-DOES-NOT-EXIST` · 6d — loud row — must be reported on every session
...
- 🔔 `OI-20260905-MI-128-SWEEP-WINDOW-REKEYED-AND-THE-PR-HOLDS-ON-A-BACKFILL-DECISION` · 6d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-THE-BRACKET-CALIBRATION-INSTRUMENT-EXISTS-AND-ITS-VERDICT-HAS-NOT-BEEN-ACTED-ON` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-SCALP-EXIT-HEAD-ARTIFACT-IS-THE-SECOND-MISSING-HALF-NOT-THE-FIRST` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 5d — loud row — must be reported on every session
...
- 🔔 `OI-20260907-TELEMETRY-HOOK-IS-APPROVED-AND-GREEN-IN-AN-UNMERGED-PR-AND-OBSERVED-ON-NOTHING` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-QLD-TQQQ-EXAMINED-LOCALLY-AND-THE-CI-FEED-PATH-HAS-NEVER-RUN` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-CANCEL-ACCEPTED-INEFFECTIVE-MERGED-AND-NEVER-EMITTED-ON-THE-FLEET` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-EXIT-EVAL-NEAR-MISS-BAND-AND-RESTART-GAP-SHIPPED-AND-NEITHER-HAS-GRADED-A-LIVE-BOUNDARY` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-F39-STRANDED-PACKAGE-LEGS-DETECTOR-BUILT-AND-THE-REATTACH-AWAITS-AN-OPERATOR-YES-NO` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-WORKFLOW-PAGE-SHIPPED-AND-THE-OPERATOR-HAS-NOT-SEEN-IT-RENDER` · 1d — loud row — must be reported on every session
...
- `OI-20260910-ORPHAN-ADOPT-SIZE-GATE-DEPLOYED-AND-HAS-REFUSED-NOTHING` — monitoring row has NEVER been observed
...
- `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 9d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: verdicts_differ=true. This is a KNOWN, DECLARED g…
  - #10746 ships a soak at runtime_logs/bybit_coverage_soak.jsonl whose rows are the ONLY declared evidence for the operator's conditional widening of the graded-coverage gate to bybi…
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 5d — soak READY — ✅ READY — 19 matching row(s) of 1000 scanned satisfy the declared criterion (decision_state!=not_scored). ⚠️ Ready is not CLEA…
  - MI-150 shipped the M20 exit-head consumer into `ict_scalp` (annotate-only, disarmed) -- the wiring MI-146 identified as the only passed-gate work blocked on missing code. ⚠️ IT CA…
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` · 2d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: dropped_symbol_dedupe_count>=1. This is a KNOWN, …
...
- `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 2d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: examined>=1. This is a KNOWN, DECLARED gap (`prob…
  - The closed->exchange-flat invariant -- THE ONE MECHANISM that can independently contradict 'this trade is closed' -- was silent for four independent reasons, each alone sufficient…
...
- `ERRFEED-86505576` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 60 rows 2026-09-04T12:34:09 → 2026-09-08T17:52:08 · accou…
...
- `ERRFEED-02a919f1` · 4d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 56 rows 2026-09-07T08:06:36 → 2026-09-08T14:05:20 · accou…
...
- `ERRFEED-a46d2970` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 39 rows 2026-09-04T19:13:36 → 2026-09-11T04:08:33 · symbo…
...
- `ERRFEED-fa77ad80` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 34 rows 2026-09-04T21:55:38 → 2026-09-11T04:08:16 · symbo…
...
- `ERRFEED-72e2ad1e` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 33 rows 2026-09-08T19:46:55 → 2026-09-10T23:17:16 · accou…
...
- `ERRFEED-b3c9b0e2` · 7d — error-level condition on `bot_logs`, STANDING (predates the last digest) — 25 rows 2026-09-04T12:28:34 → 2026-09-10T10:27:04 · accounts=byb…
...
- `ERRFEED-6da04340` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 17 rows 2026-09-04T19:13:36 → 2026-09-08T14:16:09 · symbo…
...
- `ERRFEED-9da85381` · 4d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 16 rows 2026-09-07T08:38:27 → 2026-09-10T09:07:19 · symbo…
...
- `ERRFEED-f754bb5d` · 3d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 16 rows 2026-09-08T19:22:45 → 2026-09-10T05:45:10 · symbo…
...
- `ERRFEED-c8553cf1` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 7 rows 2026-09-04T12:36:04 → 2026-09-10T18:07:57 — decide…
...
- `ERROR-FEED-SUMMARY` — 57 further error group(s) and every warn group are NOT listed above — read `docs/claude/ERROR-FEED-DIGEST.json` for the full set. Digest ve…
