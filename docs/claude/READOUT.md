...
_Generated 2026-09-16T11:23:26+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
86 of 183 objects (47.0%) have an ASSESSED `blocked_on` basis, below the declared floor of 50%. **No stage is named.** 96 objects carry an empty `blocked_on` that is NOT a claim that nothing blocks them — it is nobody having looked. A stage computed over this graph would describe the 86 rows somebody assessed, not the system.

| population | assessed | coverage | floor |
|---|---|---|---|
| 183 objects | 86 | **47.0%** | 50.0% |

**Edge basis, never collapsed** — `blocked` 28 · `declared_none` 58 · **`unstated` 96** · `malformed` 1.
...
⚠️ **21 object(s) disagree with their OWN edge basis.** Both halves were already computed and their INTERSECTION was reported nowhere — a signal written and never read.
...
Objects by stage: `QUESTION` 16 · `EVIDENCE` 31 · `DECISION` 18 · `DEPLOYMENT` 26 · `OBSERVATION` 7 · `CAPABILITY` 33 · `INTEGRITY` 43 · `(unstated)` 9

⚠️ **24 of 183 of those stages were assigned in BULK FROM THE SOURCE FILENAME, not by reading the row.** The Phase C migration maps `health-review-backlog.json` → `INTEGRITY` and `{ml,performance,research}-review-backlog.json` → `EVIDENCE`, with no per-row judgement, so `INTEGRITY 43` is a census of ONE filename. Only **156** stage(s) in the whole store were chosen per object — and choosing one is not a claim it is RIGHT, only that a filename did not decide it.

Stage by how the stage was arrived at: `bulk_by_source_file` → EVIDENCE 9, INTEGRITY 15 · `per_object` → (unstated) 6, CAPABILITY 33, DECISION 18, DEPLOYMENT 26, EVIDENCE 22, INTEGRITY 28, OBSERVATION 7, QUESTION 16 · `unstated` → (unstated) 3

**The assessed subgraph — every object that declares an edge (28 of 183):**
...
|---|---|---|---|---|
| 43 | 32.6% | -21.2575 | -21.2575 | **76.7%** |

Provenance split — measured 33 · estimated 10 · fabricated 0 · unverified 0.
...
**7 in flight against a ceiling of 8** (headroom 1) · 31 waiting.
...
- `WO-20260901-PHASE-C` (waiting) — 15d — Phase C — migration, the WIP ceiling, and the priority that reaches a session
- `WO-20260901-PHASE-D` (waiting) — 15d — Phase D — the constraint, computed rather than judged
- `WO-20260902-DECISION-REVIEW-PACKET-POPULATION` (waiting) — 14d — Which POPULATION does the M7 strategy-review gate decide on — real money, or real blended with paper?
- `WO-20260902-EVIDENCE-TRADE-PRIORITISATION-AB` (waiting) — 14d — Confidence-first is the LIVE ranking key for competing trades and has never been shown to pick the better trade — and the A/B that would settle it cannot be run
...
**From the work store: 13 `operator_decision` edge(s).**
...
⚠️ Zero here does NOT mean no decision is pending — it means no object DECLARES one, and 96 of 183 objects have never been assessed for edges at all.
...
**Completeness: `all_sources_read`** · **171 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `probes` | `read` | 9 | freshness=stale age=90.9h cadence=daily (cron 20 5 * * *) \| 4 probe result(s) deferred to the `soaks` source,… |
| `red_crons` | `read` | 4 |  |
| `red_main_runs` | `read` | 2 |  |
| `unlanded_automation` | `read` | 38 |  |
| `error_feed` | `read` | 12 | digest 2026-09-13T05:20:09+00:00, age 78.1h |
| `sunset` | `read` | 12 |  |
| `checklist_unrouted` | `read` | 2 | register_freshness=stale |
| `stuck_branches` | `read` | 1 | 184 branch(es) standing |
| `settled_disposition_owed` | `read` | 0 | settled record grades settled_graded (105 settled row(s)) |
| `spent_decision_edges` | `read` | 4 | 183 object(s) read, 0 unparseable; 12 reported edge(s): 11 spent, 1 unresolvable; 4 on a `waiting` object |
...
- 🔔 `OI-20260906-RESEARCH-THAT-SPECIFIES-WORK-IS-CARRIED-BY-NOTHING` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-OPERATING-LAYER-BUILD-IS-IN-FLIGHT-AND-CARRIED-ONLY-BY-THIS-ROW` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- `OI-20260826-MHG-OVER-COVER-MECHANISM-UNVERIFIED` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- `OI-20260826-SESSION-BRIEF-NEVER-READ-BY-A-FRESH-SESSION` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260826-STRAY-OCA-SWEEP-SHIPPED-BUT-UNARMED` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260826-MGC-JOURNAL-QTY-DIVERGENT-UNOWNED` · 3d — monitoring row 3d since last observation (cadence 2d)
...
- 🔔 `OI-20260829-ALPACA-GOLIVE-BLOCKED-ON-T1-SETTLEMENT-MODEL` · 16d — loud row — must be reported on every session
...
- 🔔 `OI-20260830-E35-GEOMETRY-SHIPPED-TO-9-LEGS-NOT-YET-LIVE-VERIFIED` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260829-E35-REVERSED-LEGS-ARE-A-TIER-3-PROPOSAL-SET-NOT-APPLIED` · 16d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-PER-ACCOUNT-ARBITRATION-SHIPPED-NOT-YET-ARMED-OR-EXERCISED` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-LIVE-WALLET-TRUTH-CANNOT-REPRODUCE-THE-LEDGER-WINDOW` · 16d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` · 16d — monitoring row 16d since last observation (cadence 14d)
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` · 16d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SCOPE-OVERLAP-ATTRIBUTOR-MISREADS-A-SESSIONS-OWN-START` · 16d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LOCAL-LLM-VERDICT-PENDING-A-DIFFERENT-SIZE-ARM` · 15d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-READ-DEBT-11-UNREAD-AND-256-SUPERSEDED-UNREAD` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-PROBE-READERS-SHIPPED-BUT-THE-ACTIONS-ONE-HAS-NEVER-RUN-FOR-REAL` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260831-ALPACA-LIVE-FIRST-REAL-MONEY-LEG-ROUTED-BUT-HAS-NEVER-TRADED` · 3d — monitoring row 3d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-CYCLE-PRIORITY-IS-RENDERED-BUT-NO-SESSION-HAS-ACTED-ON-IT` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-REVIEW-PACKET-CANNOT-PROPOSE-AN-ACTION-AND-ITS-EVIDENCE-BLOCK-IS-UNEXERCISED` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- 🔔 `OI-20260901-CONSTRAINT-READOUT-SHIPPED-AND-IT-REFUSES-NOBODY-HAS-ACTED-ON-THE-REFUSAL` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-ALPACA-SHARE-HOLD-CLASSIFIER-SHIPPED-NOT-YET-OBSERVED` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-DECISION-ROUNDTRIP-SHIPPED-AND-NO-DECISION-HAS-EVER-MADE-THE-ROUND-TRIP` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260901-SUNSET-PASS-SHIPPED-AND-NOTHING-HAS-BEEN-RETIRED` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260901-CLAUDE-CHANNEL-SEPARATION-SHIPPED-BUT-UNPROVEN` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- 🔔 `OI-20260902-TELEGRAM-DECISION-ROUNDTRIP-SHIPPED-AND-NO-TAP-HAS-EVER-BEEN-MADE` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260902-DECISION-PROMPTS-MOVED-TO-CLAUDEBOT-AND-NO-TAP-HAS-LANDED-THERE` · 10d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-CONSTRAINT-READOUT-CRON-SHIPPED-AND-HAS-NEVER-FIRED-ON-SCHEDULE` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- `OI-20260902-SUNSET-CANDIDATES-ARE-NOW-DUE-AND-NINE-ARE-UNDISPOSITIONED` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-REPLAY-PREGATE-CANNOT-FINISH-BECAUSE-THE-TRAINER-IS-OUT-OF-MEMORY` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260902-REAPER-SHIPPED-AND-THE-KILL-PROVED-A-PARTIAL-LOSS-NOT-A-CLEAN-ONE` · 14d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DECISION-DRAIN-ROUTINE-DOES-NOT-EXIST-AND-NOTHING-HAS-EVER-DRAINED` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260902-PR-QUEUE-WATCHER-SHIPPED-AND-ITS-CRON-HAS-NEVER-FIRED` · 5d — monitoring row 5d since last observation (cadence 2d)
...
- 🔔 `OI-20260902-HOURLY-DIGEST-CARRIER-PREPARED-AND-HELD-FOR-A-TIER-2-OK` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260903-OPERATOR-COMMANDS-MOVED-OFF-THE-TRADER-BOT-PREPARED-AND-HELD` · 13d — monitoring row 13d since last observation (cadence 2d)
...
- 🔔 `OI-20260903-CLOSE-WEDGE-LEDGER-MERGED-AND-HAS-NEVER-BEEN-OBSERVED-PRESENT-ON-THE-FLEET` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260904-MANAGER-WAKE-BUILT-AND-ITS-SCHEDULER-DOES-NOT-EXIST` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260905-MI-128-SWEEP-WINDOW-REKEYED-AND-THE-PR-HOLDS-ON-A-BACKFILL-DECISION` · 11d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-5M-DEMOTED-OFF-BYBIT2-AND-NOTHING-HAS-SEEN-IT-STOP` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- 🔔 `OI-20260906-THE-BRACKET-CALIBRATION-INSTRUMENT-EXISTS-AND-ITS-VERDICT-HAS-NOT-BEEN-ACTED-ON` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260906-SCALP-EXIT-HEAD-ARTIFACT-IS-THE-SECOND-MISSING-HALF-NOT-THE-FIRST` · 9d — monitoring row 9d since last observation (cadence 7d)
...
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 10d — monitoring row 10d since last observation (cadence 7d)
...
- 🔔 `OI-20260906-ML2-CALIBRATION-PASSED-BUT-THE-SHARPNESS-RESULT-IS-AGAINST-THE-WRONG-BASELINE` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260907-CLOSE-CONFIRM-RESCOPE-IS-HELD-AND-MI-140-MUST-NOT-BE-FIXED-BEFORE-IT` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-ALPACA-QUANTITY-COVERAGE-SHIPPED-AND-A-56-SHARE-NAKED-POSITION-IS-STANDING` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260908-A-HEDGE-BOOK-FLAT-READ-IS-CLOSING-LIVE-BYBIT-POSITIONS-AND-THE-FLAP-GUARD-CANNOT-SEE-IT` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260908-QLD-TQQQ-EXAMINED-LOCALLY-AND-THE-CI-FEED-PATH-HAS-NEVER-RUN` · 8d — monitoring row 8d since last observation (cadence 7d)
...
- 🔔 `OI-20260909-PROP-STATUS-REQUEST-TRIGGER-DEPLOYED-BUT-THE-SUPPRESSION-HAS-NEVER-BEEN-OBSERVED` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260909-THE-FULL-SYSTEM-AUDITS-TIER-2-AND-TIER-3-PROPOSALS-ARE-FILED-AND-CARRIED-BY-NOTHING` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- 🔔 `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260909-THE-ROLLBACK-REGISTRY-IS-COLLAPSED-AND-THE-ROLLBACK-HAS-NEVER-BEEN-EXERCISED` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-EXIT-EVAL-NEAR-MISS-BAND-AND-RESTART-GAP-SHIPPED-AND-NEITHER-HAS-GRADED-A-LIVE-BOUNDARY` · 6d — loud row — must be reported on every session
...
- 🔔 `OI-20260910-F39-STRANDED-PACKAGE-LEGS-DETECTOR-BUILT-AND-THE-REATTACH-AWAITS-AN-OPERATOR-YES-NO` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-WORKFLOW-PAGE-SHIPPED-AND-THE-OPERATOR-HAS-NOT-SEEN-IT-RENDER` · 6d — monitoring row 6d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-ALPACA-LIVE-OPTION-A-FOUR-LEGS-ADDED-TO-REAL-MONEY-AND-THE-ORDER-PATH-HAS-NEVER-BEEN-EXERCISED` · 3d — monitoring row 3d since last observation (cadence 3d)
...
- `OI-20260910-ORPHAN-ADOPT-SIZE-GATE-DEPLOYED-AND-HAS-REFUSED-NOTHING` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- `OI-20260911-THE-UNROUTED-ROW-DETECTOR-IS-SEEDED-AND-HAS-REPORTED-NOTHING` · 5d — monitoring row 5d since last observation (cadence 2d)
...
- 🔔 `OI-20260911-BLOCKED-LANE-WATCH-SHIPPED-AND-NO-LANE-HAS-BEEN-WOKEN-BY-IT` · 5d — monitoring row 5d since last observation (cadence 3d)
...
- 🔔 `OI-20260911-WEBAPP-AUTH-SECRET-MAP-LANDED-AND-HAS-CARRIED-NO-VALUE` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260911-THE-DIRECTIONAL-LEGS-BROKE-ON-2026-08-30-AND-THE-CAUSE-IS-UNATTRIBUTED` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- 🔔 `OI-20260911-THE-PROP-ACCOUNT-IS-STARVED-NOT-QUIET-AND-THE-UNBLOCK-IS-A-SEQUENCE-NOT-A-FIX` · 4d — monitoring row 4d since last observation (cadence 1d)
...
- 🔔 `OI-20260911-THE-TWO-DETECTORS-ARE-BUILT-AND-NEITHER-HAS-EVER-FIRED-ON-THE-FLEET` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260912-THE-WINNER-SIZE-COLLAPSE-IS-AN-R-COLLAPSE-AND-ITS-MAGNITUDE-IS-NOT-ESTABLISHED` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260911-TIER-2-APPROVED-SELF-LANDING-IS-BUILT-AND-NO-PR-HAS-LANDED-OR-BEEN-REFUSED-BY-IT` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260910-IB-PER-PASS-BREAKER-ARMED-AND-HAS-SKIPPED-NOTHING` · 3d — monitoring row 3d since last observation (cadence 2d)
...
- 🔔 `OI-20260912-MI280-SHIPPED-EIGHT-LANDING-AND-REGISTER-INSTRUMENTS-AND-MOST-HAVE-NEVER-BEEN-SEEN-WORKING` · 4d — monitoring row 4d since last observation (cadence 3d)
...
- 🔔 `OI-20260912-FANOUT-APPLY-PATH-REPAIRED-AND-STILL-HAS-NEVER-DISPATCHED` · 4d — monitoring row 4d since last observation (cadence 2d)
...
- 🔔 `OI-20260912-BYBIT-SYMBOL-DEDUPE-REPAIRED-AND-NO-FLEET-READ-HAS-BEEN-SEEN-WITHOUT-THE-DROP` · 3d — monitoring row 3d since last observation (cadence 2d)
...
- 🔔 `OI-20260913-A-BYBIT2-HEDGE-BOOK-IS-NEVER-FETCHED-AND-NO-SURFACE-IN-THIS-REPO-CAN-SAY-WHETHER-IT-IS-STILL-THERE` · 3d — monitoring row 3d since last observation (cadence 1d)
  - OPERATOR-RAISED 2026-09-12: a real-money ETHUSDT SHORT 0.05 on bybit_2 open at the venue with no stop and no take-profit, invisible to every bot surface. Filed as BL-20260912-BYBI…

**`soaks` — 4 due**

- `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 14d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: verdicts_differ=true. This is a KNOWN, DECLARED g…
  - #10746 ships a soak at runtime_logs/bybit_coverage_soak.jsonl whose rows are the ONLY declared evidence for the operator's conditional widening of the graded-coverage gate to bybi…
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 10d — soak READY — ✅ READY — 40 matching row(s) of 1000 scanned satisfy the declared criterion (decision_state!=not_scored). ⚠️ Ready is not CLEA…
  - MI-150 shipped the M20 exit-head consumer into `ict_scalp` (annotate-only, disarmed) -- the wiring MI-146 identified as the only passed-gate work blocked on missing code. ⚠️ IT CA…
- `OI-20260909-POSITION-READ-STATE-SOAK-DECIDES-WHETHER-THE-COLLAPSED-READ-EVER-FIRES` · 7d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: dropped_symbol_dedupe_count>=1. This is a KNOWN, …
  - MI-222 Tier-2 (operator-approved observable-first, 2026-09-09) made the collapsed position read at src/units/accounts/clients.py COUNTABLE without changing what any caller receive…
- `OI-20260909-CLOSED-FLAT-INVARIANT-CAN-FINALLY-SPEAK-AND-HAS-NOT-YET-SPOKEN` · 7d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: examined>=1. This is a KNOWN, DECLARED gap (`prob…
  - The closed->exchange-flat invariant -- THE ONE MECHANISM that can independently contradict 'this trade is closed' -- was silent for four independent reasons, each alone sufficient…
...
- 🔔 `PROBE-REPORT-STALE` · 3d — EVERY probe verdict below was observed at 2026-09-12T16:29:18.870030+00:00, NOT today. The probes job did not land a fresh report, so treat…
  - probe report is 90.9h old — expected daily (cron 20 5 * * *)
- `OI-20260903-CAPTURE-WATCH-PROVED-ITSELF-ON-DEMAND-AND-ITS-SCHEDULE-HAS-NEVER-SUCCEEDED` — we did not look — this row is currently unwatched [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` — probe FAILED — its declared observation did not hold [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` — probe FAILED — its declared observation did not hold [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
...
- `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` — we did not look — this row is currently unwatched [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
  - probe could not run (exit_2)
- `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` — we did not look — this row is currently unwatched [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` — probe FAILED — its declared observation did not hold [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
...
- `OI-20260902-SUNSET-CANDIDATES-ARE-NOW-DUE-AND-NINE-ARE-UNDISPOSITIONED` — we did not look — this row is currently unwatched [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-DECISION-DRAIN-ROUTINE-DOES-NOT-EXIST-AND-NOTHING-HAS-EVER-DRAINED` — probe FAILED — its declared observation did not hold [observed 2026-09-12T16:29:18.870030+00:00, 90.9h ago]
...
**`unlanded_automation` — 38 due**

- 🔔 `#12330` · 0d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12328` · 0d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12326` · 0d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12320` · 0d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12316` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12312` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12306` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12299` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12298` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12297` · 1d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12295` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12289` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12284` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12279` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12276` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12269` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12268` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12265` · 2d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12263` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12257` · 2d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12253` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12249` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12245` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12239` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12231` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#12230` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#12229` · 3d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#12228` · 3d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#12225` · 3d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11998` · 4d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#11956` · 4d — producer output opened a PR that has not landed
  - chore(m28): valuation snapshots (auto)
- 🔔 `#11953` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11952` · 4d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11943` · 4d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11939` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#11919` · 4d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#11916` · 4d — producer output opened a PR that has not landed
  - chore(ml): replay pre-gate fleet report (auto)
- 🔔 `#11912` · 4d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)

**`error_feed` — 12 due**

- 🔔 `ERROR-FEED-DIGEST-STALE` · 3d — EVERY group below was observed at 2026-09-13T05:20:09+00:00, NOT now. The digest workflow did not land a fresh run, so an absent condition …
  - error-feed digest is 78.1h old — expected hourly
- `ERRFEED-9da85381` · 6d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 58 rows 2026-09-10T09:07:19 → 2026-09-13T04:13:49 · symbo…
...
- `ERRFEED-fa77ad80` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 51 rows 2026-09-09T16:16:17 → 2026-09-12T22:59:19 · symbo…
...
- `ERRFEED-a46d2970` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 51 rows 2026-09-09T16:16:17 → 2026-09-12T22:59:19 · symbo…
...
- `ERRFEED-72e2ad1e` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 42 rows 2026-09-09T10:03:15 → 2026-09-11T23:15:47 · accou…
...
- `ERRFEED-f754bb5d` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 11 rows 2026-09-09T06:44:24 → 2026-09-10T05:45:10 · symbo…
...
- `ERRFEED-c8553cf1` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 5 rows 2026-09-09T15:42:30 → 2026-09-11T13:55:56 — decide…
...
- `ERRFEED-585012de` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 4 rows 2026-09-09T12:04:19 → 2026-09-12T02:07:14 · accoun…
...
- `ERRFEED-6f2d9848` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 4 rows 2026-09-09T09:38:26 → 2026-09-11T09:38:06 · accoun…
...
- `ERRFEED-831dac68` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 3 rows 2026-09-09T09:48:24 → 2026-09-11T15:57:45 · accoun…
...
- `ERRFEED-0aa1e4e0` · 4d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 3 rows 2026-09-12T05:05:31 → 2026-09-12T11:07:25 · accoun…
...
- `strategy:avax_pullback_2h` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · avax_pullback_2h
- `strategy:fade_breakout_4h` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · fade_breakout_4h
- `strategy:fvg_range_15m` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · fvg_range_15m
- `strategy:htf_pullback_trend_2h` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · htf_pullback_trend_2h
- `strategy:ief_pullback_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · ief_pullback_1d
- `strategy:iwm_trend_long_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · iwm_trend_long_1d
- `strategy:mhg_pullback_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · mhg_pullback_1d
- `strategy:qld_trend_long_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · qld_trend_long_1d
- `strategy:qqq_trend_long_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · qqq_trend_long_1d
- `strategy:slv_pullback_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · slv_pullback_1d
- `strategy:slv_trend_1h` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · slv_trend_1h
- `strategy:tlt_pullback_1d` · 15d — sunset pass proposed RETIRE (persistently_silent) and no disposition is recorded — Tier-3, so propose, never enact
  - strategy_leg · tlt_pullback_1d

**`checklist_unrouted` — 2 due**

- 🔔 `checklist-register-stale` · 5d — docs/claude/work/CHECKLIST-ROUTING-AGE.json was generated at 2026-09-11T10:39:09+00:00, and `.github/workflows/constraint-readout.yml` writ…
  - the unrouted-row register is 121h old — its producer has not run
...
- 🔔 `stuck-branches-receipt-stale` · 3d — age 90.9h against daily (cron 20 5 * * *). A receipt that is not current cannot tell you whether a branch stranded today — and the carrier …
  - the stuck-branch receipt is stale
...
- 🔔 `manager-queue-watch-never-ran-overdue` · 13d — armed 333.5h ago, on the order of 333 expected firings, and ZERO receipts have ever been committed. The Routine reports SUCCEEDED, so nothi…
