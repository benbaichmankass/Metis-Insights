...
_Generated 2026-09-09T11:11:47+00:00 by `scripts/ops/constraint_readout.py` · cycle `CY-20260906-TRADING-TRUTH` (basis DECIDED)_
...
59 of 697 objects (8.5%) have an ASSESSED `blocked_on` basis, below the declared floor of 50%. **No stage is named.** 638 objects carry an empty `blocked_on` that is NOT a claim that nothing blocks them — it is nobody having looked. A stage computed over this graph would describe the 59 rows somebody assessed, not the system.

| population | assessed | coverage | floor |
|---|---|---|---|
| 697 objects | 59 | **8.5%** | 50.0% |

**Edge basis, never collapsed** — `blocked` 24 · `declared_none` 35 · **`unstated` 638** · `malformed` 0.
...
Objects by stage: `QUESTION` 16 · `EVIDENCE` 94 · `DECISION` 13 · `DEPLOYMENT` 24 · `OBSERVATION` 6 · `CAPABILITY` 24 · `INTEGRITY` 516 · `(unstated)` 4

⚠️ **576 of 697 of those stages were assigned in BULK FROM THE SOURCE FILENAME, not by reading the row.** The Phase C migration maps `health-review-backlog.json` → `INTEGRITY` and `{ml,performance,research}-review-backlog.json` → `EVIDENCE`, with no per-row judgement, so `INTEGRITY 516` is a census of ONE filename. Only **120** stage(s) in the whole store were chosen per object — and choosing one is not a claim it is RIGHT, only that a filename did not decide it.

Stage by how the stage was arrived at: `bulk_by_source_file` → EVIDENCE 78, INTEGRITY 498 · `per_object` → (unstated) 3, CAPABILITY 24, DECISION 13, DEPLOYMENT 24, EVIDENCE 16, INTEGRITY 18, OBSERVATION 6, QUESTION 16 · `unstated` → (unstated) 1

**The assessed subgraph — every object that declares an edge (24 of 697):**
...
|---|---|---|---|---|
| 41 | 36.6% | -7.857 | -7.857 | **68.3%** |

Provenance split — measured 28 · estimated 13 · fabricated 0 · unverified 0.
...
**6 in flight against a ceiling of 8** (headroom 2) · 21 waiting.
...
⚠️ Zero here does NOT mean no decision is pending — it means no object DECLARES one, and 638 of 697 objects have never been assessed for edges at all.
...
**Completeness: `all_sources_read`** · **149 row(s) due** across the sources that answered and are not already covered above (via `scripts/ops/render_due_list.py`).
...
| `probes` | `read` | 10 | freshness=stale age=48.7h cadence=daily (cron 20 5 * * *) \| 2 probe result(s) deferred to the `soaks` source,… |
| `red_crons` | `read` | 11 |  |
| `unlanded_automation` | `read` | 52 |  |
| `error_feed` | `read` | 12 | digest 2026-09-08T05:10:01+00:00, age 30.0h |
| `sunset` | `read` | 0 |  |
...
- 🔔 `OI-20260906-BYBIT-CLOSE-QTY-SNAP-MERGED-BUT-NEVER-OBSERVED-ON-THE-FLEET` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260829-ALPACA-GOLIVE-BLOCKED-ON-T1-SETTLEMENT-MODEL` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260829-E35-REVERSED-LEGS-ARE-A-TIER-3-PROPOSAL-SET-NOT-APPLIED` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LIVE-WALLET-TRUTH-CANNOT-REPRODUCE-THE-LEDGER-WINDOW` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` · 9d — monitoring row 9d since last observation (cadence 7d)
...
- 🔔 `OI-20260831-SCOPE-OVERLAP-ATTRIBUTOR-MISREADS-A-SESSIONS-OWN-START` · 9d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-LOCAL-LLM-VERDICT-PENDING-A-DIFFERENT-SIZE-ARM` · 8d — loud row — must be reported on every session
...
- 🔔 `OI-20260831-RESEARCH-READ-DEBT-11-UNREAD-AND-256-SUPERSEDED-UNREAD` · 9d — monitoring row 9d since last observation (cadence 7d)
...
- 🔔 `OI-20260901-REVIEW-PACKET-CANNOT-PROPOSE-AN-ACTION-AND-ITS-EVIDENCE-BLOCK-IS-UNEXERCISED` · 0d — loud row — must be reported on every session
...
- 🔔 `OI-20260901-SUNSET-PASS-SHIPPED-AND-NOTHING-HAS-BEEN-RETIRED` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-TELEGRAM-DECISION-ROUNDTRIP-SHIPPED-AND-NO-TAP-HAS-EVER-BEEN-MADE` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-DECISION-PROMPTS-MOVED-TO-CLAUDEBOT-AND-NO-TAP-HAS-LANDED-THERE` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-REAPER-SHIPPED-AND-THE-KILL-PROVED-A-PARTIAL-LOSS-NOT-A-CLEAN-ONE` · 7d — loud row — must be reported on every session
...
- 🔔 `OI-20260902-API-REFERENCE-SPLIT-OUT-OF-CLAUDE-MD-AND-NOT-YET-FOUND-AT-USE` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260903-OPERATOR-COMMANDS-MOVED-OFF-THE-TRADER-BOT-PREPARED-AND-HELD` · 6d — monitoring row 6d since last observation (cadence 2d)
...
- 🔔 `OI-20260904-MANAGER-WAKE-BUILT-AND-ITS-SCHEDULER-DOES-NOT-EXIST` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260905-MI-128-SWEEP-WINDOW-REKEYED-AND-THE-PR-HOLDS-ON-A-BACKFILL-DECISION` · 4d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-ICT-SCALP-5M-DEMOTED-OFF-BYBIT2-AND-NOTHING-HAS-SEEN-IT-STOP` · 1d — monitoring row 1d since last observation (cadence 1d)
...
- 🔔 `OI-20260906-THE-BRACKET-CALIBRATION-INSTRUMENT-EXISTS-AND-ITS-VERDICT-HAS-NOT-BEEN-ACTED-ON` · 3d — loud row — must be reported on every session
...
- 🔔 `OI-20260906-SCALP-EXIT-HEAD-ARTIFACT-IS-THE-SECOND-MISSING-HALF-NOT-THE-FIRST` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260907-TELEMETRY-HOOK-IS-APPROVED-AND-GREEN-IN-AN-UNMERGED-PR-AND-OBSERVED-ON-NOTHING` · 2d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-QLD-TQQQ-EXAMINED-LOCALLY-AND-THE-CI-FEED-PATH-HAS-NEVER-RUN` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-CANCEL-ACCEPTED-INEFFECTIVE-MERGED-AND-NEVER-EMITTED-ON-THE-FLEET` · 1d — loud row — must be reported on every session
...
- 🔔 `OI-20260908-THE-REAL-MONEY-CLOSED-POPULATION-IS-447-NOT-15-AND-TWO-SURFACES-DISAGREE-ON-ITS-EXIT-LABELS` · 1d — loud row — must be reported on every session
...
- `OI-20260902-BYBIT-GRADED-COVERAGE-SOAK-IS-THE-ONLY-EVIDENCE-FOR-WIDENING-AND-NOTHING-WATCHES-IT` · 7d — soak UNKNOWN — NO PROBE IS DECLARED for this soak, so nothing is reading it. Ready means: verdicts_differ=true. This is a KNOWN, DECLARED g…
  - #10746 ships a soak at runtime_logs/bybit_coverage_soak.jsonl whose rows are the ONLY declared evidence for the operator's conditional widening of the graded-coverage gate to bybi…
- 🔔 `OI-20260906-ICT-SCALP-EXIT-HEAD-CONSUMER-SHIPPED-AND-CANNOT-SCORE-UNTIL-A-SCALP-HEAD-IS-PUBLISHED` · 3d — soak READY — ✅ READY — 37 matching row(s) of 754 scanned satisfy the declared criterion (decision_state!=not_scored). ⚠️ Ready is not CLEAR…
  - MI-150 shipped the M20 exit-head consumer into `ict_scalp` (annotate-only, disarmed) -- the wiring MI-146 identified as the only passed-gate work blocked on missing code. ⚠️ IT CA…
...
- 🔔 `PROBE-REPORT-STALE` · 2d — EVERY probe verdict below was observed at 2026-09-07T10:31:05.344186+00:00, NOT today. The probes job did not land a fresh report, so treat…
  - probe report is 48.7h old — expected daily (cron 20 5 * * *)
- `OI-20260903-CAPTURE-WATCH-PROVED-ITSELF-ON-DEMAND-AND-ITS-SCHEDULE-HAS-NEVER-SUCCEEDED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260831-PROP-RISK-GATE-ENFORCE-ARMED-BUT-HAS-NEVER-CAPPED` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
...
- 🔔 `OI-20260831-RESEARCH-QUEUE-INFEASIBLE-STATE-SHIPPED-BUT-NEVER-REACHED-LIVE` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
...
- `OI-20260831-RESEARCH-QUEUE-GPU-ROUTE-AND-SPEND-GATE-NEVER-EXERCISED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
  - probe could not run (exit_2)
- `OI-20260831-SESSION-BRIEF-DIFF-SCOPING-SHIPPED-NEVER-REPORTED-INHERITED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-BYBIT-COVERAGE-BASIS-MERGED-WITH-ARMING-DELIBERATELY-HELD-FOR-A-SOAK` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
...
- `OI-20260902-SUNSET-CANDIDATES-ARE-NOW-DUE-AND-NINE-ARE-UNDISPOSITIONED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
  - probe could not run (exit_2)
- 🔔 `OI-20260902-DECISION-DRAIN-ROUTINE-DOES-NOT-EXIST-AND-NOTHING-HAS-EVER-DRAINED` — probe FAILED — its declared observation did not hold [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
...
- `OI-20260902-DIGEST-MOVED-OFF-A-CRON-THAT-DOES-NOT-FIRE-AND-THE-NEW-TRIGGER-HAS-NEVER-FIRED` — we did not look — this row is currently unwatched [observed 2026-09-07T10:31:05.344186+00:00, 48.7h ago]
  - probe could not run (exit_2)
...
- 🔔 `work-digest` — latest scheduled run concluded 'failure'
  - work-digest

**`unlanded_automation` — 52 due**

- 🔔 `#11536` · 0d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11525` · 0d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)
- 🔔 `#11518` · 0d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11512` · 0d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11506` · 0d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11499` · 0d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11493` · 0d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11490` · 0d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11489` · 0d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11487` · 0d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11478` · 0d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11476` · 0d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11475` · 0d — producer output opened a PR that has not landed
  - chore(m1): economic-calendar PIT snapshots (auto)
- 🔔 `#11471` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11470` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11468` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11465` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11462` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11459` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11458` · 1d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11456` · 1d — producer output opened a PR that has not landed
  - chore(ops): trainer capture-watch receipt (auto)
- 🔔 `#11454` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11453` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11452` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11449` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11448` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11445` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11444` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11442` · 1d — producer output opened a PR that has not landed
  - chore(ops): queue the daily work digest (auto)
- 🔔 `#11439` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11438` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11432` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11429` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11428` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the A1 constraint readout (auto)
- 🔔 `#11422` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11420` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11416` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11408` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11404` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11402` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11399` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11397` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11393` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11386` · 1d — producer output opened a PR that has not landed
  - chore(m28): valuation snapshots (auto)
- 🔔 `#11382` · 1d — producer output opened a PR that has not landed
  - chore(ops): PR-queue watcher receipt (auto)
- 🔔 `#11376` · 1d — producer output opened a PR that has not landed
  - chore(ops): session-reaper observations (auto)
- 🔔 `#11368` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the due-list (auto)
- 🔔 `#11367` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh the error-feed digest (auto)
- 🔔 `#11366` · 1d — producer output opened a PR that has not landed
  - chore(ops): refresh probe results (auto)
- 🔔 `#11362` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)
- 🔔 `#11361` · 1d — producer output opened a PR that has not landed
  - chore(m7): strategy review packets (auto)
- 🔔 `#11353` · 1d — producer output opened a PR that has not landed
  - chore(ops): reconcile OPEN-PRS.json after merge (auto)

**`error_feed` — 12 due**

- 🔔 `ERROR-FEED-DIGEST-STALE` · 1d — EVERY group below was observed at 2026-09-08T05:10:01+00:00, NOT now. The digest workflow did not land a fresh run, so an absent condition …
  - error-feed digest is 30.0h old — expected hourly
- `ERRFEED-b0711dd4` · 2d — error-level condition on `bot_logs`, FIRST SEEN since the last digest — 3 rows 2026-09-07T12:00:59 → 2026-09-08T00:05:05 · accounts=ib_pape…
...
- `ERRFEED-897ca384` · 1d — error-level condition on `bot_logs`, FIRST SEEN since the last digest — 1 rows 2026-09-08T05:07:41 → 2026-09-08T05:07:41 · accounts=alpaca_…
...
- `ERRFEED-cf5a4edf` · 2d — error-level condition on `bot_logs`, FIRST SEEN since the last digest — 1 rows 2026-09-07T11:49:36 → 2026-09-07T11:49:36 · accounts=ib_pape…
...
- `ERRFEED-725f746a` · 1d — error-level condition on `operator_alerts`, FIRST SEEN since the last digest — 1 rows 2026-09-08T02:13:28 → 2026-09-08T02:13:28 · accounts=…
...
- `ERRFEED-86505576` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 155 rows 2026-09-02T08:33:53 → 2026-09-04T23:53:07 · acco…
...
- `ERRFEED-02a919f1` · 5d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 55 rows 2026-09-04T08:01:23 → 2026-09-07T23:25:05 · accou…
...
- `ERRFEED-9fd2fc82` · 15d — error-level condition on `bot_logs`, STANDING (predates the last digest) — 49 rows 2026-08-25T05:13:10 → 2026-08-25T22:48:45 · symbols=MGC …
  - [error] x49 strategy_builder exception: RuntimeError: ict_scalp_mgc_Nm: no candle data for symbol=MGC timeframe=Nm.
- `ERRFEED-6eb9d669` · 8d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 27 rows 2026-09-01T20:12:41 → 2026-09-02T08:17:06 · accou…
...
- `ERRFEED-a46d2970` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 25 rows 2026-09-02T02:03:23 → 2026-09-07T22:49:26 · symbo…
...
- `ERRFEED-fa77ad80` · 7d — error-level condition on `operator_alerts`, STANDING (predates the last digest) — 20 rows 2026-09-02T02:03:23 → 2026-09-07T22:49:25 · symbo…
