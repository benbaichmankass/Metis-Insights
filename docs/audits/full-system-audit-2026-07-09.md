...
- STATUS (2026-07-09): **RESOLVED — already shipped by PR #6029 (`9c3987d`), verified on `main`.** Every blocking read route across 23 routers is now a plain `def` (threadpooled) or an `async def` that offloads via `asyncio.to_thread` (`get_account_balances` plain `def`; `prop.post_report` offloads `ingest_report`). `PRAGMA busy_timeout=3000` in both connect paths. The `scripts/ci/check_async_route_blocking.py` AST guard is the `async-route-blocking-guard` CI merge gate (green) + tests. Backlog `BL-20260707-HEALTHAPI-ACCTBAL-BLOCKING-DB` → resolved. (Spec predated verifying against `main`.)
...
- STATUS (2026-07-09): **RESOLVED — token provisioned + fail-closed code MERGED + DEPLOYED.** Operator set `DASHBOARD_API_TOKEN` in the Actions secret + the Streamlit dashboard Secrets; synced to the live VM `.env` via `sync-vm-secrets` (#6069, ict-web-api restarted). Fail-closed code shipped as PR #6070 (squash `867eec0`), deployed via `pull-and-deploy` #6071 (VM HEAD `a0d8f22`→`867eec0`, ict-web-api active). **Simplification vs the spec:** rather than a new `dashboard_auth.py` module, the fix was contained to `prop.py` — `_check_admin_token` → fail-closed `_require_write_token` on `POST /prop/report` only (unset→503 / missing|wrong bearer→401 / match→pass). The read surface + Android device-register write keep their intended-permissive gates. Gate logic unit-verified 6/6 + CI route tests green (`test_router_post_fail_closed_when_token_unset` added; happy-path + bad-body tests authenticate under the gate). The token being set already armed the gate under the pre-existing enforce-when-set code; this change hardens the *unset* case so a future `.env` drop can't silently reopen the anonymous-write hole. Backlog `BL-20260705-DASHBOARD-API-TOKEN-UNSET` → resolved.
...
- STATUS (2026-07-09): **RESOLVED — verified on `main`.** `prop_journal` + `account_rulesets` delegate to `is_prop_account`; the telegram handler carries no own classifier. The CI guard already exists: `scripts/ci/check_prop_identity_single_home.py` (`prop-identity-guard.yml`, green) forbids re-deriving the prop funding-class by string-compare anywhere in `src/` (`account_class/type == "prop"`) and inside `src/prop/` (`exchange == "breakout"`), explicitly allowing connector-dispatch `exchange == "breakout"` in `execute.py`/`coordinator.py`. Backlog already resolved. Nothing to build.

### LIVE INCIDENT (2026-07-09, operator-flagged) — MGC monitor-blind + IB gateway wedge

An `ib_paper` MGC short (`mgc_pullback_1d`, pkg-a4a957610253455e, order 188, −9 @ 4128.91) tripped **MONITOR BLIND** (`candles_unavailable`, 3 ticks). Root cause: the ib-gateway container restarted ~22:15Z and the trader's IB **exec** client (498) could not re-establish — `connect_failed: TimeoutError`, breaker re-opening, `consecutive_failures` 3→8, cooldown 69→119s — staying wedged ~18 min (MES/MGC/MHG market data suppressed → MGC dynamic-exit monitor blind), while the readonly + a sibling exec client stayed fine. **Recovered** via `vm-ib-gateway-recover` (#6075, docker-restart); exec client reconnected cleanly at 22:37:25 (data farms OK, Synchronization complete), breaker closed, MGC monitor un-blinding. **Two structural findings logged:**
- **`BL-20260709-IB-BROKER-PROTECTION-UNVERIFIED` (Tier-3, guardrail — HELD for supervised build):** no IB analogue of the Alpaca broker-side protection verification (`has_protective_orders`), so a monitor-blind IB position rides on an *unverified* broker bracket. Proposed: IB `has_protective_orders` (account-wide `reqAllOpenOrders`, fail-safe) + a **cadence-gated** (NOT per-tick — avoid the BL-20260609 wedge class) broker-naked sweep reusing `_attempt_naked_autoprotect`. Deliberately not shipped unsupervised overnight (order-placing path; needs live IB cross-client order-visibility validation).
- **`BL-20260709-IB-POSTRESTART-RECONNECT-WEDGE` (Tier-2):** the exec client fails to cleanly reconnect after a gateway restart (distinct from the RISK-2 warm-up wedge — this is `connect_failed` at connect(), not warm-up); ~18-min wedge until a second manual gateway restart. Durable fix TBD (morning).
...
- STATUS (2026-07-09): **MERGED + DEPLOYED + live-verified.** Operator-approved → PR #6056 (squash `c6694e8`), deployed via `pull-and-deploy` #6061 (`/api/diag/version` git_sha=`c6694e8d`). Shipped as specified: `AlpacaClient.position_present(symbol)` (404=False / 2xx=True / err=None), `clients.account_position_present()` + `supports_position_presence()` (capability gate, **alpaca only** — IB/OANDA keep the 2-obs LIST path, bounded blast radius), reconciler closes on absence ONLY when `present is False`, ≥3 reset batch is **alert-first + latched** (never mass-closes on inference). 19 in-file + 169 cross-suite tests pass; live-verified clean ticks, zero spurious closes. Behaviour change recorded: suspected reset alerts + leaves rows OPEN (was auto-close `exchange_reset_flat`).
...
- STATUS (2026-07-09): **RESOLVED (BL-20260709 closed).** The char-slice write-side was already eliminated (dump_capped migration; the "4 leftover sites" did not exist on `main`). Shipped this pass: (a) `json_notes.sanitize_nonfinite` so `dump_capped` also never emits `NaN`/`Infinity` (a LATENT second invalid-JSON class — PR #6055 `fce8d7d`); (b) rowid fix + nonfinite-aware, **lossless** reparse in `repair_malformed_notes.py` (#6052/#6055); (c) the one-shot repair APPLIED on the live DB — dry-run classified all **1085** json_valid=0 rows as **truncation** (49 trades.notes + 1036 order_packages.signal_logic; **the NaN hypothesis was DISPROVED by the classifier before applying** — honesty gate worked), apply → `remaining malformed rows = 0`, recovering `closed_at` on the trades.notes rows that unblock `closed_flat_invariant`/INV-6. Backlog entry closed (PR #6060). CI guard `json-notes-cap-guard` in place.
...
- STATUS (2026-07-09): **MERGED + DEPLOYED (operator-approved).** PR #6044 (squash `e32ff11`), merged by the operator 16:18 UTC; `e32ff11` is an ancestor of the deployed `c6694e8` (confirmed via `/api/diag/version` git_sha=`c6694e8d`), so it is live in production. connect() warm-up now best-effort (logs + leaves `_account_data_ready=False` + returns the live handle; breaker no longer trips on warm-up timeout — only connect_failed + liveness_probe_timeout still do). Added `_bounded_account_summary` (async-bounded, stub/loop-guarded like `_probe_liveness`) and rewired `balance()` to it (raises IBConnectionError on timeout → DB-snapshot fallback). `connection_state()` reports `connected` for a probe-live handle regardless of `account_data_ready` (the field still reports the real warm-up state). Inverted the `test_warmup_never_arrives_*` breaker test → `…_does_not_condemn_connection`; added `test_balance_read_is_bounded_and_raises_on_timeout` + `test_connection_state_connected_without_account_data`; the liveness-condemns test is untouched. `test_ib_integration` + `test_ib_connection_state` = 79 passed; ruff clean. Merge only on explicit operator OK.
