...
| D | Netting partial-close phantom reconcile | **SHIPPED, operator-gated (NOT auto-applied)** | `reconcile_netting_rows.py` pure planner, dry-run default (#8401); `reconcile-netting-rows` **Tier-2 operator-gated system-action** (#8403). `BL-20260801-NETTING-PARTIAL-CLOSE-ROWS-NEVER-REDUCED` correctly stays open pending an operator `--apply` run. |
