...
## Implementation addendum (2026-05-31, operator-approved)

The Tier-3 PR that ships the "hold" policy lives at
[`#TBD`](https://github.com/benbaichmankass/ict-trading-bot/pulls) (drafted off this
verdict). Two refinements vs the design-proposal prose above:

1. **Code lands in `compute_execution_delta`, not `aggregate_intents`.**
   The proposal said "aggregator returns `DesiredPosition(side=current.side, …)`",
   but the walk-forward harness `scripts/backtest_system.py` actually applied
   `--flip-policy hold` at the *executor* layer (after the aggregator already
   voted) — it compared `pos.side != desired.side` and skipped the flip leg.
   The live equivalent is `compute_execution_delta`, which is the single place
   the per-account current position meets the aggregator output. Putting the
   change there keeps the aggregator pure / deterministic / account-agnostic
   (it still produces the same `DesiredPosition` regardless of which account
   holds what) and gives us one obvious point to toggle policy without
   plumbing position-state into the multiplexer. The walk-forward verdict
   transfers cleanly because the harness modeled it at exactly this layer.

2. **Default at `compute_execution_delta` stays `"reverse"`; the live default
   is set at the caller via env var.** Two reasons. (a) Existing tests of
   `compute_execution_delta` / `compute_execution_delta_for_package` keep
   their `action="flip"` expectations without modification — the new
   behaviour is one explicit kwarg away. (b) The live default of `"hold"`
   is set in `src/core/coordinator.py::multi_account_execute` by reading
   `INTENT_CONFLICT_POLICY` (default `"hold"`). An operator can roll back
   the live behaviour to `"reverse"` without a redeploy by setting
   `INTENT_CONFLICT_POLICY=reverse` on the systemd unit — same operator
   escape valve as `MULTI_STRATEGY_INTENT_LAYER`.

The 4 unit tests called for in § "Pre-merge validation" landed as 8 tests
in `tests/test_intent_delta_dispatch.py::TestConflictPolicyHold` (the
extra 4 cover the symmetric short-to-long case, the close-on-flat-isn't-
a-conflict case, the same-side-already-at-target case, and the
aggregator-records-dropped-intents audit-trail case) + one coordinator-
level test confirming the dispatcher noops under the default policy and
records the rejection with reason `intent_noop:conflict_hold:…`. The
existing flip-dispatch test was preserved by pinning
`INTENT_CONFLICT_POLICY=reverse` in its `monkeypatch.setenv`. All 88
intent + multi-account-execute tests stay green; ruff clean.

The harness re-verification (item 3 in § "Pre-merge validation") is a
no-op: the harness applies "hold" at the executor layer, which is what
the live code now does. Cell-by-cell numbers in the result tables above
are unchanged by definition. The activation order in § "Activation order"
is unchanged: PR merges → demo soak on bybit_1 → flip bybit_2 (or rely
on env-var rollback if anything looks wrong).
