...
## Addendum — FULL 6-member live-roster coverage (2026-05-30, verified)

Coverage extended from 4 to all **6** BTCUSDT live members so the system test
matches the real bybit_2 book. turtle_soup (15m) plugs in directly (its live
order_package adapter is single-TF; the legacy 1m-entry confirmation is not in
that path). ict_scalp_5m (5m) required injecting the 1h EMA-20 HTF bias per bar,
vectorized via pd.merge_asof (the first attempt was O(n^2) and stalled the
5m/6y stream; fixed in commit 32aa31d, verified identical output — H1-2024
ict_scalp = 166 signals either way). Same window/config (2020-06..2026-02,
$10k, risk 0.3%, daily-loss cap 3%, 15m clock). Numbers below were read directly
from a completed clean-slate run (run6_result.json) before writing.

| flip policy | net | ret% | maxDD% | ret/DD | trades | flips |
|---|---|---|---|---|---|---|
| **reverse (LIVE)** | **−$1928** | −19.28% | 21.53% | −0.90 | 3637 | 1218 |
| **hold** | **−$281** | −2.81% | 9.60% | −0.29 | 1894 | 0 |
| flat | −$1024 | −10.24% | 14.00% | −0.73 | 2154 | 671 |
...
- **flat:**    trend −171, fade −454, squeeze −114, **fvg +167**, turtle −445, ict_scalp −163

### The flip-churn finding gets STRONGER with the full roster

1. **More voters = more whipsaw under close-and-reverse.** The 6-member book
   does **1218 flips** (vs the 4-member's 246) and loses **−$1928 (−19.3%)** at
   21.5% max-drawdown — far worse than the 4-member −$411. Adding strategies to
   the current intent layer makes the book *worse*, not better: each new member
   is another source of opposite votes that tear the shared position.
2. **"hold" again dominates decisively** — net −$281 vs −$1928 (a **+$1647
   swing**), maxDD **more than halved** (21.5% → 9.6%), zero flips. Confirmed on
   the full roster: letting the position-holder ride to its own SL/TP/trail
   instead of being whipsawed is the single biggest portfolio lever found.
3. **ict_scalp_5m is the clearest churn victim:** −$772 under reverse →
   **+$408 under hold** — an $1180 swing on one strategy. Its 5m signals are the
   most frequent, so it both triggers and absorbs the most flips; left to ride
   its own exits it becomes the book's best contributor.
4. **fvg_range_15m stays net-positive under all three policies** (+167 / +78 /
   +167) — a robust diversifier even in the larger, churnier book; it never
   *costs* the portfolio.
5. **"hold" still leaves the book net-negative (−$281).** Removing the
   self-inflicted churn is necessary but not sufficient — turtle_soup (−443) and
   fade (−206) remain in-system drags even when not whipsawed. So a real decider
   that also *allocates* (down-weights persistent in-system losers, not just
   de-conflicts) is the larger prize beyond the conflict-policy fix.

### Bottom line

The headline holds and strengthens across both 4- and 6-member rosters: **the
live close-and-reverse conflict policy is the single largest drag on the
portfolio, and it scales up with roster size.** Strongest argument yet for the
single-account decider (`docs/sprint-plans/DECIDER-SINGLE-ACCOUNT-2026-05-24.md`)
— its first job a "hold / don't-whipsaw" conflict policy, its second a capital
allocator. Tier-3 caveats stand: this backtest justifies *investigating* an
`aggregate_intents` change (walk-forward first); it does not authorise patching
the live invariant.
