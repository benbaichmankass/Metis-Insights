...
|----|------|----------|----------|-------|
| D1 | ~~**pipeline.py monolith**~~ **RESOLVED** | — | — | **PR-6** (#1107) signal builders → `strategy_signal_builders.py`. **PR-8** (#1109) strategy-monocle gates → `strategy_monocle.py`, formatter → `pipeline_result.py`. **PR-9** (#1110) `_signal_to_order_package` → `order_bridge.py`, `_write_ict_signals_from_meta` → `signal_writer.py`. pipeline.py: 1412 → 626 lines (−56%). Remaining `run_pipeline` orchestrator + `multiplexed_signal_builder` are tightly coupled to the module; further split has diminishing returns. |
...
| D3 | ~~**telegram_query_bot.py partially split**~~ **RESOLVED** | — | — | **PR-4** (#1106) extracted `trade_notifier.py` + `cloud_notifier.py` (−562 lines). **PR-10** (#1111) extracted `signal_helpers.py`, `account_helpers.py`, `trade_helpers.py` (−412 lines). telegram_query_bot.py: 2630 → 2218 lines. Remaining handlers are command plumbing — further split if bot grows. |
...
| PR-11 | runtime_flags module + multiplexer pause check (D11) | Low | ✅ **MERGED** #1112 (2026-05-14) — 14 tests |
