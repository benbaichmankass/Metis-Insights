...
|----|------|----------|----------|-------|
| D1 | **pipeline.py monolith — in progress** | `src/runtime/pipeline.py` | LOW | **PR-6** signal builders → `strategy_signal_builders.py`. **PR-8** strategy-monocle gates → `strategy_monocle.py`, formatter → `pipeline_result.py`. **PR-9 in progress:** `_signal_to_order_package` → `order_bridge.py`, `_write_ict_signals_from_meta` → `signal_writer.py` — pipeline.py 729 → 626 lines (from 1412, −56%). Remaining: `run_pipeline` orchestrator (~290 lines), `multiplexed_signal_builder` (~50 lines). These are tightly coupled to the module; further split has diminishing returns. |
