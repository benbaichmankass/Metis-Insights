...
- **expected vs actual:** **Expected** — the repo's own stated standard, from `branch-protection-sync.yml`'s promotion comment for `diagnostic-provenance-guard`: *"the workflow SELF-TESTS its own failure path on every run"*. **Actual** — `provenance-consumer-guard` was promoted to a required gate on 2026-07-30 with no self-test and no test file, and **still has neither**. ⚠️ **CORRECTED 2026-09-12 (MI-280): this is REFUTED — all four now carry a planted-defect `--self-test`, shipped by MI-226 (#11592, `92f44a226`, 2026-09-10), and the fix was never carried back to this audit or to the backlog row.** VERIFIED by running each self-test and, one level up, by planting a defect in each guard's OWN detection (`return 1` -> `return 0` inside `main()`, located by AST so the self-test harness's own return is never touched) and requiring its self-test to go red: **4 of 4 CAUGHT**, each with a pre-plant green control and an assertion that the self-test actually RAN under the plant. Controls per guard: provenance 4, strategy-coverage 9, news-feed 4, workflow-failure-swallow 7. `check_workflow_failure_swallow.py` is self-referentially bad: its subject *is* silently-swallowed CI failure.
...
> ⚠️ **PARTIALLY SUPERSEDED — read the per-row corrections before rejecting an instrument's output.** Rows **1-4** were repaired by MI-226 (#11592, 2026-09-10) and re-verified by MI-280 (2026-09-12); their greens **are** admissible now. Row **5** (the other 21) is UNCHANGED and still reads *unproven, not disproven* — do not let rows 1-4 moving be read as the bucket moving. The table is left in place rather than rewritten because it is a dated record of what Phase 0 measured; the corrections are appended, which is the convention MI-226 itself used on F-61.

| # | instrument | why its green proves nothing |
|---|---|---|
| 1 | `scripts/check_provenance_consumers.py` | Required gate since 2026-07-30. **No self-test, no test file.** It is the declared preventer of the fabricated-PnL class; its green is unproven on the exact axis it was promoted for. ⚠️ **CORRECTED 2026-09-12 (MI-280): ADMISSIBLE as of MI-226 (#11592, 2026-09-10)** — it carries a planted-defect `--self-test`, and neutering its detection makes that self-test fail. See the correction on F-05 for the method and the counts. |
| 2 | `scripts/check_strategy_coverage.py` | Same — no self-test, no test file, inside the required `guards` job. ⚠️ **CORRECTED 2026-09-12 (MI-280): ADMISSIBLE as of MI-226 (#11592, 2026-09-10)** — it carries a planted-defect `--self-test`, and neutering its detection makes that self-test fail. See the correction on F-05 for the method and the counts. |
| 3 | `scripts/ci/check_news_feed_coverage.py` | Same. ⚠️ **CORRECTED 2026-09-12 (MI-280): ADMISSIBLE as of MI-226 (#11592, 2026-09-10)** — it carries a planted-defect `--self-test`, and neutering its detection makes that self-test fail. See the correction on F-05 for the method and the counts. Its `an EMPTY symbol population makes main() return 1` control is the strongest of the four: it refuses rather than reporting a pass on n=0, which is the zero-denominator class this very table is built to catch. |
| 4 | `scripts/ci/check_workflow_failure_swallow.py` | Same — and its subject *is* silently-swallowed CI failure. ⚠️ **CORRECTED 2026-09-12 (MI-280): ADMISSIBLE as of MI-226 (#11592, 2026-09-10)** — it carries a planted-defect `--self-test`, and neutering its detection makes that self-test fail. See the correction on F-05 for the method and the counts. |
...
| **F-61** `eight-registered-guards-have-no-proven-failure-path` | **Extends F-05 from 4 to 8.** `check_selftest_wiring.py`'s denominator is the 28 self-tests that already exist, not the 74 registered guards — so 8 are invisible to it, **including 4 that ship a `--self-test` `run_guards.py` never invokes**, which is precisely the defect that guard was built for: `check_cost_model_single_owner`, `check_role_pack_operating_layer`, `check_tp_venue_cap_single_owner`, `check_workflow_trigger_reachability` (self-test code present, **invoked=0**), plus the 4 from F-05. ⚠️ **CORRECTED 2026-09-09 (MI-226): the `invoked=0` half is REFUTED for all four.** Each IS invoked with `--self-test` by `run_guards.py`, self-test FIRST, with a load-bearing comment — `run_guards.py:438,465,1620,1636` — and all four execute green (`10/10`, `ALL PASS`, `13/13`, `ALL PASS`). The argv is wrapped across TWO source lines, which a single-line grep cannot see; the probe's positive control passed because it matched the *one-line* form, a different form from the one being sought. **The row's underlying claim stands and is not disputed** — `check_selftest_wiring`'s denominator IS the registry, which is what let F-05's four hide — so the class is 4, not 8. Mechanised: `check_guard_selftest_coverage.py` resolves invocations from the AST and carries a control that fails if it regresses to text matching. Full evidence: `BL-20260909-AUDIT-F61-REPORTED-FOUR-CORRECTLY-WIRED-GUARDS-AS-UNWIRED-BECAUSE-A-TWO-LINE-ARGV-IS-INVISIBLE-TO-A-ONE-LINE-GREP`. ⚠️ *The agent's first pass said 9 and corrected to 8 — `check_manifest_scope_constants` is covered by NAME via `guard_selftests.py`.* |
...
- `check_selftest_wiring` measures the registry, not the guard population — so guards with no proven failure path are invisible to it (F-05, F-61). ⚠️ **The figure `8 of 74` is CORRECTED to 4 (MI-226, 2026-09-09)**: F-61's other four are wired, and the denominator measured by filesystem glob is **80**, not 74. Re-measured on the same tree: 52/79 guards had a failure path resolving inside the required `guards` context, 22 rested on a pytest file alone, and **5 — not 4 — had no evidence anywhere**, the fifth being `check_guard_glob_coverage.py` (F-02's own subject). Now instrumented and ratcheted by `check_guard_selftest_coverage.py`.
