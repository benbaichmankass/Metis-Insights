...
| `GET /api/bot/backtests?limit=N&strategy=X&since=ISO_TS` | `src/web/api/routers/backtests.py` | **Added M5 P4 (2026-05-10).** Backtest-history feed for the dashboard. Reads `trade_journal.db::backtest_results` rows written by the M5 consumer (`src/backtest/run_backtest_m5.py`) and the standalone harness (`src/backtest/run_backtest.py`). `limit` clamped to 1..200 (default 50); `strategy` is exact match on `strategy_version`; `since` filters on `created_at`. Newest-first by `created_at` (ties broken by `id` DESC). Missing DB / missing table → 200 with `[]`. |
