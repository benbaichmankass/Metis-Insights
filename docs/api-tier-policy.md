...
| `GET /api/bot/work/checklist` | `routers/work.py` | **Added 2026-09-10 (MI-238, the live Workflow page).** `docs/claude/work/MANAGER-CHECKLIST.json`, served THROUGH `src/runtime/manager_status.py`'s existing grading rather than re-derived — the same module that renders the Telegram `/status` command, so the page and the bot cannot tell the operator two different stories about one row. ⚠️ **The `state`/`status` merge has ONE owner** (`manager_status.effective_state`): the file carries two competing status fields (MEASURED 2026-09-10 over all 244 items — 179 `state`, 83 `status`, 18 both, **13 DISAGREEING**, 65 `status`-only and therefore invisible to every existing consumer), and a second merge in a route or a renderer would become a second definition of an item's status, free to drift from the one the manager guards read. Every row's five-state `status.basis` ships beside its value (`state_only` / `status_only` / `agree` / `disagree` / `undeclared`, registered with `collapsed-state-guard` as `manager_status.status_basis`) because an `in_flight` reached on `agree` and one reached on `disagree` are the same value and different facts. ⚠️ **It carries its own as-of stamp** — the checklist's last commit sha + time + age, whether the working tree matches that commit (`null` = *we could not look*, never `false`), and the tree's state against `origin/main` — because the route reads the VM's WORKING TREE, which `ict-git-sync` pulls roughly every 5 minutes, so the page is exactly as fresh as the last PUSH plus that interval. A frozen page reading byte-identically to a live one is the coordination board's 2026-09-07 failure. Each row's declared fields are passed through VERBATIM under `fields` (the checklist's keys are free-form prose; an allowlist would silently drop the detail the page exists to show). Read-only, file-backed, no DB, no secrets, **no write surface** — the decision inbox's write route already exists and is unchanged. Best-effort: an unreadable checklist degrades to `present:false` WITH its `readState`, never a 5xx and never an empty list that reads as "no work". 20s in-process cache. |
| `GET /api/bot/work/decisions` | `routers/work.py` | The decision inbox — what is waiting on the operator, with answerable options. Four never-collapsed answer states (`not_submitted` / `in_transit` / `committed` / `unreadable`); `committed` is graded from the REPO, never from the transit log, so transit fails BACK. Publishes `writeGate.state` so a consumer can render "answering is closed" rather than a button that 503s. ⚠️ **Grades BOTH recording shapes as of 2026-09-10 (MI-254), and the four states were themselves a collapse until then.** An answer given through the route writes an `answer` block; an answer given IN CONVERSATION — how the overwhelming majority of decisions here are actually given — is written by hand as `verdict` + `chosen` + `answered_at`. Grading only the first made *nobody has answered* and *answered through the other channel* render IDENTICALLY, and the operator was shown three questions they had settled that morning. MEASURED over every `docs/claude/work/objects/*.yaml` (21 objects, 26 requests): 23 carry an `answer` block, **3 carry a `verdict` and no answer block**, 0 carry both. Seven never-collapsed states now: the original four plus `answered_in_conversation` (settled, but it never round-tripped, so it is NOT `committed`), `engaged_not_settled` (**`reframed_not_answered` — the operator responded WITHOUT settling; folding this into answered would HIDE a genuinely open decision, which is worse than the bug it fixes**) and `verdict_unrecognised` (*we could not grade it* — a grader that shrugs at a value outside its vocabulary IS the collapse). Every request carries `conversationalAnswer` (always as a key, `null` when absent) with its **`condition`**, which is never dropped: `OPEN-PRS.json`'s doctrine is that a verdict recorded without its condition is worse than a missing row because it reads as complete. `summary.decided` is read from `SETTLED_STATES` rather than re-derived, and splits into `decidedViaRoute` / `decidedInConversation` because only the first round-trips. Best-effort: degrades to `present:false` with the transit state `unreadable`, never a 5xx. ⚠️ **CORRECTED 2026-09-21 (E15).** The ONLY source of `requests`/`unanswerableOperatorEdges` used to be `docs/claude/work/objects/*.yaml`'s `decision_requests[]`; the 2026-09-21 operating reset archived that directory, so this route kept reading `present: true` with everything at zero — a genuine unanswered decision and no decision at all rendered IDENTICALLY, the exact collapse `check_collapsed_states.py` exists to catch. `unanswerableOperatorEdges` now ALSO carries two LIVE sources, each entry tagged `source`: live `blocked_on: operator_decision` edges on `docs/claude/work/MANAGER-CHECKLIST.json` rows, and open (`state` not `done`/`killed`) `next_action: ask_operator` items from `docs/claude/work/PIPELINE.jsonl` (read-only — this route never writes to the pipeline store). `summary.waitingOnYouTotal` (`actionableCount` + `unanswerableOperatorEdgeCount`) is the number a consumer should read for "is anything waiting on me", never `actionableCount` alone, which stays structurally 0 while `objects/` is archived. A new top-level `sources` block carries each source's own read-state — `workObjects` (`read`/`archived`), `checklist` (`manager_status.FileRead`'s `read`/`absent`/`unreadable`), `pipeline` (`render_daily_brief.py`'s `read`/`partial`) — so "this source has nothing pending" and "we could not read this source" are never the same zero; the degraded (exception) envelope carries the same three keys, each `unknown`, and `waitingOnYouTotal: null` rather than a fabricated `0`. |
| `GET /api/bot/work/brief` | `routers/work.py` | **Added 2026-09-21 (A3a).** The six-section daily brief (what came due · taken under mandate · decisions for you · what moved · what is running · spend), rendered LIVE from `scripts/ops/render_daily_brief.py::build()`/`render()` on every request — reads the pipeline store (`docs/claude/work/PIPELINE.jsonl`), this checklist, and `config/mandates.yaml` (today: absent, B5 not built). **No writer step and no cron**: unlike the pre-reset four-section module it replaced, none of the six sections needs a live `mcp__*` observation, so the route computes the brief straight from files already on the working tree — the same shape `/checklist` above already uses. Carries the same working-tree freshness discipline (`ict-git-sync` pulls roughly every 5 minutes, so the brief is exactly as fresh as the last PUSH plus that interval). `inputs` ships each of the three sources' three-way read state (`read`/`absent`/`unreadable`) verbatim, never collapsed. Read-only, file-backed, no DB, no secrets, **no write surface**. Best-effort: a build failure degrades to `present: false` WITH a reason, never a 5xx. 20s in-process cache. |
| `GET /api/bot/work/schedule` | `routers/work.py` | **Added 2026-09-21 (A9).** The work schedule — cadenced sessions (the daily sync, the three project-level reviews) · decisions owed · monitoring coming due · cron cadences — rendered LIVE from `scripts/ops/render_schedule.py::build()`/`render()` on every request. Renders what already exists: reads `docs/claude/work/SCHEDULE.json` (the ONE new declaration — the daily-sync/review cadences have no other machine-readable home), the pipeline store (`docs/claude/work/PIPELINE.jsonl`, via `scripts/ops/pipeline.py::due()` — never re-parsed), and `.github/workflows/*.yml`'s real `on.schedule` YAML trigger (a commented-out cron, e.g. `due-list.yml`, is correctly absent — never a text grep for the word "schedule:"). `inputs` ships each of the three sources' three-way read state (`read`/`absent`/`unreadable`) verbatim, never collapsed. Carries the same working-tree freshness discipline as `/checklist` and `/brief` (`ict-git-sync` pulls roughly every 5 minutes). Read-only, file-backed, no DB, no secrets, **no write surface**. Best-effort: a build failure degrades to `present: false` WITH a reason, never a 5xx. 20s in-process cache. |
| `GET /api/bot/work/receipts` | `routers/work.py` | **Added 2026-09-17 (MI-290, the canonized session receipt).** One SESSION RECEIPT per session from `docs/claude/work/receipts/<session_id>.json`, served to the Workflow page beside the checklist. Operator-directed: *"i want this format of summary cannonized so that this is what i get whenever i ask a session for a session receipt, and this is what should be presented on the web page"*. ⚠️ **This route computes nothing** — the format has ONE owner (`.claude/skills/session-receipt/SKILL.md`) and its measured half ONE producer (`scripts/ops/session_receipt.py`); re-deriving either here would create a second definition of what a session did, free to drift from the one the operator reads. ⚠️ **ONE FILE PER SESSION, not a shared register**, and that is measured rather than stylistic: several lanes wrap at once, and the single-mutable-array shape is what made `session-board.json::merge_slot` move in 39 of the last 40 commits touching it, sending every armed branch `dirty` and restarting its CI. ⚠️ **Each section is graded `complete` / `scaffold` / `missing`** — an empty ERRORS block is an unfilled SCAFFOLD, never a session declaring it made no mistakes, which is the one thing the receipt format exists to stop a reader assuming. A file that fails to parse lands in `readErrors` and is never dropped; a stray filename that is not a session id is reported, not skipped. Carries the same working-tree freshness stamp as `/checklist` (`ict-git-sync` pulls roughly every 5 min, so a receipt appears at the author PUSH plus that interval). Read-only, file-backed, no DB, no secrets, **no write surface**. Best-effort: an unreadable directory degrades to `present:false` WITH its `readState`, never a 5xx and never an empty list that reads as "no session has worked". 20s in-process cache. |
...
| `GET /api/diag/bybit_raw_positions` | `routers/diag.py` | **Added 2026-09-09 (MI-221; `BL-20260909-NO-REPO-SURFACE-EXPOSES-THE-RAW-BYBIT-GET-POSITIONS-PAYLOAD-AND-THIS-HAS-NOW-BLOCKED-ROOT-CAUSE-ON-TWO-REAL-MONEY-P1S`).** The RAW `get_positions` payload — **un-deduped, un-filtered, zero-size rows INCLUDED**, every row carrying `position_idx` and `size`. It answers the one question no other surface can: **does the VENUE not return a position, or does the BOT drop it after the read?** Both existing readers (`account_open_positions._emit`, `account_bybit_open_orders`) skip `size <= 0` and dedupe by SYMBOL, so *genuinely flat*, *a zero-size hedge-mode sibling book was returned* and *no row was returned at all* are ONE observation to every consumer — a collapse that blocked root-cause on TWO real-money P1s one day apart (2026-09-08 `...-ROWS0`, and MI-221). Runs the `settleCoin` page AND a symbol-scoped read for every configured symbol, **including symbols the page already returned**, because the disagreement between the two views IS the finding (`BL-20260713-BYBIT2-BTC-SETTLECOIN-BLIND`); each row is tagged with the query that produced it. `size` is reported twice (`size_raw` string beside the float, with `size_parsed`) because coercing an unparseable size to `0.0` would MANUFACTURE the flat reading this route exists to distinguish. Three-way per-query `query_state` ∈ `rows_returned` / `no_rows` (the venue answered EMPTY — a real positive measurement) / `could_not_look` (the call raised — says nothing about the world); per-account `read_state` ∈ `not_bybit` / `could_not_look` / `raw_read`; counts are `null`, never `0`, when we could not look. Read-only: grades nothing, re-arms nothing, places NO order, returns no credential material. |
| `GET /api/diag/bybit_raw_closed_pnl` | `routers/diag.py` | **Added 2026-09-09 (MI-221).** The RAW Bybit `get_closed_pnl` records for one symbol and window — **un-matched, un-filtered**. ⚠️ **The SECOND, INDEPENDENT venue endpoint.** Every existing question about whether a position closed goes through `/v5/position/list`, and this repo has been wrong about that endpoint's answer twice in two days (`BL-20260908-BYBIT-POSITION-PROTECTION-GRADES-A-SYMBOL-OFF-ROWS0-SO-A-HEDGE-BOOK-READS-FLAT-AND-A-LIVE-POSITION-IS-CLOSED`, then MI-221). `/v5/position/closed-pnl` asks a DIFFERENT question — *did the venue book a realised close?* — sharing no filter, dedupe, cursor or `size` field with the position path, so it can confirm or refute a close independently. ⚠️ **Its only existing caller REDUCES it to one record**: `_bybit_closed_pnl_lookup` scores records against a trade's side, qty, entry price and time, returns the single best match and drops the rest into `rej_*` locals that never leave the function — so *the venue booked no close*, *it booked one our matcher rejected* and *we could not ask* are ONE observation to every consumer. This reduces NOTHING: every record in the window is returned in venue order, with `nextPageCursor` followed and REPORTED (`pages_read` / `next_page_cursor_seen` / `pages_truncated`) so a truncated page can never read as complete. `query_state` ∈ `rows_returned` / `no_rows` (the venue answered EMPTY — a real positive measurement) / `could_not_look` (the call raised — says nothing about the world), with `record_count` **`null`, never `0`**, on the last. Venue strings are preserved, never coerced. Read-only: matches nothing, grades nothing, places NO order, returns no credential material. |
| `GET /api/diag/bybit_raw_order_history` | `routers/diag.py` | **Added 2026-09-09 (MI-221).** The RAW Bybit `get_order_history` records for one symbol and window. ⚠️ **It answers the one question no surface could ask: WHEN was a protective leg cancelled, and WHY?** `trades.sl_order_id` / `tp_order_id` record that a leg WAS placed at entry (written only on the Partial branch, from a before/after snapshot diff of the venue's legs); `/api/diag/bybit_open_orders` shows what rests NOW. Between them there was no way to ask what became of a leg that no longer rests — consumed at the close (the position was protected throughout) or cancelled days earlier (it ran naked). On a real-money position those are OPPOSITE findings and nothing could tell them apart. ⚠️ Its nearest existing caller, `account_order_status`, NORMALISES the answer away — it returns `{order_id, status, filled_qty, avg_price, exec_time}` and drops `createdTime`, `updatedTime`, `cancelType`, `triggerPrice` and `stopOrderType`, exactly the fields that answer *when* and *why*; it is also per-orderId, so it cannot surface a leg the journal never recorded. `cancel_type` is the load-bearing field: Bybit distinguishes a user cancel from a venue-side clear (e.g. Partial legs cleared when the position closes), which is the whole difference between the two findings. Every order in the window is returned in venue order; `nextPageCursor` followed and REPORTED (`pages_read` / `next_page_cursor_seen` / `pages_truncated`). `query_state` ∈ `rows_returned` / `no_rows` (the venue answered EMPTY — a real positive measurement) / `could_not_look` (the call raised — says nothing about the world), with `record_count` **`null`, never `0`**, on the last. Venue strings preserved, never coerced. Read-only: grades nothing, places NO order, returns no credential material. |
