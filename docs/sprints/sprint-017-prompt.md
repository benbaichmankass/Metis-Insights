...
| T7 | LIVE smoke (autonomous on next VM pull) | One-shot systemd unit `ict-smoke-once.service` runs `scripts/smoke_test_trade.py` four times: sub-min on bybit_1, real on bybit_1, sub-min on bybit_2, real on bybit_2. Per CLAUDE.md autonomous-trading rule, **no per-trade operator confirmation**. The risk manager + `ALLOW_LIVE_TRADING` env interlock + the `qty <= 0.001` hard cap are the rails. Operator can `/halt` mid-flight if anything looks wrong | **live trade** | 5 m | T8 |
