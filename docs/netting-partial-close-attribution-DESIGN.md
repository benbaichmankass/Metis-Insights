...
| 4 | Scope the first ship to `bybit_1` (paper) before `bybit_2` (real money)? | **yes** — `bybit_1` carries the observed divergence; `bybit_2` audited clean 2026-07-30T11:48Z |

## 6. Pre-implementation measurements still owed

Cheap, Tier-1, all via the existing relays — none of these are done yet:

- **`sl_order_id` coverage** among currently-open rows, per account → decides whether (b)
  is worth building.
- **Current divergence census**: journal open-row sum vs exchange size per
  `(account, symbol, direction)`, both accounts, same moment. The last measurement is
  2026-08-01; the leg backlog item records the condition as *self-worsening*, so it should
  be re-measured before sizing the fix.
- **Pairs-owned open rows**, so constraint 2 has a concrete exclusion list.
