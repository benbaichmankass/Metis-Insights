...
| `market_features` | 2 (feature/context) | Engineered features (close-to-close rolling vol, log returns, vol bucket, hour/dow/lags) + **range-based vol estimators** (`parkinson_vol`/`garman_klass_vol`/`rogers_satchell_vol`/`yang_zhang_vol` — full-OHLC, S-MLOPT-S9) + **crypto funding/OI features** (`funding_rate`/`funding_rate_zscore`/`funding_rate_abs_z`/`open_interest_change`/`open_interest_change_zscore` — optional, as-of from a Bybit funding/OI side-stream, S-MLOPT-S11) + **order-flow / microstructure features** (`ofi`/`ofi_zscore`/`vpin`/`order_imbalance`/`rel_spread_mean`/`microprice_dev` — optional, as-of from a `market_microstructure` side-stream captured forward by `scripts/ml/orderflow_capture.py`, S-MLOPT-S10) + 2-class regime label, derived from `market_raw` | `ml/datasets/families/market_features.py` + `ml/datasets/{volatility_estimators,funding_oi_features,orderflow_features}.py` | `market_raw` (+ optional `funding_oi_path` / `microstructure_path` side-streams) | aligned to `market_raw` | regime classifier (WS5-B-PART-2 PR 2B; S-MLOPT-S9 vol; S-MLOPT-S11 funding/OI; S-MLOPT-S10 order-flow), future feature consumers |
...
| `backtest_results` | 1 (data) | ⚠️ **HISTORICAL — NO PRODUCER.** The M5 `/test` writer was removed 2026-08-20; it ran one hardcoded ICT engine regardless of the strategy named and stamped `sharpe_ratio`/`total_pnl_pct`/`max_drawdown_pct` as literal `0.0`, so those columns are **FABRICATED, not measured**. Rows kept as a record. | *(none — writer removed)* | `trade_journal.db::backtest_results` | *(no longer written)* | **Do not use as evidence.** Real backtests: `/api/bot/backtests/sweeps`. |
