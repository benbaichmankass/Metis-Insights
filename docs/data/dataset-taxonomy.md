...
> (2026-05-10). Updated through **S-AI-WS5-C** (2026-05-10):
> `trade_outcomes`, `market_raw`, `market_features`, and
> `setup_labels` are now buildable.
>
...
| `setup_labels` | 2 (feature/context) | Labels for pattern / setup quality (binary `won` + continuous `r_multiple`) | `ml/datasets/families/setup_labels.py` | `trade_journal.db::trades` (CLOSED, non-backtest, non-empty `setup_type`) | per closed setup-tagged trade | setup quality scorer (WS5-C); future cross-feature setup classifier |
