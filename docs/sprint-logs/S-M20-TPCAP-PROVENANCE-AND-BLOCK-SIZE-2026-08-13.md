...
| **(e)** | Change the sweep's **default split mode**? (criterion (2) of `BL-20260814-SPLIT-TARGETS-EXACTLY-THE-FLOOR`) | **NEW.** Now has a measured basis and a designed fix, and still moves verdicts fleet-wide. All 4 affected legs measured both ways: `htf` 95→24, `tlt` 56→22 (**shrank** rich windows), `mhg` 7→24 (**enlarged**, still short by **one trade**), `mes` fell back at lifetime 33. Fix the evidence supports: `max(derive(target = floor + margin), fixed_window)` — both terms load-bearing. **The margin value needs a boundary-loss distribution; the illustrative 30 rests on n=4.** Two SHIPPED real-money cells are currently refused as *"waiting for trades"* when they have 407 and 527 lifetime trades. |
