...
`ict_scalp_5m` from `bybit_2`'s `strategies:` list in `config/accounts.yaml`.

> **RESOLVED 2026-09-06 — the operator was asked and chose the narrower lever.** DECIDED,
> operator-approved in conversation on a question that named this exact tradeoff; scope
> `bybit_2` only, no condition. `config/strategies.yaml` is untouched and the leg stays
> `execution: live`. Rationale, in the operator's framing: stop the real-money bleed and
> **keep the paper books trading it** so evidence keeps accruing for the `bracket_geometry`
> sweep of § 3.2 — the one exit lever that is both unrun and applicable here. Shipped as the
...
- **It cannot say whether stopping the leg on `bybit_2` will fix anything**, because
  de-routing does not change the strategy — it stops real money paying for the answer. The
  accrual it buys is only useful if (1)–(4) get done. This applies to the lever actually
  pulled exactly as it applied to the `execution: shadow` one it replaced: neither is a fix,
  both are a stop.

**Recommendation (a proposal, not an action).** Keep the leg off `bybit_2`. Run the
...
**One consequence of the narrower lever that must not be misread when grading it.** Because
the leg keeps `execution: live` and keeps running on `bybit_1` and `bybit_portfolio`,
**order packages and paper fills for `ict_scalp_5m` will keep appearing after this ships.**
That is this change working, not failing — it is the entire reason the lever was chosen. The
correct grading query is `trades` rows carrying **both** `strategy_name='ict_scalp_5m'` and
`account_id='bybit_2'`; those, and only those, should stop.

**And the M20 tp_geometry ratchet does NOT move under this lever — do not copy #11115's
number across.** MEASURED this session against `docs/research/exit-refinement-coverage.json`
at `origin/main` ca73d00e, via `m20_coverage_rollup.cells(live_only=True)`: `live_cells=396,
unstamped=229, ceiling=229` — exactly tight, zero slack — and **identical after the
`config/accounts.yaml` edit**. #11115 correctly lowered the ceiling 229 -> 220 because it
flipped this leg's `execution` to `shadow`, and `cells(live_only=True)` filters on the
matrix row's own `execution` field, dropping all 9 of the leg's lever cells from the
denominator (396 - 387 = 9 = 229 - 220, which is why its arithmetic checks out). This change
touches no `execution` field, so the denominator is unchanged and **220 would have opened 9
cells of slack** — precisely the silent loosening
`BL-20260823-TP-GEOMETRY-RATCHET-LOOSENS-WHEN-THE-LIVE-DENOMINATOR-SHRINKS` exists to catch,
arrived at from the opposite direction.

The negative is load-bearing, so it carries a positive control rather than resting on a
code read: in the same run, `_funding_by_leg()` — the rollup's only `config/accounts.yaml`
reader — **did** flip `ict_scalp_5m` from `real_money` to `paper`, while `trend_donchian`
(still routed to `bybit_2`) held at `real_money`. The instrument demonstrably sees this
change; the ratchet's non-movement is a measured negative, not a dead probe. That
`accounts.yaml` reader feeds only the stale-DECISIONS funding banner
(`m20_coverage_rollup.py:470,1719`), never the ceiling path.
