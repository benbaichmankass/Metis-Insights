...
4. **The coordination-board START comment could not be posted.** Issue #6927 returned `403 Resource not accessible by integration` on both repo names and through both the MCP tool and the REST API with `GH_TOKEN`. CLAUDE.md already records a prior session hitting `issue_write create -> 403` (`OI-20260901-CLAUDE-CHANNEL-SEPARATION-SHIPPED-BUT-UNPROVEN`). **The board protocol is unenforceable from a session with this token scope**, which means the board understates concurrent activity by an unknown amount.
