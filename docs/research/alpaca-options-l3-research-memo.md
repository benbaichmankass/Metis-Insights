...
> **Honesty flag:** I could not independently confirm in the repo the stated claim that "the vol-router beats the frozen-edge label in backtest + walk-forward." `detect_vol_regime` exists but is observe-only and never order-influencing, and there is no equity-ETF regime model. If that result lives in a sprint-log/backtest artifact, point me at it and I'll wire the regime-fit thread to it precisely.
