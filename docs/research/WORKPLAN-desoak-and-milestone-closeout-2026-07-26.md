...
> **Doc status:** `closed_unfinished` · category `plan` · last verified `2026-09-11` · registered in [`docs/DOCUMENT-INDEX.md`](../../docs/DOCUMENT-INDEX.md)
...
| **0.WS-5** | Legibility/reliability: log the silent FRED-fetch swallow (audit E-1, `fred_adapter.py`); (follow-on) trainer-vm-diag SSH-drop keepalive (`BL-20260721`); scheduled-cron liveness monitoring for the macro producers. | T1 | E-1 fix; keepalive; producer-liveness monitor | **DONE** — E-1 fixed; trainer-diag keepalive `ServerAliveInterval=30/CountMax=10` shipped (#7639, `BL-20260721` resolved); **macro-producer cron liveness monitor** (`scripts/macro/check_producer_liveness.py` + `.github/workflows/macro-producer-liveness.yml`, daily 12:00 UTC, alerts Telegram+issue+red-run on a >48h-stale `valuation_snapshots.jsonl`, silent+auto-resolve when fresh) — the "silently-skipped scheduled job" dead-man switch |
...
|---|---|---|---|
| **1.1** | **Label-volume plumbing — WS-B candle-shard coverage expansion** (alt-USDT ADA/AVAX/XRP + equities/metals daily → every closed trade path-resolves → eval book grows past the ~376-row wall). Then L3 paper-book labels + L5 net-R label swap. **Scoped 2026-07-26** → [`WS-B-candle-shard-labelvol-scoping-2026-07-26.md`](./WS-B-candle-shard-labelvol-scoping-2026-07-26.md): the shard *roster* already landed (PR #6934), but the eval-book harness (`m23_phase2_labelvol.sh`) still hardcodes BTC/ETH/SOL @1h and never consumes the new shards — so the real remaining plumbing is 3b (widen the harness to the full covered roster at correct per-symbol timeframes) + 3a/3c (trainer-VM verify + rerun). Tier-1 / offline. **3b DONE (PR #7677) + 3a/3c MEASURED 2026-07-27** ([`M23-phase2-labelvol-eval-widening-2026-07-27.md`](./M23-phase2-labelvol-eval-widening-2026-07-27.md)): the widening is mechanically correct (eval book 383→400; XRP 8 + ADA 6 + IEF 1 now path-resolve) but the payoff is small and **diagnostic** — AVAX/SOL + the whole equities/metals fleet added ~0 because the eval book is **real-money-closed-trade-COUNT-bound, not shard-coverage-bound** (BTC dominates real-money volume; alt/equity activity is on excluded paper/demo accounts). EV-gate still FAILs at usable volume. **Redirect: the real lever is L3 paper-book labels (operator-gated design call — admit paper closed trades as a distinct tagged eval population) + offline discrimination (1.2), NOT more shards.** | T1 | The single binding constraint on the whole ML program (M23/M24-P4/G1-G2 all starve without it). The one heavy investment. |
