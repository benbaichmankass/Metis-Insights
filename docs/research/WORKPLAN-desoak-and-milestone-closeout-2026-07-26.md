...
|---|---|---|---|
| **1.1** | **Label-volume plumbing — WS-B candle-shard coverage expansion** (alt-USDT ADA/AVAX/XRP + equities/metals daily → every closed trade path-resolves → eval book grows past the ~376-row wall). Then L3 paper-book labels + L5 net-R label swap. | T1 | The single binding constraint on the whole ML program (M23/M24-P4/G1-G2 all starve without it). The one heavy investment. |
