...
- **It does not close the backlog row.** Its criteria require the analyses to *share* the declaration, and the two known callers are unwired: `stop_width_counterfactual_2026_09_11.py` is open under PR #12092 and touching it here would guarantee a conflict, and `stop_integrity_both_arms.py` is imported by this unit rather than converted by it. Wiring is the follow-up, and I would rather say so than let a green PR imply a closed row.
