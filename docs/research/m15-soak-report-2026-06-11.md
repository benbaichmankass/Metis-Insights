...
| ETF legs first session tick (post-fix) | **VERIFIED 13:30:21 UTC** — `spy_trend_long_1d` evaluated a real Donchian-30 channel over genuine Alpaca history (`close=725.58 within channel [708.435, 760.4]`), correctly non-actionable; no errors, no insufficient-data path | #3376 (`journalctl` since 13:30) |
