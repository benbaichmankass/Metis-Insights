...
> ⚠️ **CORRECTED BEFORE THIS MEMO EVER LANDED — the stop-rate figure in the paragraph below is superseded.** MI-278 U15 ([`m20-u15-stop-integrity-both-arms-2026-09-12.md`](m20-u15-stop-integrity-both-arms-2026-09-12.md)) applied `BL-20260911-A-TRAILED-STOP-OUT-IS-ATTRIBUTED-TO-THE-DECLARED-GEOMETRY-THAT-DID-NOT-END-IT`'s requirement to it: **the +21.2pp is graded with `adjudicate_exit`, which reads `trades.stop_loss` — a field a trailing amend overwrites in place — and it does not state the declared-vs-amended split.** Restricted to stop-outs at the ENTRY-DECLARED stop it is **+7.6pp** on this memo's own row selection; and a second undeclared choice — which fan-out row is adjudicated — puts it anywhere in **4.3–28.2pp on identical data**, with the entire swing arriving through the CONTROL arm. **The sign survives every combination**, so the e35 stop-side effect is real at a third to a half of the size stated below, on an e35 pre cell of two stop-outs. ⚠️ **§4's excursion result is untouched** — a fixed window measured forward from entry does not depend on stop adjudication at all.
