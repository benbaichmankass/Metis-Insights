...
| 13 | Value sleeve (SPY←ERP / TLT·IEF·GLD·SLV←real-yield / GLD·SLV←GSR) | **D1 transform (change/detrend/accel) + D2 conditioning (level_x_turning) + D3 cross-section**, graded through the **P4 net-of-cost LIFECYCLE gate** (`thesis_backtest_run`, 30d cohorts, 10bps) — NOT the S2/S3 horizon-IC grader entries 2/12 used | `level_x_turning` (fundamental-momentum gate) | both TS + XS | value-native (cheaper→long) | **NOTHING CLEARS** (`edge_vs_baseline > 0` bar): every cell still **loses to naive all-long** net-of-cost over 21yr. BUT a genuine directional signal — **D1 `change`/`detrend` + D2 `turning` BEAT the level baseline AND flip calibration positive**: `change` n=837 win 0.515 mean_net **+0.0027** calib **+0.0224** edge **−0.0031**; `detrend` edge −0.0034 calib +0.0094; `level_x_turning` edge −0.0017 calib +0.0049 — vs baseline edge **−0.0047** calib −0.0037. `xsec` (−0.0059) + `accel` (−0.0066) + raw `level` (−0.0068) are worse than baseline. (`value_construction_sweep.py` on the committed 21yr backfill + real ETF candles, hosted US-IP runner, issue #7773 / PR #7766) | **Confirms the methodology's core hypothesis under the P4 arbiter: the value edge lives in the *shift*, not the *level*.** The change/detrend/turning transforms are a real, measurable improvement over the unconditioned level sleeve (better-than-own-baseline + positive conviction calibration), but the improvement is **sub-threshold** — none beats the naive-all-long benchmark net-of-cost, so nothing is deployable and **C4 (exit-conditioning) stays un-warranted** (no edge-positive base thesis to protect). `change` is the identified lever. Distinct result from entry 12 (that graded value D3 through the S2/S3 horizon-IC gate; this grades D1/D2/D3 through the P4 lifecycle gate — the arbiter the operator's directive names). **Next iteration (queued, not exhaustion):** D4 composite (`change ⊕ detrend`) + D5 horizon sweep on the `change` cell (find the hold horizon where its edge survives cost) + D2 regime-conditioning. If those stay sub-threshold, value is cross-gate-conclusively exhausted → pivot. **[DONE — entry 16: all sub-threshold; value is cross-gate-conclusively exhausted, pivot recommended.]** |
| 15 (M35) | Crypto **OI + basis MOMENTUM** (BTC/ETH/SOL) | **D1 transform** — ROC + rolling z-score of OI and basis at windows 7/14 (rising OI/basis → the feature); **directional sign read empirically** (not assumed contrarian) | none | time-series | empirical (sign from data) | `no_directional_edge`, worth_building=**False** — OVERALL null across all 8 features × 4 horizons (1/3/7/14d) × 2 windows; `pays_oos=False` everywhere. **Real coverage: 1000 aligned days/symbol** (this is the genuine grade after the #7600 coverage-0 fix — the earlier #7598 run's `no_edge` on **0 days** was a data-fetch bug, NOT a verdict, and was correctly never banked). Strongest in-sample cell `basis_roc7@1d` **IC −0.046, t=−2.49** (contrarian) collapses OOS (oosIC t=−1.82); `basis_roc7@14d` IS t=2.20 → oosIC t=0.15. Nothing survives OOS + 10 bps (`crypto_momentum_probe.py`, trainer-VM #7601) | **Closes the one crypto cell entry 3 never graded.** Entry 3 tested only the contrarian **level** (crowding fade); the OI/basis **momentum directional sign** was untested — this grades it honestly and finds **no directional edge**. So the "crypto exhausted" claim (entries 3/9/10/11, all funding/level) now covers momentum too, by measurement rather than assumption. The pattern matches the whole program: a couple of in-sample flashes (basis change at 1d/14d) that do not survive the honest OOS+cost bar. *Infra learning:* a coverage/plumbing failure (`n_days=0`) must be diagnosed and fixed before its verdict is trusted — the data-starved false-null trap (`RESEARCH-RIGOR-STANDARD.md`), caught here. |

| 16 | Value sleeve (same 9 drivers) | **D4 composite (`change ⊕ detrend`, equal- + IC-weighted) + D5 horizon sweep on `change` (7/14/30/60/90/180d) + D2 regime-conditioning (calm-vol / up-trend)**, graded through the **P4 net-of-cost LIFECYCLE gate** — the continuation entry 13 queued | `composite` / `regime` | both TS + XS | value-native | **NOTHING CLEARS** — every D4/D5/D2-regime cell still **loses to naive all-long** net-of-cost (`edge_vs_baseline < 0` everywhere; baseline −0.0047). **D4 equal-weight composite is the BEST-CALIBRATED construction in the whole program** — `composite_eq` n=674 win 0.522 mean_net +0.0015 calib **+0.0549** edge **−0.0034** (beats baseline, positive calibration, still sub-all-long). **IC-weighting HURT** — `composite_ic` calib +0.0160 edge **−0.0050** (< baseline: tilting to the higher-calib `change` leg lost the equal-weight averaging gain; the two legs are near-redundant, so diversification — not IC-tilt — is what a value composite buys). **D5:** `change`'s edge is negative at EVERY horizon and monotonically WORSE with the hold (7d −0.0025 → 14d −0.0025 → 30d −0.0031 → 60d −0.0102 → 90d −0.0147 → 180d −0.0342) — its mean_net rises with horizon (+0.0043@180d) but the all-long benchmark rises faster in the 21yr up-market, so no hold lets the value book beat buy-and-hold. **D2 regime:** `change_x_calm_vol` edge −0.0043 (marginally beats baseline), `change_x_uptrend` −0.0056 (worse); regime doesn't rescue it. Also fixed the `n=0` price-gate bug entry 13 flagged (weekly-Saturday `as_of` vs trading-day gate → new `_asof_gate` as-of-or-prior), so the D2 price-conditioners now actually score (`level_x_price_turning` n=610 edge −0.0097 — price-turning on the LEVEL actively hurts, consistent with the level being the weakest base). (`value_construction_sweep.py` on the committed 21yr backfill + real ETF candles, hosted US-IP runner via the push-triggered `m28-value-grade-push` workflow, run 30295695610 / PR #7777.) | **Value is CROSS-GATE-CONCLUSIVELY EXHAUSTED.** The full construction space — D1 transform · D2 conditioning (incl. regime) · D3 cross-section · D4 composite · D5 horizon — is now graded through BOTH arbiters (the S2/S3 horizon-IC grader, entry 12; AND the P4 net-of-cost lifecycle gate, entries 13 + 16) and produces **zero cost-surviving edges**. The directional signal is REAL — the *shift* beats the *level*, and blending it (D4 equal-weight) gives the program's single best conviction-calibration (+0.0549) — but its magnitude never overcomes buy-and-hold after cost, **at any horizon, in any regime, under any blend**. Same landing as COT (entries 2/5/7) and crypto (3/9/10/11/15): a real-but-sub-benchmark tilt, not a deployable edge. **C4 exit-conditioning is now permanently un-warranted** (no edge-positive base thesis to protect). *Methodology learning:* an IN-SAMPLE-OPTIMISTIC IC-weighted composite (weights read from the same full-sample grade) still failing to beat equal-weight is decisive — you cannot manufacture edge by tilting a blend toward the higher-IC leg when the legs are near-redundant. **Recommended pivot** (per the 2026-07-24 Escalation, already underway as M30/M36-D): (1) higher-freq microstructure off the existing feeds (Tier-1, no new cost), (2) the operator-gated non-FRED dataset (Schwab options-skew Track B / paid). Value construction is closed under both gates. |
...
- ~~**15 (M35) · Crypto OI/basis MOMENTUM (directional sign)**~~ — **DONE** (row 15 above;
  `no_directional_edge`, trainer-VM #7601). The one crypto cell entry 3 never graded — entry 3
  tested the contrarian *level* only; this grades the OI/basis *momentum* directional sign and
  finds no edge (OVERALL null, `pays_oos=False` at every feature × horizon, over 1000 real
  aligned days/symbol). "Crypto exhausted" now covers momentum by **measurement**, not assumption.
  *Caught the data-starved false-null trap:* the earlier #7598 run reported `no_edge` on **0
  aligned days** — a coverage/wrapper bug (`spot=` vs `category=`), fixed in #7600 and re-graded
  here; the 0-day run was never recorded as a verdict (`RESEARCH-RIGOR-STANDARD.md` § a
  coverage-0 run is a plumbing bug, not a research null).
...
- **Value is now also exhausted** — the distinct-driver D3 cross-section (entry 12, #7534)
  ...
  - **P4 re-test nuance (entry 13, 2026-07-27):** re-graded through the **P4 net-of-cost
    LIFECYCLE gate** (not the S2/S3 grader), the value D1/D2 transforms are **not** a flat
    null — `change`/`detrend`/`level_x_turning` **beat the level baseline and flip conviction
    calibration positive** (`change`: calib +0.0224, win 0.515, mean_net +0.0027) — confirming
    "edge lives in the shift, not the level." But every cell still **loses to naive all-long
    net-of-cost**, so nothing is deployable. One iteration remains before value is
    cross-gate-conclusive: **D4 composite (`change ⊕ detrend`) + D5 horizon sweep on `change`**.
    If those stay sub-threshold, the "value exhausted" verdict holds under BOTH arbiters.
