...
**Posted by `board-post.yml`** (third attempt). The first push did not trigger it at all; the second triggered but failed in 11s. Root cause, found by reasoning from *no result file despite `if: always()`*: the commit-back step ran `git add -f automation/board-posts`, and consuming the last pending request `git rm`s it, leaving an empty directory git does not track — so the path vanished and `git add` exited non-zero under `set -e`. The post loop was never the problem. `pr-opener.yml` firing on the same first push is what proved push-triggered workflows work here and the fault was mine.
