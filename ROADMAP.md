...
| **M13** | auto-claude (separate session — AI Analyst track) | AI Analyst — server-side LLM agent over live trading data, surfaced as a new FastAPI router on `:8001` | 📋 IN PLANNING (S1 scoped, not started). **NOT a registry model.** Tier-1 read-only endpoint; no influence on the order path. Anthropic Claude API + prompt caching, server-only key (`ANTHROPIC_API_KEY` already on the live VM). Four cached endpoints under `/api/bot/insights/*` (`summary`, `recent`, `strategy/{name}`, `health`), each returning `{summary_md, grade, signals[], data_window, generated_at, cache_age_seconds}`. Caches regenerated by `ict-insights-generator.{service,timer}` every 10 min so on-demand calls stay sub-100ms. Consumers: Streamlit dashboard + M12 Android companion. Pairs with the operator-invoked `/health-review` skill (same vendor, same data sources, different cadence). S1 deliverable: router skeleton + generator + tests + runbook + ROADMAP/sprint plan. **Out of scope (S1):** chat/Q&A, statistical anomaly detection, FCM push (M12 S4), writes to `comms/claude_strategy_scores.jsonl`. Detailed scope + the operator's session brief are in the M13 prompt; plan PR will follow. |
...
3. **M13 — AI Analyst (server-side LLM, bot repo).** New FastAPI router
   ...
   planning by a separate session; plan PR pending.
