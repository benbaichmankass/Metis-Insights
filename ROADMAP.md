...
| **M12** | mixed (auto-claude bot-side, operator-driven app-side) | Native Android companion app — push notifications + read-only dashboard + home-screen widget | 🔄 IN PROGRESS — S1 in flight (2026-05-26). Bot-side foundation merged (PR #2047: notifier module + `device_tokens` table + `/api/bot/devices/*` router + 37 tests). Operator actions + runbook in PR #2050 (green, awaiting merge: `enable-mobile-push`, `disable-mobile-push`). Observer hook re-add + `set-mobile-push-secrets` action + S1 sprint log + this status flip in the follow-up PR off `claude/m12-s1-android-scaffold-Se7to`. Android scaffold opened on `benbaichmankass/ict-trader-android#1` (single-screen Compose debug app: registers FCM token + displays incoming pushes). **S1 closes when the operator's phone receives a real `trade_closed` push from `bybit_1` end-to-end** — that's the verification gate. S2/S3 = Android repo mature + dashboard surfaces; S4 = event-driven notifications + subscriptions; S5/S6 = Glance home-screen widget (small → 4×5); S7 = release + runbook. Live order path explicitly untouched; new bot-side surface is one router + one new SQLite table + a feature-flagged, `try/except`-wrapped notifier that observes existing event sinks. See [`ROADMAP-ANDROID-COMPANION-APP-2026-05-26.md`](docs/sprint-plans/ROADMAP-ANDROID-COMPANION-APP-2026-05-26.md). |
| **M13** | auto-claude (separate session — AI Analyst track) | AI Analyst — server-side LLM agent over live trading data, surfaced as a new FastAPI router on `:8001` | 🔄 IN PROGRESS — S1 in flight (2026-05-26). **NOT a registry model.** Tier-1 read-only endpoint; no influence on the order path. Anthropic Claude API + prompt caching, server-only key (`ANTHROPIC_API_KEY` already on the live VM, reused from `ict-claude-bridge.service`). Four cached endpoints under `/api/bot/insights/*` (`summary`, `recent`, `strategy/{name}`, `health`), each returning `{summary_md, grade, signals[], data_window, row_counts, generated_at, cache_age_seconds, model_id}`. Mixed-model split: Haiku 4.5 on the high-cadence `summary`+`recent` endpoints, Sonnet 4.6 on the deeper `strategy/{name}`+`health` endpoints. Caches regenerated by `ict-insights-generator.{service,timer}` every 10 min so on-demand calls stay sub-100ms; `INSIGHTS_ENABLED=0` is the kill switch. Consumers: Streamlit dashboard + M12 Android companion. Pairs with the operator-invoked `/health-review` skill (same vendor, same data sources, different cadence). S1 = 5-PR scaffold (sprint plan → router → generator → systemd → sprint log); the verification gate is a live curl returning a real `summary_md` citing a real `trade_id`/`order_package_id`. **Out of scope (S1):** chat/Q&A, statistical anomaly detection, FCM push (M12 S4), writes to `comms/claude_strategy_scores.jsonl`. Plan: [`ROADMAP-AI-ANALYST-2026-05-26.md`](docs/sprint-plans/ROADMAP-AI-ANALYST-2026-05-26.md). |
