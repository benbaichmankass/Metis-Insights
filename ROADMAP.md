...
|---|---|---|---|
| **S-AI-WS7-INFLUENCE-OP** | **WS7 advisory influence operator — rollout step 1 (2026-05-25).** The "act" layer the ladder was missing: `src/runtime/advisory_influence.py` ships `apply_advisory_influence` (reductive-only — veto a trade to `qty=0` or annotate; an `_assert_reductive` guard makes increasing size / altering any risk field impossible), `AdvisoryPolicy` + `parse_policy`, gated by the existing `ADVISORY_MODE` flag (default off). **Inert: not wired to any strategy** — wiring onto a specific model/strategy is the separately operator-gated enablement (rollout step 2+). 12 invariant tests. Tier-3 (draft PR, explicit merge approval required). Design + open questions: [`ws7-advisory-influence-operator-DESIGN.md`](docs/sprint-plans/ai-traders/ws7-advisory-influence-operator-DESIGN.md). | 🔄 Step 1 done 2026-05-25 | M9 |
