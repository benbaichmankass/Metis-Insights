...
| **M5** | auto-claude | Strategy testing workflow | Telegram-triggered test flow, validation logging, backtest workflow docs | ✅ CLOSED 2026-05-10 — P1 #637 (consumer + validation log + dispatch guard), P2 #639 (subprocess + timeout + lock + env gate), P3 #640 (runbook + close-out docs), P4 (this PR — Tier-1 `GET /api/bot/backtests` feed for the dashboard's backtest-history tab). Companion dashboard UI tab queued in `benbaichmankass/ict-trader-dashboard`. Runbook: [`docs/runbooks/strategy-testing.md`](docs/runbooks/strategy-testing.md). |
...
1. **M6 — Web app UI (dashboard repo)** — Vercel SPA wiring of mock-data feeds (equity chart, Active ICT Strategies, Trading Conditions) to live `/api/bot/*` data; positions and signals to follow. The bot-side `/api/bot/backtests` feed for the optional backtest-history tab is live (M5 P4); dashboard side wires the consumer + UI.
2. **S-050-followup Phase-3 — HTF reference 4h → 1h EMA-200** — Tier 2/3, gated on ≥ 30 days of Phase-2 live metrics. Expected +0.4 Sharpe lift.
