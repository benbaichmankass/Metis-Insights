...
| **M1** | Energy event calendar + signals | Signals | EIA release history + **point-in-time** published consensus + realized values + MNG price series, joined. **Calendar/consensus/surprise half BUILT 2026-07-29** (Bigdata.com econ-calendar spine — `scripts/macro/econ_calendar_{data,produce}.py`, PIT snapshots at `comms/macro/econ_calendar_snapshots.jsonl`; [design](docs/research/M1-econ-calendar-spine-2026-07-29.md)). **Price-JOIN + surprise→forward-return measurement BUILT 2026-07-29** (`scripts/macro/econ_event_study.py` — IC(H)=Spearman(surprise, forward return) across trading-day horizons, NG=F/CL=F via the existing off-VM candle fetcher; [design](docs/research/M1-econ-event-study-2026-07-29.md)). **⚠️ 2026-07-30 — the "self-graduates as history accrues" claim above was FALSE on two counts, both corrected (operator-approved).** (1) The price join had **never returned a single bar** (`price_bars: 0`, `verdict: no_data`) — yfinance absent on the runner + a malformed Stooq URL for `=F` futures, hidden by a `\|\| echo "::warning::"`; at zero bars no amount of accrual graduates anything. (2) **Accrual was never the constraint** — the econ-calendar producer was simply the one macro producer built forward-only, **missing its backfill sibling** (cf. `valuation_snapshot_backfill.py` / `cot_snapshot_backfill.py` / `crypto_signals_backfill.py`). FRED serves the weekly release families (EIA gas storage, crude stocks, initial/continuing claims) **keylessly with decades of dated history**, so real n is in the thousands, not 7. **CONCLUDED 2026-07-30 — HONEST NEGATIVE.** Both backfill siblings built and run (model side `econ_calendar_snapshot_backfill.py` → 6,966 rows over 75y; survey side `econ_calendar_survey_backfill.py` → 12,076 rows). **M3 PASSED** — the gate's own satisfiability condition: `model_tracks_survey` at n=1263 (Spearman 0.5885 ≥ 0.50, sign agreement 0.720 ≥ 0.70), `min_honest_n` never lowered. Then, operator-directed, the study was re-run against **REAL survey consensus** rather than the model stand-in, 2015→2026: **natgas 553 releases / price_bars 2911, crude 575 / 2910, cpi_yoy 136 — ALL `no_edge_at_tested_horizons`**, and on BOTH energy kinds the strongest IC carries the **WRONG SIGN** vs the pre-registered hypothesis (positive where an inventory build predicts negative). This is a real null at horizons 1/3/5/10/21td with the market's own yardstick — not a thin-sample or dead-join artifact (price_bars non-zero, verified). It does NOT say macro events never matter; it says surprise→forward-return at these horizons does not pay. **Do not re-litigate without a new hypothesis** (different horizons, conditioning, or a non-surprise formulation). Caveat bounding any FUTURE use of the model expectation: it is a worse predictor than the survey on all three kinds (RMSE 1.304× / 1.358× / 2.603×) and is beaten by a naive random walk on `initial_jobless_claims` — see `BL-20260730-M3-GATE-TESTS-TRACKING-NOT-USEFULNESS`. Record: [`docs/sprint-logs/S-M1-SURVEY-CONSENSUS-NULL-20260730.md`](docs/sprint-logs/S-M1-SURVEY-CONSENSUS-NULL-20260730.md). Plan of record: [`RESEARCH-PROGRAM-2026-07-30.md`](docs/research/RESEARCH-PROGRAM-2026-07-30.md) §1–§2. | **Clean joined dataset over multiple years of releases** — where "surprise" is the release's **unanticipated component** measured against a **point-in-time expectation**, satisfiable EITHER by (a) archived published consensus, or (b) **[operator-approved 2026-07-30]** a **pre-registered PIT expectation model** (seasonal + AR fit strictly on data available before each release; leakage-safe by construction), **validated against the ~6 months of captured survey consensus on the overlap**. The realized-value half must stamp its **vintage basis** (keyless FRED = current vintage; ALFRED = true first prints) and must not present revised data as PIT. | (b) fails its overlap validation against captured consensus — i.e. model-surprise does not track survey-surprise where both exist → the expectation model is not a sound stand-in; fall back to (a) and re-scope the data source. **[Superseded 2026-07-30]** the original condition ("consensus not available point-in-time (revised-only) → stop") no longer halts the study, because the gate no longer depends on archived survey consensus — but the *anti-lookahead* requirement it protected is now carried by (b)'s strict before-release fit + the vintage-basis stamp. |
...
## Change log
- **2026-08-02** — **M28 value D4/D5 sweep — clean NULL for the STANDALONE-book expression; the value sleeve is a validated-but-weak lead, NOT dead (continue building).** Ran `value_construction_sweep.py` through the UNCHANGED P4 net-of-cost lifecycle gate (trainer relay, ~21yr ETF closes): the **D4 composite** (`change ⊕ detrend`, equal + IC-weighted), the **D2 regime-conditioning** of `change` (calm-vol, up-trend), and the **D5 horizon sweep** on `change` (7/14/30/60/90/180d) are **all sub-threshold on `edge_vs_baseline`** — every construction loses to naive all-long net-of-cost (best: `level_x_turning` −0.0017; `composite_eq` −0.0034 at the sweep's best calibration +0.0547; `change@180d` calib +0.0638 but edge −0.0340). **Scope (operator correction 2026-08-02 — "dead/exhausted" overstates it):** what is closed is precisely the trailing-percentile construction family (D1–D5) on the free daily value inputs, expressed as a **standalone long-biased cheap-long/rich-short book** vs an all-long benchmark — that configuration doesn't clear. The `change`/composite cells **calibrate positively** (a real weak signal), and the expressions suited to a weak signal are **untested and are the continue-building work**: (a) value as a **conditioning/overlay input** to another signal (not standalone); (b) a **market-neutral / vol-targeted** book vs a **fairer benchmark than all-long**; (c) **richer inputs**. M28 (a methodology program) continues. **C4 gate NOT re-run** (precondition unmet; C4 grades the same standalone expression). Observe-only, Tier-1. Doc: [`M28-D4-D5-value-construction-sweep-2026-08-02.md`](docs/research/M28-D4-D5-value-construction-sweep-2026-08-02.md); scorecard `comms/macro/value_construction_scorecard.json`; ledger entry 14.
...
- **2026-07-23 (cont. 9)** — **M28 reframed as a signal-research METHODOLOGY program (operator directive).**
  ...
  *first three data points in an iterative construction search*, not a ship-or-drop verdict. All three
  ...
  honest, cost-aware gate — not to ship these three. **Authored the full program design of record**
  [`M28-signal-RnD-program.md`](docs/research/M28-signal-RnD-program.md) — the cheap→expensive **funnel**
  (S0 feasibility screen *before any build* → S1 construct → S2 signal grade → **S3 PnL grade** →
  S4 productionize → S5 express+live), the research map, the phasing + decision checkpoints, and the
  **signal→PnL translation** + the one instrument still owed (a conviction-weighted net-of-cost portfolio
  PnL harness — IC ≠ PnL); plus its iteration-mechanics component
  ...
  **D4 composite**; the M29 sysdyn engine as a mispricing-signal generator graded on the *same* gate;
  ...
  repo-wide CI regression (ruff 0.16.0 default-ruleset expansion → pinned `<0.16`). **Phase A1 DONE** —
  the construction toolkit (`scripts/macro/signal_constructions.py`: D1–D4 pure transforms, 10 tests).
  **Next (Phase A2): the S3 PnL harness** (conviction-weighted net-of-cost portfolio backtest → Sharpe/
  PnL/maxDD/turnover — the "translates into PnL" instrument), then Phase B: work the construction space
  on the existing inputs (COT change/divergence, crypto funding×OI, cross-sectional, composites, sysdyn
  mispricing) through S2+S3, recording each in the ledger.
