...
## Change log
- **2026-08-02** — **M28 value D4/D5 sweep — the last construction iteration is a clean NULL; value is exhausted under BOTH arbiters.** Ran `value_construction_sweep.py` through the UNCHANGED P4 net-of-cost lifecycle gate (trainer relay, ~21yr ETF closes): the **D4 composite** (`change ⊕ detrend`, equal + IC-weighted), the **D2 regime-conditioning** of `change` (calm-vol, up-trend), and the **D5 horizon sweep** on `change` (7/14/30/60/90/180d) are **all sub-threshold on `edge_vs_baseline`** — every construction loses to naive all-long net-of-cost (best: `level_x_turning` −0.0017; `composite_eq` −0.0034 at the sweep's best calibration +0.0547; `change@180d` calib +0.0638 but edge −0.0340, worsening monotonically past 30d as the all-long baseline captures more of the same drift). This closes the "one iteration remains" clause of ledger entry 13: the value-input construction space (D1–D5) is exhausted, all NULL under the S2/S3 **and** the P4 arbiters. The `change` cell's positive calibration is a **banked lead** (a conditioning input to another signal, never a standalone cheap-long/rich-short book), not a deployable strategy. **C4 conditioned-lifecycle gate NOT re-run** (precondition — a construction clearing P4 — unmet). Observe-only, Tier-1. Doc: [`M28-D4-D5-value-construction-sweep-2026-08-02.md`](docs/research/M28-D4-D5-value-construction-sweep-2026-08-02.md); scorecard `comms/macro/value_construction_scorecard.json`; ledger entry 14.
