...
# ---------------------------------------------------------------------------
#
# The strategy ``monitor()`` loop is the primary path that closes open
# trades on TP/SL/VWAP-cross/time-decay. On Bybit spot-margin (the
# current production deployment for vwap on bybit_2) the entry order
# cannot carry an exchange-side bracket — Bybit V5 returns retCode
# 170130 for SL/TP on Market orders (see
# ``execute.py::_submit_order``). So a missed close verdict from the
# strategy means the position runs unprotected.
#
# This backstop closes that gap. Each tick it walks every DB-open trade,
# fetches the latest price for the symbol, and asks
# ``RiskManager.check_position_breach`` whether the trade row's own
# ``stop_loss`` / ``take_profit_1`` columns have been crossed. On
# breach: DB-close + exchange-close (via the same ``close_open_position``
# path the strategy monitor uses).
#
# Gated by ``RISK_MANAGER_BACKSTOP_ENABLED`` (default ``false``). This
# is its own flag — independent of ``MONITOR_RECONCILE_ENABLED`` and
# ``MONITOR_APPLY_TO_EXCHANGE`` — because turning the backstop on means
# the operator is opting into automatic exchange-side closes from a
# different layer than the strategy monitor; that decision needs its
# own opt-in. Same flip-at-runtime semantics as the other env flags
# (read at call time, no restart needed).

_RISK_BACKSTOP_FLAG = "RISK_MANAGER_BACKSTOP_ENABLED"


def _risk_backstop_enabled() -> bool:
    raw = os.environ.get(_RISK_BACKSTOP_FLAG, "false")
    return str(raw).strip().lower() in {"true", "1", "yes", "on"}


def _emit_risk_backstop_audit(
    *,
    trade_id: Any,
    account_id: Any,
    symbol: Any,
    ...
    stop_loss: Any,
    take_profit_1: Any,
    exchange_close_result: Optional[Dict[str, Any]],
) -> None:
    """Sticky audit row for a risk-manager backstop close.

    The strategy-monitor close path doesn't write to the audit log on
    every close, but the backstop firing means the strategy missed —
    which is high-signal for operators. Emit one structured outcome
    row per close so a tail of ``runtime_logs/signal_audit.jsonl``
    surfaces the layer that actually fired. Best-effort — never raises.
    """
    try:
        from src.utils.signal_audit_logger import log_signal
        log_signal({
            "event": "outcome",
            "action": "risk_manager_backstop_close",
            "status": "ok" if (exchange_close_result or {}).get("ok") else "partial",
            "db_trade_id": trade_id,
            "account_id": account_id,
            "symbol": symbol,
            "direction": direction,
            ...
            "exchange_result": exchange_close_result,
        })
    except Exception as exc:  # noqa: BLE001
        logger.warning(
            "_emit_risk_backstop_audit: write failed for trade_id=%s: %s",
            trade_id, exc,
        )


def _risk_manager_position_check(
    db,
    ohlcv_fetcher: Optional[Callable[[str, Optional[str]], Any]] = None,
) -> Dict[str, int]:
    """Risk-manager backstop sweep — emergency-close any DB-open trade
    whose latest price has breached its own ``stop_loss`` /
    ``take_profit_1`` levels.

    Runs after the strategy ``monitor()`` pass in each tick, so the
    strategy gets first crack at every package. The backstop only
    fires on trades the strategy left open. On breach the DB row
    transitions to ``status='closed'`` with ``exit_reason`` set to
    ``risk_manager_sl_breach`` / ``risk_manager_tp_breach`` and an
    exchange close is sent via the same ``close_open_position`` helper
    used by the strategy-monitor close path. The exchange close is
    idempotent on a flat position (Bybit returns a small or rejected
    market order that the existing close path tolerates).

    No-op when ``RISK_MANAGER_BACKSTOP_ENABLED`` is false or
    ``ohlcv_fetcher`` is None (no price source). Best-effort — every
    step is wrapped, one bad row never aborts the sweep.

    Returns
    -------
    Dict[str, int]
        ``{checked, breaches, closes_sent, skipped_no_cfg,
        skipped_no_price, skipped_dry, errors}``.
    """
    summary = {
        "checked": 0,
        "breaches": 0,
    ...
    if not _risk_backstop_enabled() or ohlcv_fetcher is None:
        return summary
    ...
    try:
        ...
        try:
            ...
            rows = conn.execute(
                ...
                "stop_loss, take_profit_1, notes, created_at "
                "FROM trades WHERE status='open' AND COALESCE(is_backtest,0)=0"
            ).fetchall()
        finally:
            conn.close()
    except Exception as exc:  # noqa: BLE001
        logger.warning("_risk_manager_position_check: open-trades read failed: %s", exc)
        summary["errors"] += 1
        return summary
    ...
    # Dedupe price fetches — many open trades on the same symbol cost
    # one ohlcv call instead of N. Bybit-side rate limits don't bite at
    # this volume but the latency saving compounds across the tick.
    ...
    def _last_price(symbol: str) -> Optional[float]:
        ...
        try:
            candles = ohlcv_fetcher(symbol, None)
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "_risk_manager_position_check: ohlcv fetch raised for %s: %s",
                symbol, exc,
            )
        ...
        try:
            if candles is None or len(candles) == 0:
                price_cache[symbol] = None
                return None
            # ``candles`` is a DataFrame with a ``close`` column in the
            # production fetcher; tests pass a small list-of-dicts shim.
            ...
            logger.warning(
                "_risk_manager_position_check: candle decode failed for %s: %s",
                symbol, exc,
            )
            price_cache[symbol] = None
            return None
    ...
    for r in rows:
        ...
        cfg = cfgs.get(aid)
        if cfg is None:
            summary["skipped_no_cfg"] += 1
            continue
        if str(cfg.get("mode") or "live").lower() in {"dry", "dry_run", "dry-run", "paper"}:
            summary["skipped_dry"] += 1
            continue
        ...
        trade_dict = dict(r)
        verdict = RiskManager.check_position_breach(trade_dict, price)
        if verdict is None:
            continue
        summary["breaches"] += 1
        ...
        # DB-side close — mirrors ``_mark_orphaned`` shape so the
        # strategy-monocle gate releases on the same notes-stamp idiom.
        notes = _decode_notes(r["notes"])
        notes.update({
            "risk_backstop_closed_at": now,
            "risk_backstop_closed_by": "risk_manager_backstop",
            "risk_backstop_reason": reason,
            "risk_backstop_price": price,
        })
        try:
            ...
            logger.warning(
                "_risk_manager_position_check: trade update failed for "
                "trade_id=%s: %s",
                r["id"], exc,
            )
            summary["errors"] += 1
        ...
        # Cascade the linked package so the gate is released.
        pkg_id = _extract_package_id(r["notes"])
        if pkg_id:
            try:
                db.update_order_package(pkg_id, {
                    "status": "closed",
                ...
                logger.warning(
                    "_risk_manager_position_check: package cascade failed "
                    "for pkg_id=%s trade_id=%s: %s",
                    pkg_id, r["id"], exc,
                )
                summary["errors"] += 1

        # Exchange-side close — always, not gated by
        # MONITOR_APPLY_TO_EXCHANGE. The backstop's own flag implies
        # the operator wants automatic intervention; deferring the
        # close to ``MONITOR_APPLY_TO_EXCHANGE=true`` would re-create
        # the same gap the backstop is supposed to fill.
        ex_result = _send_close_to_exchange(trade_dict)
        if (ex_result or {}).get("ok"):
            summary["closes_sent"] += 1
        else:
            # Don't increment errors — the close is best-effort and a
            # flat exchange position is a valid "close failed" outcome.
            logger.info(
                "_risk_manager_position_check: exchange close result "
                "for trade_id=%s: %s",
                r["id"], ex_result,
            )

        _emit_risk_backstop_audit(
            trade_id=r["id"],
            account_id=aid,
            symbol=symbol,
            ...
            exchange_close_result=ex_result,
        )

    if summary["breaches"] or summary["errors"]:
        logger.info(
            "_risk_manager_position_check: checked=%d breaches=%d "
            "closes_sent=%d skipped_no_cfg=%d skipped_no_price=%d "
            "skipped_dry=%d errors=%d",
            summary["checked"], summary["breaches"], summary["closes_sent"],
            summary["skipped_no_cfg"], summary["skipped_no_price"],
            summary["skipped_dry"], summary["errors"],
        )
    return summary
...
def run_monitor_tick(
    *,
    ...
    # Risk-manager active-position backstop. Runs BEFORE the
    # reconciler / watchdog chain so a breach detected here closes the
    # position cleanly via the same path the strategy uses, and the
    # downstream sweeps see the trade in a terminal status (no
    # spurious orphan-stamping race). No-op when
    # ``RISK_MANAGER_BACKSTOP_ENABLED`` is false (the default).
    try:
        backstop = _risk_manager_position_check(db, ohlcv_fetcher)
        if backstop.get("breaches") or backstop.get("errors"):
            summaries["__risk_backstop__"] = backstop
    except Exception as exc:  # noqa: BLE001
        logger.warning("run_monitor_tick: risk-manager backstop raised: %s", exc)
