...
def tradovate_client_for(account: Dict[str, Any]):
    """Return a ``TradovateAdapter`` for *account*, or ``None`` if creds missing.

    Mirrors :func:`bybit_client_for` / :func:`velotrade_client_for` so the
    coordinator's ``multi_account_execute`` client-construction switch has
    a uniform shape: factory returns ``None`` → loader/coordinator treats
    the account as not-configured and emits a diagnostic ping.

    Unlike Bybit / Velotrade, Tradovate doesn't use the ``api_key`` +
    ``api_secret`` pair pattern. Auth is a 7-tuple
    (username, password, app_id, app_version, cid, secret, device_id)
    that ``TradovateConfig.load()`` reads directly from ``os.environ``.
    Per-account override of the environment is the ``tradovate_env``
    YAML field (``demo`` | ``live``); when set we copy it into the
    process env before constructing the adapter so the same process can
    in principle build adapters for both environments. Default = demo.

    Returns
    -------
    TradovateAdapter | None
        Constructed adapter when all required env vars are populated;
        ``None`` otherwise. The adapter is built without a WebSocket
        attached — the WS is opt-in via ``attach_ws=True`` on
        ``TradovateAdapter.build``.
    """
    if not isinstance(account, dict):
        return None
    exchange = str(account.get("exchange", "")).lower()
    ...
    yaml_env = str(account.get("tradovate_env", "demo")).strip().lower()
    if yaml_env not in ("demo", "live"):
        logger.warning(
            "tradovate_client_for: account %s has invalid tradovate_env=%r; "
            "defaulting to demo",
            account.get("account_id") or "unknown", yaml_env,
        )
        yaml_env = "demo"

    # The standard env-var names ``TradovateConfig.load()`` looks for.
    required = (
    ...
    if missing:
        logger.warning(
            "tradovate_client_for: account %s not fully configured — "
            "missing env vars: %s",
            account.get("account_id") or "unknown", ", ".join(missing),
        )
        return None

    # Per-account env scope: copy the YAML toggle into the process env so
    # TradovateConfig.load() picks the right (demo|live) URLs.
    os.environ["TRADOVATE_ENV"] = yaml_env
    ...
    except Exception as exc:  # noqa: BLE001 — config errors surface as None
        logger.warning(
            "tradovate_client_for: TradovateConfig.load() failed for %s: %s",
            account.get("account_id") or "unknown", exc,
        )
        return None
    return TradovateAdapter.build(cfg, attach_ws=False)
