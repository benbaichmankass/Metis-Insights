...
def _submit_order(client: Any, order: dict, account_cfg: dict) -> str:
    ...
    # Tradovate futures (phase-1 wiring). Dispatches to the injected
    # TradovateAdapter built by tradovate_client_for(account_cfg). Mirrors
    # the IB branch's missing-client → ping contract: a None client raises
    # TradovateConfigError that the coordinator treats as "account not
    # fully configured" + emits a diagnostic ping. The adapter's own
    # dry_run flag (TRADOVATE_DRY_RUN env) is a *second* gate on top of
    # accounts.yaml::mode and strategies.yaml::execution — placement only
    # hits the wire when ALL THREE gates are open.
    if exchange == "tradovate":
        ...
        if client is None:
            raise TradovateConfigError(
                f"tradovate live placement: account "
                f"'{account_cfg.get('account_id') or 'unknown'}' is not "
                f"fully configured (no TradovateAdapter injected — "
                f"check TRADOVATE_USERNAME/PASSWORD/APP_ID/APP_VERSION/"
                f"CID/SECRET/DEVICE_ID env vars)."
            )
        if not isinstance(client, TradovateAdapter):
            raise TypeError(
                f"tradovate _submit_order: expected TradovateAdapter, got "
                f"{type(client).__name__}"
            )
        # Resolve the Tradovate accountId — the per-account YAML field
        # the operator populates after running list_accounts.cli once.
        account_id = int(account_cfg.get("tradovate_account_id") or 0)
        if account_id <= 0:
            raise TradovateConfigError(
                f"tradovate live placement: account "
                f"'{account_cfg.get('account_id') or 'unknown'}' has no "
                f"tradovate_account_id set in config/accounts.yaml. Run "
                f"`python -m src.units.accounts.tradovate.cli.list_accounts` "
                f"to discover it, then add it to the YAML."
            )
        ...
        except TradovateRiskRejection as exc:
            raise RuntimeError(
                f"Tradovate risk rejected {order['symbol']}: {exc.reason}"
                + (f" ({exc.detail})" if exc.detail else "")
            ) from exc
        except TradovateAPIError as exc:
            raise RuntimeError(
                f"Tradovate rejected order for {order['symbol']}: "
                f"HTTP {exc.status} {exc.payload!r}"
            ) from exc
