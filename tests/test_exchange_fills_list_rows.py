...
# Pinned reference clock. EVERY window-dependent `list_fills` call below passes
# `now=_NOW`, because the window is measured against the CURRENT time and the
# fixtures use ABSOLUTE dates — so without injection each test silently becomes a
# time bomb that passes until its fixture date ages past the window, then fails
# forever. `test_newest_first` detonated exactly that way on 2026-08-12, when its
# "old" fill (2026-08-05) crossed the 7-day boundary; the rest of the file was one
# day behind it, since `_fill`'s default `when` is 2026-08-06.
#
# The sibling `test_window_excludes_older_fills` already injected `now` for this
# reason. The lesson was learned once and applied to one test; this applies it to
# the file. A test whose pass/fail depends on the date it is RUN is not a test.
...
def test_returns_rows_not_sums(fills_db):
    ...
    rows = list_fills(7, path=fills_db, now=_NOW)
    assert len(rows) == 2
...
def test_symbol_filter_is_exact_and_bound(fills_db):
    ...
    rows = list_fills(7, path=fills_db, symbol="AVAX/USDT:USDT", now=_NOW)
...
def test_symbol_filter_does_not_accept_sql(fills_db):
    ...
    rows = list_fills(7, path=fills_db, symbol="' OR '1'='1", now=_NOW)
    assert rows == []
...
def test_account_filter_scopes(fills_db):
    ...
    assert [r["exec_id"] for r in list_fills(7, path=fills_db, account_id="bybit_1", now=_NOW)] == ["b1"]
...
def test_newest_first(fills_db):
    ...
    assert [r["exec_id"] for r in list_fills(7, path=fills_db, now=_NOW)] == ["new", "old"]
...
def test_limit_is_clamped_and_truncation_is_detectable(fills_db):
    ...
    rows = list_fills(7, path=fills_db, limit=3, now=_NOW)
    assert len(rows) == 3
    ...
    assert len(list_fills(7, path=fills_db, limit=100, now=_NOW)) == 10
...
def test_rows_carry_the_join_keys_an_attributor_needs(fills_db):
    ...
    row = list_fills(7, path=fills_db, now=_NOW)[0]
...
def test_raw_column_is_not_exposed(fills_db):
    ...
    upsert_fills([_fill("e1")], path=fills_db)
    assert "raw" not in list_fills(7, path=fills_db, now=_NOW)[0]
