...
# ---------------------------------------------------------------------------
# Phase 2 — HTF EMA trend gate (S-050; experiments/2026-05-08-all-models-training)
# ---------------------------------------------------------------------------

class TestHtfTrendGate:
    """Each test exercises one arm of the BUY-block / SELL-block / pass-through
    matrix. Mechanics are documented in vwap.py § Phase 2 HTF EMA trend gate."""

    def test_buy_passes_when_no_htf_inputs(self):
        """Backwards-compat: omitting HTF args restores Phase-1 behaviour."""
        df = _candles_below_vwap()
        signal = build_vwap_signal(df, symbol="BTCUSDT")
        assert signal["side"] == "buy"
        assert "htf_blocked" not in signal["meta"]

    def test_buy_blocked_when_htf_close_below_band(self):
        """4h close ≥ 2 % below EMA-200 → strong downtrend → block BUY."""
        df = _candles_below_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=95.0, htf_ema=100.0, htf_band_pct=0.02,
        )
        assert signal["side"] == "none"
        assert signal["meta"]["htf_blocked"] is True
        assert "htf_trend_block" in signal["meta"]["reason"]
        # No SL/TP on a no-trade signal.
        assert "entry_price" not in signal
        assert "stop_loss" not in signal
        assert "take_profit" not in signal

    def test_buy_passes_when_htf_close_inside_band(self):
        """4h close within ±2 % of EMA-200 → consolidation → BUY passes."""
        df = _candles_below_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=99.5, htf_ema=100.0, htf_band_pct=0.02,
        )
        assert signal["side"] == "buy"
        assert signal["meta"]["htf_blocked"] is False
        assert signal["meta"]["htf_close"] == 99.5
        assert signal["meta"]["htf_ema"] == 100.0
        assert signal["meta"]["htf_band_pct"] == 0.02

    def test_buy_passes_when_htf_close_above_band(self):
        """4h close > 2 % above EMA-200 means strong uptrend — only SELL is
        blocked here, BUY is *with-trend* so still passes."""
        df = _candles_below_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=105.0, htf_ema=100.0, htf_band_pct=0.02,
        )
        assert signal["side"] == "buy"
        assert signal["meta"]["htf_blocked"] is False

    def test_sell_blocked_when_htf_close_above_band(self):
        """4h close ≥ 2 % above EMA-200 → strong uptrend → block SELL."""
        df = _candles_above_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=105.0, htf_ema=100.0, htf_band_pct=0.02,
    ...
    def test_sell_passes_when_htf_close_inside_band(self):
        df = _candles_above_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=100.5, htf_ema=100.0, htf_band_pct=0.02,
        )
        assert signal["side"] == "sell"
        assert signal["meta"]["htf_blocked"] is False

    def test_sell_passes_when_htf_close_below_band(self):
        """SELL is *with-trend* in a downtrend → passes."""
        df = _candles_above_vwap()
        signal = build_vwap_signal(
            df, symbol="BTCUSDT",
            htf_close=95.0, htf_ema=100.0, htf_band_pct=0.02,
    ...
    def test_partial_htf_input_treated_as_no_gate(self):
        """When only one of htf_close / htf_ema is supplied, the gate
        degrades to no-op rather than producing partially-defined behaviour."""
        df = _candles_below_vwap()
        signal = build_vwap_signal(df, symbol="BTCUSDT", htf_close=95.0)
        assert signal["side"] == "buy"
        assert "htf_blocked" not in signal["meta"]

    def test_band_default_is_two_percent(self):
        """Sanity: HTF_BAND_PCT_DEFAULT matches the 2026-05-08 run finding."""
        from src.units.strategies.vwap import HTF_BAND_PCT_DEFAULT
        assert HTF_BAND_PCT_DEFAULT == 0.02
