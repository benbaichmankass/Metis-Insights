...
* RiskManager.approve refuses when daily loss > daily_usd, for both
  strategies.
* safe_place_order refuses when the kill-switch flag is set.
* (E3a) RiskManager.approve refuses when intra-day drawdown ≥ max_dd_pct.

NOTE (2026-06-28 audit): the cap tests previously drove the gate through the
vestigial ``TradingAccount.place_order`` (removed with ``integrator.route_order``);
they now assert the surviving ``RiskManager.approve`` primitive directly. The
arbitrary position-NOTIONAL cap was already removed 2026-06-24 (see below).
"""
...
# NOTE (2026-06-28 audit): these tests previously drove the cap through the
# vestigial ``TradingAccount.place_order`` (removed with ``integrator.route_order``).
# They now assert the surviving risk-gate primitive ``RiskManager.approve``
# directly — the same gate the live Coordinator path runs before raising/handling
# ``RiskBreach``. Coverage (daily-loss reject, the strict-`<` boundary, reset) is
# unchanged.
...
class TestMaxDrawdownIntraday:
    ...
    def test_drawdown_via_account_risk_manager_rejects(self):
        """The account-level gate: a TradingAccount's RiskManager rejects when
        the drawdown cap is breached (was driven through the removed
        ``place_order`` → ``RiskBreach``; now asserts the surviving primitive)."""
