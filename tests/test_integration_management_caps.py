...
def test_interactive_brokers_only_open_positions():
    # account_open_positions reads IB positions, but there is no IB modify/close
    # primitive and account_order_status returns None for IB — P3 wires those.
    ...
    assert clients.exchange_management_caps("ib") == frozenset({"open_positions"})


def test_alpaca_and_oanda_support_nothing_yet():
    # account_open_positions has no alpaca/oanda branch, and no modify/close.
...
def test_account_supports_management_resolver():
    ...
    # IB supports open_positions only — not modify/close.
    ...
    assert clients.account_supports_management(ib_acc, "close") is False

    # Alpaca supports nothing yet.
    assert clients.account_supports_management(alpaca_acc, "close") is False
