#!/usr/bin/env python3
"""
ICT Kill Zone Scalper - Terminal Control Panel (Milestone 6)
...
def build_dashboard():
    ...
    # Performance panel with daily drawdown
    perf_text = Text()
    ...
    daily_start = state["daily_start_balance"] or 0.0
    daily_limit = daily_start * state["daily_loss_limit_pct"]
    daily_used_pct = (abs(min(daily_pnl, 0)) / daily_limit * 100) if daily_limit > 0 else 0.0
    if state["daily_limit_hit"]:
        dd_color = "bold red"
        dd_status = "HALTED"
    elif daily_pnl < 0 and daily_used_pct >= 75:
        dd_color = "yellow"
        dd_status = "WARNING"
    else:
        dd_color = "green"
        dd_status = "OK"
    perf_text.append(f"Daily P&L: ", style="bold")
    daily_pnl_color = "green" if daily_pnl >= 0 else "red"
    perf_text.append(f"[{daily_pnl_color}]${daily_pnl:+,.2f}[/{daily_pnl_color}] ")
    perf_text.append(f"Daily DD Limit: ", style="bold")
    perf_text.append(f"[{dd_color}]{daily_used_pct:.1f}% used ({dd_status})[/{dd_color}] ")
    perf_text.append(f"DD Budget Left: ", style="bold")
    remaining = max(daily_limit - abs(min(daily_pnl, 0)), 0)
    perf_text.append(f"${remaining:,.2f} / ${daily_limit:,.2f} ")

    perf_panel = Panel(perf_text, title="[bold magenta]Account Info + Drawdown[/bold magenta]", border_style="magenta", width=40)
    ...
    alerts_text = " ".join(state["alerts"][-6:]) if state["alerts"] else "System initialization complete."
        ...
        try:
            ...
            if df_1h is not None and len(df_1h) >= 2:
                ...
                if last_close > prev_high:
                    BOT_STATE["trend"] = "bullish"
                elif last_close < prev_low:
                    BOT_STATE["trend"] = "bearish"
                else:
                    BOT_STATE["trend"] = "neutral"

            # Update Account Info
            balance = bybit.get_balance()
            if balance:
                usdt_data = balance['total'].get('USDT', 0)
                current_balance = float(usdt_data)
                BOT_STATE["balance"] = current_balance
                ...
                # --- Daily Drawdown Tracking ---
                today = date.today()
                if BOT_STATE["daily_date"] != str(today) or BOT_STATE["daily_start_balance"] is None:
                    # New trading day: reset baseline
                    BOT_STATE["daily_date"] = str(today)
                    BOT_STATE["daily_start_balance"] = current_balance
                    BOT_STATE["daily_pnl"] = 0.0
                    BOT_STATE["daily_limit_hit"] = False
                else:
                    daily_pnl = current_balance - BOT_STATE["daily_start_balance"]
                    BOT_STATE["daily_pnl"] = daily_pnl
                    daily_limit = BOT_STATE["daily_start_balance"] * BOT_STATE["daily_loss_limit_pct"]
                    if daily_pnl < 0 and abs(daily_pnl) >= daily_limit:
                        if not BOT_STATE["daily_limit_hit"]:
                            BOT_STATE["daily_limit_hit"] = True
                            BOT_STATE["alerts"].append(
                                f"{datetime.utcnow().strftime('%H:%M:%S')} | DAILY MAX DRAWDOWN HIT! Loss: ${abs(daily_pnl):,.2f} -- Trading HALTED"
                            )
